...
impl ExtractedLookup {
    ...
    fn zklean_pretty_print_base<const XLEN: usize>(
        ...
        let formula = self
            .graph
            .format_formula_for_lean(&formula_name, num_inputs)
            .map_err(std::io::Error::other)?;
        writeln!(
            f,
            "/- Readable arithmetic expansion emitted mechanically from the extracted graph."
        )?;
        writeln!(
            f,
    ...
    pub fn as_modules(&self) -> std::io::Result<Vec<Module>> {
        ...
        for table in &self.tables {
            if table.materializer.is_some() {
                ...
                let module_name = format!("Lookup{}", table.table_name);
                let mut body = Vec::new();
                table.zklean_pretty_print_base::<XLEN>(&mut body)?;
                main_imports.push(format!("Jolt.{module_name}"));
                modules.push(lookup_module(module_name, self.zklean_imports(), body));
            }
        }
...
mod test {
    ...
    fn emits_all_lookups_through_one_canonical_module() {
        ...
        assert_eq!(
            module.imports.len(),
            ZkLeanLookupTable::<XLEN>::iter().count() + 2
        );
        assert!(modules
            .iter()
            .any(|module| module.name == "LookupXor_64_lookup_table"));
        ...
        assert_eq!(
            lean.matches("_64_lookup_table_formula [Field f]").count(),
            ZkLeanLookupTable::<XLEN>::iter().count()
        );
        assert!(lean.contains(" v * "));
