...
- **Stall counter**: 39 (iter 95 P76-E REVERTED + STRATEGIC PIVOT — Re-applied the P76-E hot-path wiring (RuntimeState.eq_project_handles cache, materialize_binding signature extension, 3 handler call sites, end-of-execute drain) on top of iter-94's Arc-ified HandleStore infra. Correctness 50/50 PASS + clippy fully clean. Perf 92,229.39 ms modular vs 70,762.94 ratchet = +30.3% past reject — far worse than iter 93's +15%/+23% despite removing the Mutex serialization that diagnosed iter 93. Physically inconsistent with the code-level change being the cause, which means the regressions are dominated by measurement noise and/or by per-materialize HashMap<Vec<ChallengeIdx>, HandleId> lookup overhead that's an architectural cost (the cache key hash dominates the work it caches). **Pivot**: per user direction 2026-04-21, stop chasing sub-10% kernel-level P-attacks on a noisy bench with a stale iter-54 ratchet. Architectural audit now underway (tasks #119/#120/#121): map every source of ML-compiler-abstraction overhead (Op dispatch, device_buffers HashMap, DeviceBuffer enum arms, materialize_binding allocations, tracing spans, cross-round re-materialization), enforce zero-cost for the abstraction layer, preserve Hybrid/Metal interoperability. Target: ~10x modular speedup (19× gap → 1.9×). Revert iter 95 per protocol. Stall 38 → 39. Ratchet unchanged 70,762.94. iter 94 P92 INFRA-LANDED — Arc-ified `CpuHandleState::Eq`: `Vec<F>` → `Arc<Vec<F>>` in `crates/jolt-cpu/src/handles.rs:40`, `Arc::new(...)` wrap in `HandleStore::open`, and refactored `CpuBackend::eq_project_from_handle` in `crates/jolt-cpu/src/backend.rs:785-801` to clone the Arc out of `with_state` (drops the Mutex guard) then run `eq_project_with_table` on the clone. ~10 LOC delta. Fixes the iter-93 diagnosis: the global `HandleStore` mutex no longer held across the rayon-parallel dot-product. No production callers of `eq_project_from_handle` exist (grep confirms only test + trait + backend files), so this iter is structurally perf-neutral — pre-wiring infra for a later P76-E retry. Correctness 50/50 PASS + clippy jolt-core host + host,zk + modular lib+bin + jolt-cpu -D warnings clean + handle_scratch_roundtrip 7/7 PASS (covers scratch_roundtrip_serial, scratch_roundtrip_parallel, eq_shape_parallel_queries, eq_shape_queries_match_eq_polynomial_evals, eq_project_from_handle_matches_eq_project, eq_shape_bind_panics, scratch_handle_ids_unique_under_par). Perf: 72,940.18 ms modular (+3.08% in ±5% inconclusive band, core typical 3,964.90). Per iter-87/88 infra precedent (structurally perf-neutral infra lands flat without rerun), ratchet unchanged 70,762.94. Iter 95 will retry P76-E wiring on top of this unblocked infra. Stall 37 → 38. iter 93 P76-E REVERTED — re-applied the iter-75 hot-path wiring: added `RuntimeState::eq_project_handles: HashMap<Vec<ChallengeIdx>, HandleId>`, extended `materialize_binding` signature to take `&mut` of that cache, updated 3 handler call-sites (`Op::Materialize`, `Op::MaterializeUnlessFresh`, `Op::MaterializeIfAbsent`) to thread the cache, and added an end-of-execute handle-close drain. On cache miss: `backend.open_handle(HandleShape::Eq { challenges: &point, order: LowToHigh })` materializes the eq table once; on hit: `backend.eq_project_from_handle(id, ...)` reuses the cached table. Correctness 50/50 PASS (transcript_divergence, zkvm_proof_accepted_by_core_verifier, modular_self_verify, modular_self_verify_commit_skip_alignment all green); clippy jolt-core host + host,zk + modular lib+bin + jolt-cpu -D warnings clean. Perf: run1 81,383.64 ms (+15.01% vs ratchet 70,762.94, past-reject); run2 87,287.06 ms (+23.35% past-reject); mean +19.18%. **Diagnosis**: the iter-75 revert was NOT a stale-baseline artifact — the regression reproduces cleanly on the 70,762 ratchet. The root cause is in `CpuBackend::eq_project_from_handle` (crates/jolt-cpu/src/backend.rs:785-801): `HandleStore::with_state` grabs `entries.lock().unwrap()` on a process-global `Mutex<HashMap<HandleId, Box<dyn Any>>>` and HOLDS THAT GUARD for the duration of the inner closure — which runs the rayon-parallel `eq_project_with_table` dot-product. Any concurrent handle access (other materialize threads or nested rayon tasks touching other handles) must serialize on this lock. The naive `eq_project` path (rebuild table, parallel dot) has no global contention, so replacing it with the handle path adds a whole-process serialization point to the hot path. Eq-table materialization via `EqPolynomial::evals` is ~1-5 ms per build (small compared to the dot-product); the handle-cache win was only 9,766 ms CPU vs whatever lock contention introduced — which turned out to be dominant. The infra as it stands cannot be wired without restructuring `HandleStore` to expose the table via `Arc<Vec<F>>` (clone the Arc out of the mutex, drop the guard, then run the parallel dot with zero contention). Scope: ~30-50 LOC in jolt-cpu/src/handles.rs + a trait signature change to return a guard-less ref. Revert per protocol. Stall 36 → 37. Ratchet unchanged 70,762.94. **Lesson** (memory candidate): a global `Mutex` that guards a data structure used to hoist a large, parallel, in-place compute must expose the inner data via `Arc` / `&'static` / clone-out so the caller can drop the guard before the compute. Holding the outer guard across the parallel work serializes every other concurrent consumer of the same store. For eq-table caches specifically: the table is immutable after build, so `Arc<Vec<F>>` is the natural fit. iter 92 DESIGN — re-ranked untapped attacks from `perf/report_tools/kernel_gap_memo.md` after cycling through P87-P91 queue: P87 (prefix/suffix, 15-20% target) is major multi-iter surgery, deferred; P88 reverted iter 90 (WideAccumulator cache pressure); P89 reverted iter 91 (par_iter_mut overhead > late-round bind work); P90 has 5+ prior reverts (iter 63 and others — parallelize ram_val); P91 is DEAD per iter-90 cache lesson (WideAccumulator in eq_project). The kernel_gap_memo.md sensitivity analysis identifies 4 untapped buckets with concrete CPU savings: (c) eager witness polys 14,127 ms (move derived::ram_val etc. from lazy Op::Materialize to an eager Op::BuildWitnessPolys pass — untouched); (B) stateful Eq handle wiring 9,766 ms (P76-A/B/C/D infra landed iter 72-75 but hot-path wiring reverted in iter 75 P76-D "under noise" at baseline 104,246 ms; retry on fresh ratchet 70,762 ms is warranted); (d) PCS commit wrapper dedupe 7,700 ms (multi_pair_g2_setup_parallel is 2.65× slower than core — profile + dedupe the jolt-dory scheme wrapper); (A) persistent scratch in interpolate 4,000 ms (small win, enables (C) but (C) is reverted multiple times). **Iter 93 pick**: (B) P76-E re-wire eq handle hot-path. Justification: infra is 90% landed (HandleStore, CpuHandleState::Eq, eq_project_from_handle, eq_project_with_table helper), the reverted piece was just the RuntimeState cache + materialize_binding signature + handlers.rs call-site changes; ratchet dropped from 104,246 (iter 75) to 70,762 so the "noise that caused revert" was probably a stale higher-baseline artifact; targeted 9,766 ms CPU savings = ~7% wall projection; smaller blast radius than (c) which requires compiler/op-schedule changes. Stall 35 → 36. Ratchet unchanged 70,762.94. iter 91 P89 REVERTED — outer `par_iter_mut` on `CpuBackend::bind` inputs when `half < PAR_THRESHOLD=1024` AND `inputs.len() > 1`. Single-site change in `crates/jolt-cpu/src/backend.rs:184-199` adding a 6-line parallel branch before the serial fallback. Correctness 50/50 PASS (transcript_divergence, zkvm_proof_accepted_by_core_verifier, modular_self_verify, modular_self_verify_commit_skip_alignment — all green); clippy jolt-core host + host,zk + modular lib+bin + jolt-cpu -D warnings clean. Perf: run1 75,987.85 ms (+7.38% vs ratchet 70,762.94), run2 81,993.91 ms (+15.88%), mean +11.63% past reject. **Diagnosis**: per-buffer work in later sumcheck rounds is ~100-300 μs (inner `interpolate_vec_inplace` with half ∈ [1,512] × ~100 ns/bind-op). Outer `par_iter_mut` on a 5-10 item slice has ~20-50 μs of fork-join overhead per call — same order of magnitude as per-item work — plus cache-line bouncing on the `&mut [Buf<..>]` slice across cores and disrupted higher-level rayon stealing. Net negative. Revert per protocol. Stall 34 → 35. Ratchet unchanged 70,762.94. **Lesson** (memory candidate): don't add outer `par_iter_mut` on a dim where per-item work is < ~1ms AND outer count is < ~32. The fork-join overhead floor dominates. For `bind()`'s later-round pattern (5-10 inputs × ~200 μs each), outer parallelism is unwinnable; the real fix would need to either fuse binds into a single larger kernel (P77 direction, previously reverted) or accept that late-round binds are serial and look elsewhere. iter 90 P88 REVERTED — WideAccumulator swap in `gruen_segmented_reduce.reduce_outer` inner loop and `eq_project_with_table` hot loops (both branches + parallel + serial); q_const/q_quad in gruen and per-slot F in eq_project replaced with `F::Accumulator` using `fmadd`/`reduce`. Correctness 50/50 PASS (transcript_divergence, zkvm_proof_accepted_by_core_verifier, modular_self_verify, modular_self_verify_commit_skip_alignment — all green); clippy jolt-core host + host,zk + modular lib+bin -D warnings clean; jolt-cpu 76/76 PASS. Perf: run1 74,765 ms modular (+5.66% vs ratchet 70,762.94, past-reject by a hair), run2 83,679 ms (+18.25%, far past-reject), mean +11.9% past reject. **Diagnosis**: BN254 `WideAccumulator` is 9 limbs = 72 bytes vs `Fr`'s 4 limbs = 32 bytes (2.25× larger). `eq_project_with_table` allocates a per-chunk `Vec<F::Accumulator>` of size `chunk_size` and does an inner loop that writes to each element per eq_table entry — working set = chunk_size × 72 bytes overflows L2 (~512KB on x86, ~12MB on M-series) where the original `Vec<F>` at 32 bytes/elem fit. 2.25× cache pressure > the 30% Mont-reduce savings the acc gives. Also adds per-call allocation overhead: 72MB × 82 calls = ~5.9GB alloc churn. Revert per protocol. Stall 33 → 34. Ratchet unchanged 70,762.94. **Lesson**: `WideAccumulator` saves Mont reductions but doubles memory traffic — only a win in hot loops whose working set ALREADY fits L2 with F-sized elements. For buffers ≥ L2 size, the cache miss cost dominates. Rule: before swapping F→F::Accumulator in a per-element vec, check working-set fit. Inner-loop scalars (small num_evals like reduce_dense_fixed's NE=4) where accumulators stay in registers/L1 are where the trait shines. gruen's q_const/q_quad are only 2 acc per outer, fit in regs — swap would be fine there standalone, but the eq_project half dominated the change. iter 89 DESIGN — P84-C closed without implementing after sizing-analysis showed ~0.006% savings [~3 ms push-phase `F::from_i128` + ~1 ms bind-phase fast-path delta vs 71 s prove]; violates the documented hypothesis bar; ran fresh Perfetto trace, aggregated via `analyze_trace.py` → `perf/iter89-modular-spans.json`, 122,909 ms total_self_ms; top 3 = 63% (reduce_dense 25.1%, interpolate_inplace 20.6%, gruen 17.6%); queued P87-P91 below with sizing. Ratchet unchanged 70,762.94; iter 88 P84-B infra — `ComputeBackend::bind_compact` trait method + CpuBackend fast-path override via `Field::mul_i128(diff)` + two `jolt-cpu` unit tests landed; +~90 LOC across `jolt-compute/src/traits.rs` and `jolt-cpu/src/backend.rs`; correctness 50/50 PASS + clippy jolt-core host + host,zk + modular lib+bin + jolt-cpu --tests -D warnings clean; perf 2 runs both past reject (+20.4% / +17.65%) but with core typical (4,118 / 4,175 ms vs historical ~4,100–4,200) — environmental variance isolated to the modular bench window, not caused by this iter since grep confirms no production code path matches on `DeviceBuffer::Compact` or calls `bind_compact`; per iter-87 precedent, infra iters that are structurally perf-neutral land flat without rerun disambiguation; ratchet unchanged 70,762.94; iter 87 P84-A' infra — `DeviceBuffer::Compact { data: Vec<i128>, bits: u8, signed: bool }` variant + accessors landed (`as_compact` / `as_compact_mut` / `compact_encoding` / `is_compact`) in `crates/jolt-compute/src/traits.rs`; `jolt-hybrid` exhaustive matches patched with Compact arms (migrate no-op, on_primary true, test panics); +60 LOC total; no backend fast path yet, no hot-path wiring, no compiler changes; correctness 50/50 PASS + clippy jolt-core host + host,zk + modular lib+bin clean; perf 71,939.25 ms modular vs 70,762.94 ratchet = +1.66% in the ±5% inconclusive band, consistent with zero-hot-path-usage infra expectation; ratchet unchanged; iter 86 DESIGN — user directive against a core-style `SmallScalar` trait mid-implementation; pivot P84 framing to buffer-encoding-first: rewrite P84 to forbid a generic `SmallScalar` + `CompactPolynomial<T,F>` pattern and pin P84-A' as a runtime-tagged `DeviceBuffer::Compact` variant (encoding as *data*, not as generic type parameters); no code; iter 85 P86 REVERTED — dory cache seed is a no-op under `--stack both` because core's `DoryCommitmentScheme::setup_prover` seeds the dory-pcs shared global cache before modular's `DoryScheme::setup_prover` runs; the 167→93ms/call gap in iter84_modular_only.json only exists in solo modular runs, not the production benchmark; iter 84 DESIGN — fresh per-stack profiling disambiguates prior conflicting per-call ratios; appended P84/P85/P86 backlog items; no code; iter 83 P83 reverted — sparse eq_project for ram_ra_indicator, -3.9% median across 3 runs but rerun +6.78% in reject zone; iter 82 P78 reverted — bind_low_to_high in-place compact regressed +47% mean; iter 81 P77-D reverted — CpuBackend batch_round_evaluate par_iter override + re-applied emission switch regressed +6.86% mean; iter 80 P77-C reverted — emission switch alone regressed 10-14%; iter 79 P77-B infra — BatchRoundEvaluate handler landed, flat; iter 78 P77-A infra — BatchInstanceSpec/BatchReduceKind trait surface; iter 77 P77 infra — memoize RamRaIndicator; iter 76 P80 parallelize RAM derived polys — ~7.5% improvement vs same-session pre-change, but +12% past-reject vs stale iter-54 ratchet; code kept, ratchet unchanged; iter 75 P76-D hot-path wiring reverted under noise, infra kept; iter 74 P76-C infra; iter 73 P76-B infra; iter 72 P76-A infra stub; iter 71 P75-B infra stub; iter 70 P75-A infra-only commit; iter 69 design-only; iter 68 P73 reverted; iter 67 P72 reverted; iter 66 design-only commit; iter 65 P71 reverted; iter 64 P70 reverted; iter 63 P90 reverted).

  iter 91 P89 REVERTED — **outer par_iter_mut on CpuBackend::bind
  when inner would be serial**.

  **Change**: in `crates/jolt-cpu/src/backend.rs:184-199`, added
  a conditional outer-parallel branch to `CpuBackend::bind` for
  the `Dense | DenseTensor | Gruen` iteration types:
  ```rust
  #[cfg(feature = "parallel")]
  {
      if inputs.len() > 1 {
          let half = inputs[0].as_field().len() / 2;
          if half < PAR_THRESHOLD {
              use rayon::prelude::*;
              inputs.par_iter_mut().for_each(|buf| {
  ...
  // serial fallback unchanged
  ```
  Rationale: in later sumcheck rounds (rounds 7-16 at log_t=16)
  `half < PAR_THRESHOLD=1024` so inner `interpolate_vec_inplace`
  runs serially; outer-parallelizing across a kernel's 5-10
  input polynomials should extract otherwise-dormant parallelism.
  ...
  **Correctness**: 50/50 jolt-equivalence PASS (transcript_divergence,
  ...
  jolt-core host + host,zk + modular lib+bin + jolt-cpu -D warnings
  clean.

  **Perf**: run 1 modular 75,987.85 ms (+7.38% vs ratchet 70,762.94,
  past-reject); run 2 modular 81,993.91 ms (+15.88%, far past-reject);
  mean 78,991 ms (+11.63%). Core typical in both (5,371 / 3,917 ms).
  Both samples past the ≥5% reject band — **revert** per protocol.

  **Diagnosis**: per-buffer work in the targeted branch is
  ~100-300 μs (inner serial bind of half ∈ [1, 512] × ~100 ns
  per Fr add/mul). Rayon fork-join overhead for
  `par_iter_mut().for_each(...)` on a 5-10 item slice is
  ~20-50 μs per call plus thread-pool synchronization and
  cache-line ownership transfer for the `&mut [Buf<..>]`
  references. Per-bind this is same order of magnitude as the
  work it's trying to parallelize; over thousands of bind calls
  in the late-round regime the overhead accumulates net-negative.
  Additionally, nested or adjacent higher-level rayon contexts
  (e.g., commit / MSM threads running concurrently) see their
  stealing dynamics disrupted by many small outer par_iter_mut
  fork-joins, indirectly pessimizing unrelated work.

  **Lesson** (memory candidate): outer `par_iter_mut` on a
  dim where per-item work is < ~1ms AND outer count is < ~32
  is net-negative. Fork-join overhead floor dominates. This
  is why the existing `interpolate_vec_inplace` inner rayon
  branch is gated on `half >= PAR_THRESHOLD=1024` (≈ 1024 ×
  100 ns = 100 μs of work before per-thread overhead amortizes).
  The same threshold applies to outer dims: don't add outer
  parallelism unless per-item work is comfortably above rayon's
  fork-join floor. For `bind()`'s late-round pattern (many
  calls × few items × small work), outer parallelism is
  unwinnable; the real fix would need to either (a) fuse binds
  into a larger backend operation the kernel can batch
  internally (P77-style, prior reverts caution against this
  direction), or (b) accept late-round binds are serial and
  hunt the bigger waste elsewhere (reduce_dense, gruen_outer,
  ram_val). Saved as `feedback_rayon_outer_parallelism.md`.

  Stall 34 → 35. Ratchet unchanged 70,762.94. Next iter should
  target something with per-item work comfortably > 1 ms
  (reduce_dense kernels, gruen outer loop, eq_project inner
  chunks) rather than amplifying parallelism on tiny binds.
  ...
  **P84-C sizing**: the iter-89 plan was to route RD_INC through
  `DeviceBuffer::Compact` via `Polynomials::push` + an `Op::Bind`
  Compact match arm that calls `backend.bind_compact` on first-
  round bind. Savings decomposition:
  - Push-phase `F::from_i128` × T=2^16 = ~3 ms on BN254 (mul_u64
    with precomp table, ~50 ns/op).
  - First-round bind with `mul_i128(diff)` vs current
    `F::from_i128(diff) * scalar` = ~50 ns/op × T/2 = ~1.6 ms
    saved per pair-op.
  - Total expected: ~4 ms out of 71,000 ms = **~0.006%**.
  - Even routing ALL small-scalar polys (RD_INC + RAM_INC +
    indicators ≈ ~5 polys) nets under 30 ms = **<0.05%**.

  Violates the hypothesis bar documented in `memory/
  feedback_perf_hypothesis_bar.md` ("order-of-magnitude gains
  while modular is >2× off core; no micro-opts"). Close
  P84-C without implementing. The `bind_compact` trait
  method + CpuBackend fast path (iter 88) remain in tree as
  dead infra; keep them — they're correct, tested, and cost
  nothing — but don't wire them through the compiler until a
  hypothesis that NEEDS them surfaces.

  **Fresh profile** (`benchmark-runs/perfetto_traces/
  iter89_modular.json` → `perf/iter89-modular-spans.json`):
  stall ≥ 5 triggers re-profile per Perf Loop Protocol step 4.
  Aggregate self_ms across rayon workers = 122,909 ms;
  modular wall = ~71 s. Top hotspots:

  | Rank | Span                                | self_ms | %     | Calls | Mean ms |
  |------|-------------------------------------|--------:|------:|------:|--------:|
  | 1    | reduce_dense                        | 30,907  | 25.1% | 2,373 |    13.5 |
  | 2    | interpolate_inplace                 | 25,365  | 20.6% |17,416 |     1.5 |
  | 3    | CpuBackend::gruen_segmented_reduce  | 21,632  | 17.6% |    16 |  1352.0 |
  | 4    | CpuBackend::eq_project              |  9,581  |  7.8% |    82 |   117.7 |
  | 5    | multi_pair_g2_setup_parallel        |  9,290  |  7.6% |    62 |   165.9 |
  | 6    | derived::ram_val                    |  5,591  |  4.5% |     1 |  5591.5 |
  | 7    | CpuBackend::segmented_reduce        |  2,968  |  2.4% |    16 |   185.5 |
  | 8    | derived::ram_ra_indicator           |  2,864  |  2.3% |     2 |  1432.1 |
  | 9    | G1::msm                             |  2,488  |  2.0% |43,043 |     0.1 |
  | 10   | derived::ram_combined_ra            |  2,453  |  2.0% |     1 |  2452.7 |

  Top 3 = 63%; top 5 = 78.7%. Rank 5 (`multi_pair_g2_setup_parallel`)
  is a solo-run artifact per iter-85 lesson — skip as non-target.

  **New P-item queue** (ranked by expected impact ≥ 5%):

  - **P87 — prefix/suffix sumcheck on reduce_dense** (target
    15–20% prove_ms). Decompose the composition formula as
    `Σ P_i(prefix) · Q_i(suffix)`. Current reduce_dense evaluates
    the full composition at each grid point in every round —
    O(num_inputs × num_evals × half) per round. With prefix/
    suffix, the per-round work becomes O(prefix_size + suffix_size)
    plus a one-shot O(full) materialize step. For degree-d kernels
    savings scale with `d`. **Major surgery** — likely 5–10 iters
    to land (new `ClaimFactor::PrefixSuffix` variant on
    `jolt_compiler::module::ClaimFormula`, compiler emission
    change, runtime prefix/suffix materialize ops, backend method
    `reduce_prefix_suffix`). **Risk high**; **reward highest**.

  - **P88 — delayed reduction via WideAccumulator**
    (target 5–8% prove_ms). `reduce_dense` + `gruen_segmented_reduce`
    accumulate sumcheck evals with full field reductions per
    `F::mul + F::add`. BN254 already ships `WideAccumulator`
    (`crates/jolt-field/src/arkworks/wide_accumulator.rs` per
    `lib.rs:22`) that carries 5 uint64-limbs unreduced and only
    reduces at the end. Switching the per-input inner accumulator
    in `reduce_dense_fixed` (and the gruen inner loop) to
    `WideAccumulator` saves the per-mul montgomery reduce — on
    BN254 this is ~30-50% of field mul cost. Conservative
    estimate: 30% × `reduce_dense` + 30% × `gruen_segmented_reduce`
    = 30% × 52.5 s / 122.9 s total = ~12.8% of aggregate self_ms,
    which at modular's parallel saturation ≈ 4-6% wall. **Smallest
    architectural surgery** — accumulator type swap in the hot
    inner loops of the two biggest kernels. Preferred iter-90
    attack.

  - **P89 — outer-level parallelism on interpolate_inplace
    batches** (target 5-10% prove_ms, uncertain). 17,416 calls ×
    1.5 ms avg ≈ many are below `PAR_THRESHOLD=1024` and run
    serially. When `Op::Bind` dispatches to many polys in one
    op, today CpuBackend::bind loops serially over buffers. If
    we `par_iter_mut` over the poly-slice before falling through
    to `interpolate_vec_inplace`, we parallelize at the poly
    dimension when individual polys are small. **Low-medium
    risk** — one change in `backend.rs:188` inside `bind()`
    (and similar on `interpolate_inplace` callers in handler).
    Magnitude uncertain; depends on call-size distribution.

  - **P90 — parallelize `derived::ram_val`** (target 3-5% prove_ms).
    Single 5.6 s call, count=1, means single-threaded. If the
    derivation is data-parallel over T cycles or K addresses,
    rayon-ing it is a straightforward chunked loop. Needs reading
    `crates/jolt-zkvm/src/runtime/derived.rs` to identify the
    parallelization axis. **Low risk**.

  - **P91 — delayed reduction on eq_project** (target 3-5%).
    Same idea as P88 applied to the eq-weighted projection inner
    loops. 9.6 s / 122.9 s = 7.8%; 30% of that = 2.3% aggregate
    → ~1-2% wall. Smaller but cheap to stack after P88 (same
    accumulator pattern). **Low risk**.

  **Iter 90 plan**: **P88 delayed reduction** — smallest
  architectural surgery with defensible ≥5% expected gain,
  leverages existing `WideAccumulator`, compound-reusable for P91.
  Read `crates/jolt-cpu/src/backend.rs:reduce_dense_fixed` + gruen
  inner loop; identify the per-eval accumulator variable; swap
  from `F` to `WideAccumulator`; call `.reduce()` once at the
  inner loop end. Verify no correctness regression (50/50
  jolt-equivalence). Measure on sha2-chain log_t=16.

  Stall 32 → 33. No code change this iter. Ratchet unchanged
  70,762.94.
