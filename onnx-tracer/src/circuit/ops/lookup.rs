...
impl<F: PrimeField + TensorType + PartialOrd> Op<F> for LookupOp {
  ...
  fn f(&self, x: &[Tensor<F>]) -> Result<ForwardResult<F>, TensorError> {
    ...
    let res = match &self {
      ...
      LookupOp::RoundHalfToEven { scale } => Ok(tensor::ops::nonlinearities::round_half_to_even(
        &x,
        scale.into(),
      )),
      ...
      LookupOp::GreaterThanEqual { a } => Ok(tensor::ops::nonlinearities::greater_than_equal(
        &x,
        f32::from(*a).into(),
      )),
  ...
  fn out_scale(&self, inputs_scale: Vec<crate::Scale>) -> Result<crate::Scale, Box<dyn Error>> {
    let scale = match self {
      ...
      LookupOp::Recip { scale } => {
        ...
        out_scale += multiplier_to_scale(scale.0 as f64 / scale_to_multiplier(out_scale).powf(2.0));
        out_scale
      }
      LookupOp::Sign
