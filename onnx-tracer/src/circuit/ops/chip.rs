...
impl FromStr for Tolerance {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    if let Ok(val) = s.parse::<f32>() {
      ...
      Err(
        "Invalid tolerance value provided. It should expressed as a percentage (f32).".to_string(),
      )
    }
  }
}
...
// #[allow(missing_docs)]
// /// An enum representing activating the sanity checks we can perform on the
// accumulated arguments #[derive(
...
// #[cfg(feature = "python-bindings")]
// /// Converts CheckMode into a PyObject (Required for CheckMode to be compatible
// with Python) impl IntoPy<PyObject> for CheckMode {
...
// #[cfg(feature = "python-bindings")]
// /// Obtains CheckMode from PyObject (Required for CheckMode to be compatible with
// Python) impl<'source> FromPyObject<'source> for CheckMode {
...
// #[cfg(feature = "python-bindings")]
// /// Converts Tolerance into a PyObject (Required for Tolerance to be compatible
// with Python) impl IntoPy<PyObject> for Tolerance {
//     fn into_py(self, py: Python) -> PyObject {
...
// #[cfg(feature = "python-bindings")]
// /// Obtains Tolerance from PyObject (Required for Tolerance to be compatible with
// Python) impl<'source> FromPyObject<'source> for Tolerance {
//     fn extract(ob: &'source PyAny) -> PyResult<Self> {
...
//     /// the VarTensor reserved for lookup operations (could be an element of
// inputs)     /// Note that you should be careful to ensure that the lookup_input is
// not simultaneously assigned to by other non-lookup operations eg. in the case of
// composite ops.     pub lookup_input: VarTensor,
//     /// the (currently singular) output of the accumulated operations.
//     pub output: VarTensor,
//     /// the VarTensor reserved for lookup operations (could be an element of
// inputs or the same as output)     /// Note that you should be careful to ensure
// that the lookup_output is not simultaneously assigned to by other non-lookup
// operations eg. in the case of composite ops.     pub lookup_output: VarTensor,
//     ///
//     pub lookup_index: VarTensor,
//     /// [Selector]s generated when configuring the layer. We use a [BTreeMap] as
// we expect to configure [BaseOp].     pub selectors: BTreeMap<(BaseOp, usize,
// usize), Selector>,     /// [Selector]s generated when configuring the layer. We
// use a [BTreeMap] as we expect to configure many lookup ops.
...
// the operations.     /// * `check_mode` - The variable representing the (currently
// singular) output of the operations.     pub fn configure(
...
//                 nonaccum_selectors.insert((BaseOp::IsZero, i, j),
// meta.selector());                 nonaccum_selectors.insert((BaseOp::Identity, i,
// j), meta.selector());
...
//         for ((base_op, block_idx, inner_col_idx), selector) in
// nonaccum_selectors.iter() {             meta.create_gate(base_op.as_str(), |meta|
...
//                     BaseOp::IsBoolean => {
//                         vec![(qis[1].clone()) * (qis[1].clone() -
// Expression::Constant(F::from(1)))]                     }
//                     BaseOp::IsZero => vec![qis[1].clone()],
//                     _ => {
//                         let expected_output: Tensor<Expression<F>> = output
//                             .query_rng(meta, *block_idx, *inner_col_idx,
// rotation_offset, rng)                             .expect("non accum: output query
// failed");

//                         let res = base_op.nonaccum_f((qis[0].clone(),
// qis[1].clone()));
...
//                 let res =
//                     base_op.accum_f(expected_output[0].clone(), qis[0].clone(),
// qis[1].clone());                 let constraints =
// vec![expected_output[base_op.constraint_idx()].clone() - res];
...
//             // as all tables have the same input we see if there's another table
// who's input we can reuse             let table = if let Some(table) =
// self.tables.values().next() {                 Table::<F>::configure(
//                     cs,
//                     lookup_range,
//                     logrows,
//                     nl,
...
//                         // we index from 1 to avoid the zero element creating
// soundness issues                         // this is 0 if the index is the same as
// the column index (starting from 1)

//                         let col_expr = sel.clone()
//                             * table .selector_constructor
...
//                         let not_expr = Expression::Constant(multiplier) -
// col_expr.clone();
...
//                         log::trace!("---------------- col {:?}
// ------------------", col_idx,);                         log::trace!("expr: {:?}",
// col_expr,);                         log::trace!("multiplier: {:?}", multiplier);
...
//                                     + not_expr.clone() *
//                                       Expression::Constant(default_x),
//                                 *input_col,
//                             ),
//                             (
//                                 col_expr.clone() * output_query.clone()
//                                     + not_expr.clone() *
//                                       Expression::Constant(default_y),
//                                 *output_col,
//                             ),
//                         ]);
...
//     pub fn layout_tables(&mut self, layouter: &mut impl Layouter<F>) -> Result<(),
// Box<dyn Error>> {         for (i, table) in self.tables.values_mut().enumerate() {
...
//                 // during key generation this will be unknown vals so we use this
// as a flag to check                 let mut is_assigned =
// !claimed_output.any_unknowns()?;                 for val in values.iter() {
