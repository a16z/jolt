...
impl<F: PrimeField + TensorType + PartialOrd> Op<F> for HybridOp {
  ...
  fn f(&self, inputs: &[Tensor<F>]) -> Result<ForwardResult<F>, TensorError> {
    ...
    let (res, intermediate_lookups) = match &self {
      HybridOp::ReduceMax { axes, .. } => {
        ...
        let inter_2 =
          (unit - tensor::ops::sum(&tensor::ops::nonlinearities::leakyrelu(&inter_1, 0.0))?)?;

        (res.clone(), vec![inter_1, inter_2])
      }
      HybridOp::ReduceMin { axes, .. } => {
        let res = tensor::ops::min_axes(&x, axes)?;
        let min_plus_one = Tensor::from(vec![x.clone().into_iter().min().unwrap() + 1].into_iter());
        let unit = Tensor::from(vec![1].into_iter());
        // relu(min(x + 1) - x)
      ...
      HybridOp::ReduceArgMax { dim } => {
        ...
        let inter = Op::f(&HybridOp::ReduceMax { axes: vec![*dim] }, inputs)?.intermediate_lookups;
      ...
      HybridOp::ReduceArgMin { dim } => {
        ...
        let indices = Tensor::from(0..x.dims()[*dim] as i128);
        let mut inter_equals: Vec<Tensor<i128>> = vec![indices.clone(), -indices];
        let inter = Op::f(&HybridOp::ReduceMin { axes: vec![*dim] }, inputs)?.intermediate_lookups;
        inter_equals.extend(inter);
  ...
  fn required_lookups(&self) -> Vec<LookupOp> {
    match self {
      HybridOp::ReduceMax { .. } | HybridOp::ReduceMin { .. } | HybridOp::MaxPool2d { .. } => {
        Op::<F>::required_lookups(&LookupOp::ReLU)
      }
