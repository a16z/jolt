...
impl<F: PrimeField + TensorType + PartialOrd> Op<F> for PolyOp<F> {
  ...
  fn f(&self, inputs: &[Tensor<F>]) -> Result<ForwardResult<F>, TensorError> {
    ...
    let res = match &self {
      ...
      PolyOp::Concat { axis } => tensor::ops::concat(&inputs.iter().collect::<Vec<_>>(), *axis),
      PolyOp::Slice { axis, start, end } => {
        if 1 != inputs.len() {
  ...
  //         stride,
  //         modulo,
  //       } => layouts::downsample(config, region, values[..].try_into()?, axis,
  // stride, modulo)?,       PolyOp::Resize { scale_factor } => {
  ...
  //       PolyOp::Einsum { equation } => layouts::einsum(config, region, &values,
  // equation)?,       PolyOp::Sum { axes } => layouts::sum_axes(config, region,
  // values[..].try_into()?, axes)?,       PolyOp::Prod { axes, .. } => {
  ...
  //         )?
  //       }
  //       PolyOp::Add => layouts::pairwise(config, region, values[..].try_into()?,
  // BaseOp::Add)?,       PolyOp::Sub => layouts::pairwise(config, region,
  // values[..].try_into()?, BaseOp::Sub)?,       PolyOp::Mult =>
  // layouts::pairwise(config, region, values[..].try_into()?, BaseOp::Mult)?,
  //       PolyOp::Identity => layouts::identity(config, region,
  // values[..].try_into()?)?,       PolyOp::Reshape(d) | PolyOp::Flatten(d) =>
  // layouts::reshape(values[..].try_into()?, d)?,       PolyOp::Pad(p) => {
  //         if values.len() != 1 {
  ...
  //         input
  //       }
  //       PolyOp::Pow(exp) => layouts::pow(config, region, values[..].try_into()?,
  // *exp)?,       PolyOp::Pack(base, scale) => {
