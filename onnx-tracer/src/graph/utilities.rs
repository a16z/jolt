...
// use super::{Rescaled, SupportedOp, VarScales};
...
//   tract_core::ops::{
//     cast::Cast,
//     cnn::{conv::KernelFormat, MaxPool, PaddingSpec, SumPool},
//   },
// };
    ...
    "Range" => {
      ...
      for (i, input) in inputs.iter_mut().enumerate() {
        if !input.opkind().is_constant() {
          return Err(
            "Range only supports constant inputs in a zk
circuit"
              .into(),
          );
        } else {
    ...
    "ScatterElements" => {
      ...
      let mut op = SupportedOp::Hybrid(crate::circuit::ops::hybrid::HybridOp::ScatterElements {
        dim: axis,
        constant_idx: None,
      });
    ...
    "GatherElements" => {
      ...
      let mut op = SupportedOp::Hybrid(crate::circuit::ops::hybrid::HybridOp::GatherElements {
        dim: axis,
        constant_idx: None,
      });
    ...
    "Cast" => {
      ...
      match dt {
          ...
          if input_scales[0] != 0 {
            replace_const(
              ...
              SupportedOp::Nonlinear(LookupOp::Cast {
                scale: crate::circuit::utils::F32(scale_to_multiplier(input_scales[0]) as f32),
              }),
            )?
          } else {
        ...
        DatumType::F16 | DatumType::F32 | DatumType::F64 => SupportedOp::Linear(PolyOp::Identity),
        _ => return Err(Box::new(GraphError::UnsupportedDataType)),
      }
    }
    ...
    "Resize" => {
      ...
      if !resize_node.contains("interpolator: Nearest") && !resize_node.contains("nearest: Floor") {
        unimplemented!(
          "Only nearest
