...
use tract_onnx::{
  prelude::{
    tract_itertools::Itertools, Framework, Graph, InferenceFact, InferenceModelExt, SymbolValues,
    TypedFact, TypedOp,
  },
        ...
        if constant.datum_type() == DatumType::TDim {
          ...
          unsafe {
            ...
            op.0 = std::sync::Arc::new(
              tract_onnx::prelude::Tensor::from_raw_dt(DatumType::TDim, constant.shape(), bytes)
                .unwrap(),
            );
          }
        }
                ...
                match mapping {
                  ...
                  OutputMapping::Stacked { outlet, axis, .. } => {
                    if !full_results.is_empty() {
                      let stacked_res =
                        crate::tensor::ops::concat(&[&full_results[*outlet], &outlet_res], *axis)?;
  ...
  pub fn run_onnx_predictions(
    ...
    for chunk in data_chunks {
      ...
      outputs.push(
        result
          .into_iter()
          .map(|t| crate::graph::utilities::extract_tensor_value(t.into_arc_tensor()).unwrap())
          .collect(),
      );
    }
    Ok(outputs)
    ...
    for (idx, node) in &self.graph.nodes {
      match node {
        ...
        NodeType::SubGraph { model, inputs, .. } => {
          ...
          table.with(
            tabled::settings::style::BorderColor::default().top(tabled::settings::Color::BG_YELLOW),
          );
          string = format!("{} \n\n  MAIN GRAPH \n\n{}", string, table);
