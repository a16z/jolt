...
// /// Label enum to track whether model input, model parameters, and model output
// are public, private, or hashed #[derive(Clone, Debug, Serialize, Deserialize,
// PartialEq, Eq, PartialOrd, Ord, Default)] pub enum Visibility {
//     /// Mark an item as private to the prover (not in the proof submitted for
// verification)     #[default]
//     Private,
//     /// Mark an item as public (sent in the proof submitted for verification)
//     Public,
//     /// Mark an item as publicly committed to (hash sent in the proof submitted
// for verification)     Hashed {
//         /// Whether the hash is used as an instance (sent in the proof submitted
// for verification)         /// if false the hash is used as an advice (not in the
// proof submitted for verification) and is then sent to the computational graph
//         /// if true the hash is used as an instance (sent in the proof submitted
// for verification) the *inputs* to the hashing function are then sent to the
// computational graph         hash_is_public: bool,
//         ///
//         outlets: Vec<usize>,
//     },
//     /// Mark an item as publicly committed to (KZG commitment sent in the proof
// submitted for verification)     KZGCommit,
...
// #[cfg(feature = "python-bindings")]
// /// Converts Visibility into a PyObject (Required for Visibility to be compatible
// with Python) impl IntoPy<PyObject> for Visibility {
...
// #[cfg(feature = "python-bindings")]
// /// Obtains Visibility from PyObject (Required for Visibility to be compatible
// with Python) impl<'source> FromPyObject<'source> for Visibility {
...
// PartialEq, PartialOrd)] pub struct VarVisibility {
...
// impl VarVisibility {
//     /// Read from cli args whether the model input, model parameters, and model
// output are Public or Private to the prover.     /// Place in [VarVisibility]
// struct.     pub fn from_args(args: &RunArgs) -> Result<Self, Box<dyn Error>> {
...
//             return Err(
//                 "public visibility for params is deprecated, please use `fixed`
// instead".into(),             );
//         }
...
//         let num_const_cols =
//             VarTensor::constant_cols(cs, logrows, num_constants,
// module_requires_fixed);         debug!("model uses {} fixed columns",
// num_const_cols);
