...
use crate::{
  circuit::ops::{
    hybrid::HybridOp, lookup::LookupOp, poly::PolyOp, Constant, ForwardResult, Input, Op, Unknown,
  },
  fieldutils::{felt_to_i128, i128_to_felt},
  graph::utilities::{
    multiplier_to_scale, new_op_from_onnx, node_output_shapes, quantize_tensor, scale_to_multiplier,
  },
      ...
      .filter(|i| !deleted_indices.contains(i))
    {
      if inputs.len() > input {
        ...
        if let Some(constant) = input_opkind.get_mutable_constant() {
          rescale_const_with_single_use(constant, in_scales.clone(), input_node.num_uses())
            .unwrap();
