...
def pad1(ids, L):
    a = np.full((1, L), PAD_ID, dtype=np.int64)
...
preds = []
for t in texts:
    tokens = pad1(tok(t), MAX_LEN)
    out = sess.run(None, {"tokens": tokens})  # single bool output
    y_bool = out[0].ravel()[0]
    preds.append(int(bool(y_bool)))
