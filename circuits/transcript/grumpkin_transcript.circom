// Poseidon hash is over Bn254 Scalar
...
template AppendScalar() {
    ...
    signal lo, hi;
    (lo, hi) <== Num2LoHi(3, 122)(scalar.limbs[1]);
   
    signal new_scalar_limbs[2];
    new_scalar_limbs[0] <== scalar.limbs[0] + (2 ** 125) * lo;
    new_scalar_limbs[1] <== hi + (2 ** 122) * scalar.limbs[2];

    var int_state[stateSize] = [0, transcript.state, transcript.nRounds, new_scalar_limbs[0], new_scalar_limbs[1]];
