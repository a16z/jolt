...
The claim reduction sumchecks can be found in `crates/jolt-prover/src/zkvm/claim_reductions/` and include:
...
The implementation is located in `compute_final_opening_point()` in `crates/jolt-prover/src/zkvm/mod.rs`.
...
So if $P$ is the smaller polynomial and $P_{\mathrm{emb}}$ is its embedding into the final matrix, then

$$
P_{\mathrm{emb}}(r_{\mathrm{final}})
=
