...
3. **Private Input** (`jolt::PrivateInput<T>`, requires `zk` feature): Equivalent to `UntrustedAdvice<T>` but only available when zero-knowledge mode is enabled. Using `PrivateInput<T>` guarantees that the input is cryptographically hidden from the verifier — the [BlindFold](../../how/blindfold.md) protocol ensures no information about the input leaks from the proof.
...
// Setup: functions with PrivateInput<T> params get a 3-arg preprocess_verifier
...
When a guest function has `PrivateInput<T>` parameters, the generated `preprocess_verifier_*` function takes a third `BlindfoldSetup` argument. Functions without `PrivateInput<T>` get a simpler 2-argument signature.
