...
1. **Input**: These are public inputs known to both the prover and verifier. The prover proves that given the bytecode and these inputs, the claimed output is correct.

2. **Untrusted Input**: These inputs are only known to the prover, not the verifier. Here, the prover proves that there exists some input that produces the claimed output, without revealing the actual input to the verifier.
