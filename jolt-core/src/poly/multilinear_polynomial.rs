...
impl<F: JoltField> MultilinearPolynomial<F> {
            ...
            .any(|poly| matches!(poly, MultilinearPolynomial::OneHot(_)))
        {
            ...
            for (coeff, polynomial) in coefficients.iter().zip(polynomials.iter()) {
                result = match polynomial {
                    ...
                    MultilinearPolynomial::I128Scalars(poly) => poly.mul_add(*coeff, result),
                    MultilinearPolynomial::U128Scalars(poly) => poly.mul_add(*coeff, result),
    ...
    /// Gets the polynomial coefficient at the given `index`, returning a field element
...
impl<F: JoltField> PolynomialEvaluation<F> for MultilinearPolynomial<F> {
    ...
    fn evaluate(&self, r: &[F]) -> F {
        match self {
            ...
            MultilinearPolynomial::I128Scalars(poly) => poly.optimised_evaluate(r),
            MultilinearPolynomial::U128Scalars(poly) => poly.optimised_evaluate(r),
