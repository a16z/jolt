...
//     pub K: usize,
//     num_rows: usize,
//     pub nonzero_coeffs: Vec<(usize, usize, F)>,
...
//             for (t, k, coeff) in self.nonzero_coeffs.iter() {
...
//             for (_, k, coeff) in self.nonzero_coeffs.iter() {
...
// impl<F: JoltField> PolynomialBinding<F> for IncPolynomial<F> {
...
//                 })
//                 .collect();
//             std::mem::swap(&mut self.nonzero_coeffs, &mut self.binding_scratch_space);
//         } else {
...
//             self.nonzero_coeffs.sort_unstable_by_key(|(_, k, _)| *k);
//             self.binding_scratch_space = self
//                 .nonzero_coeffs
...
//         assert_eq!(self.nonzero_coeffs.len(), 1);
//         self.nonzero_coeffs[0].2
//     }
// }
...
// //             .take(T)
// //             .collect();
// //         let mut inc_poly = IncPolynomial::<Fr>::from_indices(nonzero_indices, K);
...
// //             let inc_message = inc_poly.compute_sumcheck_prover_message(&split_eq);
