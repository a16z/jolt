...
#[derive(Debug, Serialize, Deserialize)]
pub struct SparseMatPolyEvalProof<F, ProofTranscript>
where
    F: JoltField,
...
impl<F: JoltField, ProofTranscript: Transcript> SparseMatPolyEvalProof<F, ProofTranscript> {
    fn protocol_name() -> &'static [u8] {
        b"Sparse polynomial evaluation proof"
    }
    ...
    pub fn prove(
    ...
    ) -> SparseMatPolyEvalProof<F, ProofTranscript> {
        ...
        let (mem_rx, mem_ry) = {
            ...
            let poly_rx = EqPolynomial::evals(&rx_ext);
            let poly_ry = EqPolynomial::evals(&ry_ext);
            (poly_rx, poly_ry)
        };
