...
impl<const WORD_SIZE: usize> JoltLookupTable for SignedGreaterThanEqualTable<WORD_SIZE> {
    ...
    fn evaluate_mle<F: JoltField>(&self, r: &[F]) -> F {
        F::one() - SignedLessThanTable::<WORD_SIZE>::default().evaluate_mle(r)
    }
}
...
mod test {
    ...
    fn bge_materialize_entry() {
        materialize_entry_test::<Fr, SignedGreaterThanEqualTable<32>>();
    }

    #[test]
    fn bge_mle_full_hypercube() {
        instruction_mle_full_hypercube_test::<Fr, SignedGreaterThanEqualTable<8>>();
    }

    #[test]
    fn bge_mle_random() {
        instruction_mle_random_test::<Fr, SignedGreaterThanEqualTable<32>>();
    }
