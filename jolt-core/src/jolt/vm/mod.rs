...
use crate::jolt::{
    instruction::{
        ...
        mulh::MULHInstruction,
        mulhsu::MULHSUInstruction,
        rem::REMInstruction,
        remu::REMUInstruction,
...
impl<'a, InstructionSet: JoltInstructionSet> TraceOracle<'a, InstructionSet> {
    pub fn new(trace: &'a Vec<JoltTraceStep<InstructionSet>>) -> Self {
        Self {
            // TODO: This is actually padded length.
            length: trace.len(),
            step: 0,
            trace: &trace,
        }
...
impl<'a, InstructionSet: JoltInstructionSet> Oracle for TraceOracle<'a, InstructionSet> {
    ...
    fn peek(&mut self) -> Option<Self::Item> {
        if self.step < self.length { Some(&self.trace[self.step..self.step + 1]) } else { None }
    }
    ...
    fn get_step(&self) -> usize {
        println!("In get step, self.step = {}", self.step);
        self.step
    }
}

pub struct ProverDebugInfo<F, ProofTranscript> where F: JoltField, ProofTranscript: Transcript {
...
>
    ...
    pub instruction_lookups: InstructionLookupsProof<
        C,
        M,
        F,
        PCS,
        ...
        where PCS: CommitmentScheme<ProofTranscript, Field = F>, ProofTranscript: Transcript
    {
        ...
        let polynomial_stream = Box::new(
            ...
            | {
                ...
                let r1cs = R1CSStuff {
                    chunks_x: chunks_x.into_iter().map(MultilinearPolynomial::from).collect(),
                    chunks_y: chunks_y.into_iter().map(MultilinearPolynomial::from).collect(),
...
impl<F: JoltField> JoltPolynomials<F> {
        ...
        where PCS: CommitmentScheme<ProofTranscript, Field = F>, ProofTranscript: Transcript
    {
        ...
        let _guard = span.enter();
        (commitments.read_write_memory.v_final, commitments.read_write_memory.t_final) =
            join_conditional!(
        ...
        ProofTranscript: Transcript
{
        ...
        let bytecode_rows: Vec<BytecodeRow> = bytecode
            ...
            .flat_map(|instruction| {
                match instruction.opcode {
                    ...
                    tracer::RV32IM::MULHSU =>
                        MULHSUInstruction::<32>::virtual_sequence(instruction),
    ...
    fn prove<'a>(
        ...
        for n in 0..no_of_shards {
            ...
            for i in 0..shard_len {
                assert_eq!(
                    ...
                    bytecode_polynomials.a_read_write.get_coeff(n * shard_len + i)
                );
            }
        ...
        for n in 0..no_of_shards {
            ...
            for i in 0..shard_len {
                ...
                for poly_index in 0..instruction_polynomials.instruction_flags.len() {
                    assert_eq!(
                        ...
                        instruction_polynomials.instruction_flags[poly_index].get_coeff(
                            n * shard_len + i
                        )
                    );
                }
            ...
            for i in 0..shard_len {
                assert_eq!(
                    ...
                    instruction_polynomials.lookup_outputs.get_coeff(n * shard_len + i)
                );
            }
            ...
            for shard in 0..shard_len {
                ...
                for i in 0..jolt_polynomials.r1cs.aux.relevant_y_chunks.len() {
                    assert_eq!(
                        ...
                        jolt_polynomials.r1cs.aux.relevant_y_chunks[i].get_coeff(
                            n * shard_len + shard
                        )
                    );
                    assert_eq!(
                        streamed_polys.r1cs.aux.left_lookup_operand.get_coeff(shard),
                        jolt_polynomials.r1cs.aux.left_lookup_operand.get_coeff(
                            n * shard_len + shard
                        )
                    );
                    assert_eq!(
                        streamed_polys.r1cs.aux.right_lookup_operand.get_coeff(shard),
                        jolt_polynomials.r1cs.aux.right_lookup_operand.get_coeff(
                            n * shard_len + shard
                        )
                    );
                    assert_eq!(
                        streamed_polys.r1cs.aux.product.get_coeff(shard),
                        jolt_polynomials.r1cs.aux.product.get_coeff(n * shard_len + shard)
                    );
                    assert_eq!(
                        streamed_polys.r1cs.aux.write_lookup_output_to_rd.get_coeff(shard),
                        jolt_polynomials.r1cs.aux.write_lookup_output_to_rd.get_coeff(
                            n * shard_len + shard
                        )
                    );
                    assert_eq!(
                        streamed_polys.r1cs.aux.write_pc_to_rd.get_coeff(shard),
                        jolt_polynomials.r1cs.aux.write_pc_to_rd.get_coeff(n * shard_len + shard)
                    );
                    assert_eq!(
                        streamed_polys.r1cs.aux.next_pc_jump.get_coeff(shard),
                        jolt_polynomials.r1cs.aux.next_pc_jump.get_coeff(n * shard_len + shard)
                    );
                    assert_eq!(
                        streamed_polys.r1cs.aux.should_branch.get_coeff(shard),
                        jolt_polynomials.r1cs.aux.should_branch.get_coeff(n * shard_len + shard)
                    );
                    assert_eq!(
                        streamed_polys.r1cs.aux.next_pc.get_coeff(shard),
                        jolt_polynomials.r1cs.aux.next_pc.get_coeff(n * shard_len + shard)
                    );
                }
            }
        ...
        let mut opening_accumulator: ProverOpeningAccumulator<
            F,
            ProofTranscript
        ...
        let opening_proof = opening_accumulator.reduce_and_prove::<PCS>(
            &preprocessing.generators,
            &mut transcript
        );
    ...
    fn verify(
        ...
        let mut transcript = ProofTranscript::new(b"Jolt transcript");
        let mut opening_accumulator: VerifierOpeningAccumulator<
            F,
            PCS,
            ProofTranscript
        ...
        if let Some(debug_info) = _debug_info {
            ...
            opening_accumulator.compare_to(
                debug_info.opening_accumulator,
                &preprocessing.generators
            );
        }
        Self::fiat_shamir_preamble(
        ...
        Self::verify_r1cs(r1cs_proof, &commitments, &mut opening_accumulator, &mut transcript)?;
