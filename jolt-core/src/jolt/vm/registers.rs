...
impl<F: JoltField, ProofTranscript: Transcript> ReadWriteCheckingProof<F, ProofTranscript> {
    ...
    pub fn prove_from_array(
        ...
        read_values_1: Vec<u32>,
        read_values_2: Vec<u32>,
            ...
            val.par_chunks_mut(T).enumerate().for_each(|(k, val_k)| {
                ...
                for j in 0..T {
                    ...
                    if write_addresses[j] == k {
                        current_val = F::from_u32(write_values[j]);
                    }
                }
            });
                        ...
                        .for_each(|inc_chunk| {
                            ...
                            for k in dirty_indices.ones() {
                                ...
                                if !rd_wa[0][k].is_zero() || !rd_wa[1][k].is_zero() {
                                    ...
                                    inner_sum_evals[0] +=
                                        wa_eval_0 * (inc_evals[0][k] + val_j_r[0][k]);
                                    inner_sum_evals[1] += wa_eval_2 * (inc_eval_2 + val_eval_2);
                                    inner_sum_evals[2] += wa_eval_3 * (inc_eval_3 + val_eval_3);
        ...
        for round in 0..num_rounds - chunk_size.log_2() {
            ...
            if eq_r_prime.len() > 1 {
                ...
                [&mut rs1_ra, &mut rs2_ra, &mut rd_wa, &mut val, &mut inc]
                    .into_par_iter()
                    .for_each(|poly| poly.bind_parallel(r_j, BindingOrder::HighToLow));
            }
        }
...
mod tests {
    ...
    fn test_read_write_sumcheck() {
        ...
        let mut read_values_1: Vec<u32> = Vec::with_capacity(T);
        let mut read_values_2: Vec<u32> = Vec::with_capacity(T);
        ...
        for _ in 0..T {
            ...
            let read_address_1 = rng.next_u32() as usize % K;
            let read_address_2 = rng.next_u32() as usize % K;
