    ...
    G: CurveGroup<ScalarField = F>,
{
    ...
    fn commit(batched_polys: &Self::BatchedPolynomials) -> Self::Commitment {
        ...
        let generators =
            InstructionCommitmentGenerators {
        ...
        for _round in 0..num_rounds {
            ...
            for mle_leaf_index in evaluate_mles_iterator {
                ...
                for eval_index in 2..num_eval_points {
                    ...
                    for memory_index in 0..multi_memory_evals.len() {
                        let memory_eval = multi_memory_evals[memory_index][mle_leaf_index]
                            [eval_index - 1]
                ...
                for instruction in InstructionSet::iter() {
                    ...
                    for mle_leaf_index in 0..mle_half {
                        ...
                        for memory_index in &memory_indices {
                            terms.push(
                                multi_memory_evals[*memory_index][mle_leaf_index][eval_index],
                            );
                        }

                        let instruction_collation_eval = instruction.combine_lookups(&terms, C, M);
                        let flag_eval =
                            multi_flag_evals[instruction_index][mle_leaf_index][eval_index];

                        // TODO(sragss): May have an excessive group mul here.
                        evaluations[eval_index] += eq_evals[mle_leaf_index][eval_index]
                            * flag_eval
