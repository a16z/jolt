...
use crate::jolt::instruction::{
    add::ADDInstruction, and::ANDInstruction, sll::SLLVirtualSequence, sra::SRAVirtualSequence,
...
impl<const WORD_SIZE: usize> VirtualInstructionSequence for LBInstruction<WORD_SIZE> {
    ...
    fn virtual_trace(trace_row: RVTraceRow) -> Vec<RVTraceRow> {
        ...
        virtual_trace.extend(SLLVirtualSequence::<WORD_SIZE>::virtual_trace(sll));

        let left_aligned_byte = SLLVirtualSequence::<WORD_SIZE>::sequence_output(word, bit_shift);
        let sll = RVTraceRow {
            instruction: ELFInstruction {
                address: trace_row.instruction.address,
                opcode: RV32IM::SLL,
        ...
        let sign_extended_byte =
            SRAVirtualSequence::<WORD_SIZE>::sequence_output(left_aligned_byte, 24);
        ...
        virtual_trace.extend(SRAVirtualSequence::<WORD_SIZE>::virtual_trace(sra));

        for (i, row) in virtual_trace.iter_mut().enumerate() {
            row.instruction.virtual_sequence_remaining = Some(Self::SEQUENCE_LENGTH - i - 1);
        }
