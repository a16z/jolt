...
impl<const WORD_SIZE: usize> VirtualInstructionSequence for LHInstruction<WORD_SIZE> {
    ...
    fn virtual_trace(trace_row: RVTraceRow) -> Vec<RVTraceRow> {
        ...
        let left_aligned_halfword =
            SLLVirtualSequence::<WORD_SIZE>::sequence_output(word, bit_shift);
        let sll = RVTraceRow {
            instruction: ELFInstruction {
                address: trace_row.instruction.address,
                opcode: RV32IM::SLL,
        ...
        let sign_extended_halfword =
            SRAVirtualSequence::<WORD_SIZE>::sequence_output(left_aligned_halfword, 16);
