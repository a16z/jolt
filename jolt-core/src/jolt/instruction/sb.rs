...
use crate::jolt::instruction::{
    add::ADDInstruction, and::ANDInstruction, sll::SLLVirtualSequence, xor::XORInstruction,
    JoltInstruction,
};
...
impl<const WORD_SIZE: usize> VirtualInstructionSequence for SBInstruction<WORD_SIZE> {
    ...
    fn virtual_trace(trace_row: RVTraceRow) -> Vec<RVTraceRow> {
        ...
        let bit_shift = (ram_address << 3) % (1 << WORD_SIZE);
        let sll = RVTraceRow {
            instruction: ELFInstruction {
                address: trace_row.instruction.address,
                opcode: RV32IM::SLLI,
        ...
        let shifted_mask =
            ((byte_mask as u64) << (bit_shift % WORD_SIZE as u64)) % (1 << WORD_SIZE);
        debug_assert!(
        ...
        let shifted_value = (value << (bit_shift % WORD_SIZE as u64)) % (1 << WORD_SIZE);
        let sll = RVTraceRow {
            instruction: ELFInstruction {
                address: trace_row.instruction.address,
                opcode: RV32IM::SLL,
