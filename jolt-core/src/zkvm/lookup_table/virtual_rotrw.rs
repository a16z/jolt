...
impl<const WORD_SIZE: usize> JoltLookupTable for VirtualRotrWTable<WORD_SIZE> {
    fn materialize_entry(&self, index: u128) -> u64 {
        ...
        (0..WORD_SIZE).rev().skip(WORD_SIZE / 2).for_each(|i| {
            let x = x_bits >> i & 1;
            let y = y_bits >> i & 1;
            first_sum *= 1 + y;
            first_sum += x * y;
    ...
    fn evaluate_mle<F: JoltField>(&self, r: &[F]) -> F {
        ...
        for (i, chunk) in r.chunks_exact(2).enumerate().skip(WORD_SIZE / 2) {
            let r_x = chunk[0];
            let r_y = chunk[1];
