...
pub fn lookup_table_mle_random_test<F: JoltField, T: JoltLookupTable + Default>() {
    ...
    for _ in 0..1000 {
        ...
        assert_eq!(
            ...
            T::default().evaluate_mle_field::<F>(&index_to_field_bitvector::<F>(index, 64)),
...
pub fn lookup_table_mle_full_hypercube_test<F: JoltField, T: JoltLookupTable + Default>() {
    ...
    for (i, entry) in materialized.iter().enumerate() {
        assert_eq!(
            F::from_u64(*entry),
            T::default().evaluate_mle_field::<F>(&index_to_field_bitvector::<F>(i as u64, 16)),
...
//                     .map(|prefix| {
//                         prefix.prefix_mle_field::<32, F>(&prefix_checkpoints, r_x, c, prefix_bits, j)
//                     })
//                     .collect();
//
