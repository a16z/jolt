...
impl R1CSCycleInputs {
    /// Build directly from checkpoints and preprocessing,
    /// mirroring the optimized semantics used in `compute_claimed_r1cs_input_evals`.
    pub fn from_checkpoints<F>(
        bytecode_preprocessing: &BytecodePreprocessing,
        checkpoints: &[std::iter::Take<LazyTraceIterator>],
    ...
    where
        F: JoltField,
    {
        // Calculate checkpoint and offset
        let checkpoint_idx = t / checkpoint_interval;
        let offset = t % checkpoint_interval;
        ...
        let cycle = iter.next().expect("cycle should exist");

        // Get next cycle if needed
        let next_cycle = {
