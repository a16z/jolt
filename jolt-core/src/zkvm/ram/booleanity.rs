...
impl<F: JoltField> BooleanitySumcheck<F> {
    ...
    pub fn new_prover<ProofTranscript: Transcript, PCS: CommitmentScheme<Field = F>>(
        ...
        let r_cycle: Vec<MontU128> = state_manager
            .transcript
            .borrow_mut()
            .challenge_vector_u128(T.log_2());

        let r_address: Vec<MontU128> = state_manager
            .transcript
            .borrow_mut()
            .challenge_vector_u128(DTH_ROOT_OF_K.log_2());
...
impl<F: JoltField> SumcheckInstance<F> for BooleanitySumcheck<F> {
    ...
    fn normalize_opening_point(&self, opening_point: &[MontU128]) -> OpeningPoint<BIG_ENDIAN> {
        ...
        let mut r_big_endian: Vec<MontU128> = r_address.iter().rev().copied().collect();
