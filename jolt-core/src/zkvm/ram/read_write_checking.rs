        ...
        let quadratic_coeffs: [F; DEGREE_BOUND - 1] = if gruens_eq_r_prime.E_in_current_len() == 1 {
            ...
            I.par_iter()
                ...
                .map(|((I_chunk, buffers), checkpoint)| {
                    ...
                    let mut evals_for_current_E_out = [F::Unreduced::<9>::zero(), F::Unreduced::<9>::zero()];
                        ...
                        .for_each(|inc_chunk| {
                            ...
                            match x_out_prev {
                                ...
                                Some(x) if x_out != x => {
                                    ...
                                    evals_for_current_E_out = [F::Unreduced::<9>::zero(), F::Unreduced::<9>::zero()];
                                }
                                _ => (),
                            }
                            ...
                            evals_for_current_E_out[0] += E_in_eval.mul_unreduced::<9>(inner_sum_evals[0]);
                            evals_for_current_E_out[1] += E_in_eval.mul_unreduced::<9>(inner_sum_evals[1]);
                        });
