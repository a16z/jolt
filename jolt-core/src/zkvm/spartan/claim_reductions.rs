    ...
    fn gen(
        ...
        Q.par_iter_mut().enumerate().for_each(|(x_lo, q)| {
            ...
            for x_hi in 0..(1 << suffix_n_vars) {
                ...
                // TODO: Improve if necessary. Currently not good memory access pattern.
                let cycle = &trace[x];
                ...
                q_lookup_output += eq_suffix_evals[x_hi].mul_u64_unreduced(lookup_output);
                q_left_lookup_operand += eq_suffix_evals[x_hi].mul_u64_unreduced(left_lookup);
                q_right_lookup_operand += eq_suffix_evals[x_hi].mul_u128_unreduced(right_lookup);
            }
