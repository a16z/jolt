...
fn test_matrix_mle_definition_direct() {
    ...
    for _i in 0..num_constraints {
        ...
        virtual_claims.push(F::from(5u64)); // MulLhs
        virtual_claims.push(F::from(6u64)); // MulRhs
        virtual_claims.push(F::from(7u64)); // MulResult
        virtual_claims.push(F::from(8u64)); // MulQuotient
        virtual_claims.push(F::from(9u64)); // G1ScalarMulXA
        virtual_claims.push(F::from(10u64)); // G1ScalarMulYA
        virtual_claims.push(F::from(11u64)); // G1ScalarMulXT
        virtual_claims.push(F::from(12u64)); // G1ScalarMulYT
        virtual_claims.push(F::from(13u64)); // G1ScalarMulXANext
        virtual_claims.push(F::from(14u64)); // G1ScalarMulYANext
        virtual_claims.push(F::from(15u64)); // G1ScalarMulIndicator
    }
...
fn test_matrix_mle_with_multiple_constraints() {
    ...
    // Calculate num_s_vars: log2(15 * 2) = log2(30) = 5
    let total_rows = num_poly_types * num_constraints_padded;
    ...
    // First constraint gets values 1-15
    for i in 1..=15 {
        virtual_claims.push(F::from(i as u64));
    }

    // Second constraint gets values 101-115
    for i in 101..=115 {
        virtual_claims.push(F::from(i as u64));
    }
