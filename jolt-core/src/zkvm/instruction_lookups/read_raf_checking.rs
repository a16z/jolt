...
impl<F: JoltField> ReadRafSumcheckProver<F> {
    ...
    pub fn gen(
        ...
        let cfg = &config::params().instruction;
        let log_T = trace.len().log_2();
        ...
        let right_operand_ps =
            PrefixSuffixDecomposition::new(Box::new(right_operand_poly), cfg.log_m, LOG_K);
        let left_operand_ps =
            PrefixSuffixDecomposition::new(Box::new(left_operand_poly), cfg.log_m, LOG_K);
        let identity_ps = PrefixSuffixDecomposition::new(Box::new(identity_poly), cfg.log_m, LOG_K);
        drop(_guard);
        drop(span);
        ...
        let mut res = Self {
            ...
            suffix_polys,
            v: (0..cfg.phases)
                .map(|_| ExpandingTable::new(cfg.m, BindingOrder::HighToLow))
                .collect(),
            u_evals_rv,
            u_evals_raf,
            right_operand_ps,
    ...
    fn init_phase(&mut self, phase: usize) {
        ...
        if phase != 0 {
            ...
            self.lookup_indices
                ...
                .for_each(|((k, u), u_raf)| {
                    let (prefix, _) = k.split((cfg.phases - phase) * cfg.log_m);
                    let k_bound: usize = prefix % cfg.m;
                    ...
                    let unreduced_polys = lookup_indices
                        ...
                        .map(|chunk| {
                            let mut chunk_result: Vec<Vec<F::Unreduced<6>>> =
                                vec![unsafe_allocate_zero_vec(cfg.m); suffixes.len()];

                            for j in chunk {
                                let k = self.lookup_indices[*j];
                                let (prefix_bits, suffix_bits) =
                                    k.split((cfg.phases - 1 - phase) * cfg.log_m);
                                for (suffix, result) in suffixes.iter().zip(chunk_result.iter_mut())
                                {
                                    ...
                                    if t != 0 {
                                        ...
                                        result[prefix_bits % cfg.m] += u.mul_u64_unreduced(t);
                                    }
                                }
                            }
        ...
        let ra = {
            ...
            self.lookup_indices
                ...
                .map(|k| {
                    (0..cfg.phases)
                        .map(|phase| {
                            let (prefix, _) = k.split((cfg.phases - 1 - phase) * cfg.log_m);
                            let k_bound: usize = prefix % cfg.m;
...
impl<F: JoltField, T: Transcript> SumcheckInstanceProver<F, T> for ReadRafSumcheckProver<F> {
    ...
    fn ingest_challenge(&mut self, r_j: F::Challenge, round: usize) {
        ...
        if round < LOG_K {
            ...
            if (round + 1).is_multiple_of(cfg.log_m) {
