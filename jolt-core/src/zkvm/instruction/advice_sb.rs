...
use tracer::instruction::{advice_sb::AdviceSB, RISCVCycle};

use crate::zkvm::lookup_table::{range_check::RangeCheckTable, LookupTables};

use super::{CircuitFlags, Flags, InstructionLookup, LookupQuery, NUM_CIRCUIT_FLAGS};

impl<const XLEN: usize> InstructionLookup<XLEN> for AdviceSB {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(RangeCheckTable.into())
    }
}

impl Flags for AdviceSB {
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<AdviceSB> {
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, AdviceSB>();
    }
}
