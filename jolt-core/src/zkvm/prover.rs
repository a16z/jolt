...
use crate::{
    ...
    zkvm::{config, ram::populate_memory_states, verifier::JoltVerifierPreprocessing},
};
use crate::{
    ...
    JoltCpuProver<'a, F, PCS, ProofTranscript>
{
    ...
    fn generate_and_commit_witness_polynomials(
        ...
        let _guard = (
            DoryGlobals::initialize(config::params().one_hot.chunk_size, self.padded_trace_len),
                ...
                let res: Vec<_> = polys
                    ...
                    .map(|poly| {
                        poly.stream_witness_and_commit_rows::<_, PCS>(
                            ...
                            self.preprocessing,
                            &chunk,
                            config::params().one_hot.compute_d(self.ram_K),
                        )
                    })
                    .collect();
    ...
    fn prove_stage7(
        ...
        let streaming_data = Arc::new(RLCStreamingData {
            ...
            ram_d: config::params().one_hot.compute_d(self.ram_K),
        });
    ...
    PCS: CommitmentScheme<Field = F>,
{
    ...
    pub fn gen(
        ...
        let generators = PCS::setup_prover(config::params().one_hot.log_chunk + max_T.log_2());
...
mod tests {
    ...
    use crate::zkvm::{config, RV64IMACProver, RV64IMACVerifier};
    use ark_bn254::Fr;
    ...
    fn small_trace_e2e_dory() {
        ...
        assert!(
            prover.padded_trace_len <= config::params().one_hot.chunk_size,
            "Test requires T <= one_hot.chunk_size ({}), got T = {}",
            config::params().one_hot.chunk_size,
