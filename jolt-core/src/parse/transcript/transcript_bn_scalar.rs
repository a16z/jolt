...
mod tests{
    ...
    fn append_scalars(){
        ...
        let scalars_str = scalars_to_append.iter().map(|x: &Fr| x.to_string()).collect::<Vec<String>>();
    ...
    fn append_bytes(){
        ...
        let bytes_str = bytes_to_append.iter().map(|x: &u8| x.to_string()).collect::<Vec<String>>();
