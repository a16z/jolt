...
use tracer::print_current_memory_usage;
...
impl Program {
    ...
    pub fn trace_analyze<F: JoltField>(mut self, inputs: &[u8]) -> ProgramSummary {
        ...
        print_current_memory_usage(&"Program::trace_analyze entry");
        // let (trace, _, io_device) = self.trace(inputs);
        let file = &PathBuf::from("/tmp/program.jolt");
        let (_, io_device) = self.trace_to_file(inputs, file);
        let open = fs::File::open(file).expect("failed to open trace file");
        let mmap = unsafe { memmap2::Mmap::map(&open).expect("failed to open") };
        // print the mapped file size in megabytes
        println!(
            "[Program::trace_analyze] Memory mapped file size: {} bytes",
            mmap.len() / 1024 * 1024
        );
        let (cycles, _) = postcard::take_from_bytes::<Vec<RV32IMCycle>>(&mmap[..])
            .expect("failed to parse trace file");
        // assert_eq!(cycles.len(), 5_000_000, "trace is too short, did you run the program?");
        ProgramSummary {
            trace: cycles,
            bytecode,
