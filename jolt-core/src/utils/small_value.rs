...
pub mod svo_helpers {
    ...
    pub fn compute_and_update_tA_inplace_2<F: JoltField>(
        ...
        let az_0I = az01 - az00;
        let bz_0I = bz01 - bz00;
        if az_0I != 0 && bz_0I != F::zero() {
            temp_tA[0] += e_in_val.mul_i128(az_0I) * bz_0I;
        }

        // 2. Point (1,I) -> temp_tA[1]
        let az_1I = az11 - az10;
        let bz_1I = bz11 - bz10;
        if az_1I != 0 && bz_1I != F::zero() {
            temp_tA[1] += e_in_val.mul_i128(az_1I) * bz_1I;
        }
        ...
        if az_I0 != 0 {
            let bz_I0 = bz10 - bz00;
            if bz_I0 != F::zero() {
                temp_tA[2] += e_in_val.mul_i128(az_I0) * bz_I0;
            }
        }
        ...
        if az_I1 != 0 {
            let bz_I1 = bz11 - bz01;
            if bz_I1 != F::zero() {
                temp_tA[3] += e_in_val.mul_i128(az_I1) * bz_I1;
            }
        }
        ...
        if az_II != 0 {
            ...
            if bz_II != F::zero() {
                temp_tA[4] += e_in_val.mul_i128(az_II) * bz_II;
            }
        }
    }
    ...
    pub fn compute_and_update_tA_inplace_3<F: JoltField>(
        ...
        if az_I0I != 0 {
            ...
            if bz_I0I != F::zero() {
                temp_tA[12] += e_in_val.mul_i128(az_I0I) * bz_I0I;
            }
        }
        ...
        if az_I10 != 0 && bz_I10 != F::zero() {
            temp_tA[13] += e_in_val.mul_i128(az_I10) * bz_I10;
        }

        // Point (I,1,1) -> temp_tA[14]
        let az_I11 = az111 - az011;
        let bz_I11 = bz111 - bz011;
        if az_I11 != 0 && bz_I11 != F::zero() {
            temp_tA[14] += e_in_val.mul_i128(az_I11) * bz_I11;
        }
        ...
        if az_I1I != 0 {
            ...
            if bz_I1I != F::zero() {
                temp_tA[15] += e_in_val.mul_i128(az_I1I) * bz_I1I;
            }
        }
        ...
        if az_II0 != 0 {
            ...
            if bz_II0 != F::zero() {
                temp_tA[16] += e_in_val.mul_i128(az_II0) * bz_II0;
            }
        }
        ...
        if az_II1 != 0 {
            ...
            if bz_II1 != F::zero() {
                temp_tA[17] += e_in_val.mul_i128(az_II1) * bz_II1;
            }
        }
        ...
        if az_III != 0 {
            ...
            if bz_III != F::zero() {
                temp_tA[18] += e_in_val.mul_i128(az_III) * bz_III;
            }
        }
    }
    ...
    >(
        ...
        for i_temp_tA in 0..M_NON_BINARY_POINTS_CONST {
            ...
            let az_val = get_extended_eval_i128::<NUM_SVO_ROUNDS, NUM_TERNARY_POINTS_CONST>(
                k_target_idx,
            ...
            if az_val != 0 {
                ...
                if bz_val != F::zero() {
                    ...
                    temp_tA[i_temp_tA] += e_in_val.mul_i128(az_val) * bz_val;
                }
            }
        }
    }

    fn get_extended_eval_i128<const N: usize, const NUM_TERN_PTS: usize>(
        k_target: usize,
        ...
        let result = if point_info.is_binary {
            ...
            // These recursive calls must use the same memoization table and info table
            let eval_at_1 = get_extended_eval_i128(
                point_info.k_val_at_one,
                binary_evals_input,
                memoized_evals,
                ternary_point_info_table,
            );
            let eval_at_0 = get_extended_eval_i128(
                point_info.k_val_at_zero,
                binary_evals_input,
                memoized_evals,
                ternary_point_info_table,
