...
#[inline(always)]
fn eval_linear<F: Field>(poly: &(F, F), n: usize) -> Vec<F> {
...
pub fn naive_eval<F: Field>(polys: &[(F, F)]) -> Vec<F> {
...
where
    F: From<u64> + core::ops::Neg<Output = F> + Field,
{
...
where
    F: Field + From<u64> + core::ops::Neg<Output = F>,
{
    let n = polys.len();
    if n == 0 {
        return vec![];
...
pub fn coeff_naive<F: Field>(polys: &[Vec<F>]) -> Vec<F> {
    let n = polys.len();
...
#[inline(always)]
pub fn coeff_kara_2<F: Field>(ps: &[F; 4]) -> [F; 3] {
    kara_2(&ps[0..2].try_into().unwrap(), &ps[2..4].try_into().unwrap())
}

#[inline(always)]
pub fn coeff_kara_3<F: Field>(ps: &[F; 6]) -> [F; 4] {
    // 2+1 split: first 2 polynomials vs last 1 polynomial
    let first_two_result = coeff_kara_2(&ps[0..4].try_into().unwrap()); // 3 mults
...
#[inline(always)]
pub fn coeff_kara_5<F: Field>(ps: &[F; 10]) -> [F; 6] {
...
#[inline(always)]
pub fn coeff_kara_4<F: Field>(ps: &[F; 8]) -> [F; 5] {
    kara_3(
...
#[inline(always)]
pub fn coeff_kara_8<F: Field>(ps: &[F; 16]) -> [F; 9] {
    kara_5(
...
#[inline(always)]
pub fn coeff_kara_16<F: Field>(ps: &[F; 32]) -> [F; 17] {
    kara_9(
...
pub fn coeff_kara_32<F: Field>(ps: &[F; 64]) -> [F; 33] {
    kara_17(
...
#[inline(always)]
fn add_3<F: Field>(a: &[F; 3], b: &[F; 3]) -> [F; 3] {
    [a[0] + b[0], a[1] + b[1], a[2] + b[2]]
}

// take the difference of two degree 2 polynomials
#[inline(always)]
fn sub_3<F: Field>(a: &[F; 3], b: &[F; 3]) -> [F; 3] {
    [a[0] - b[0], a[1] - b[1], a[2] - b[2]]
}

// double a degree 2 polynomial
#[inline(always)]
fn double_3<F: Field>(a: &[F; 3]) -> [F; 3] {
    [F::double(&a[0]), F::double(&a[1]), F::double(&a[2])]
}
...
#[inline(always)]
pub fn toom_3<F: Field>(p: &[F; 3], q: &[F; 3]) -> [F; 5] {
    let e0 = p[0] * q[0];
    let ps = p[0] + p[2];
    let qs = q[0] + q[2];
    let e1 = (ps + p[1]) * (qs + q[1]);
    let en1 = (ps - p[1]) * (qs - q[1]);
    let e2 = (F::double(&(F::double(&p[2]) + p[1])) + p[0])
        * (F::double(&(F::double(&q[2]) + q[1])) + q[0]);
    let einf = p[2] * q[2];
    [e0, e1, en1, e2, einf]
}
...
pub fn kara_toom_16<F: Field>(ps: &[F; 32]) -> [[F; 5]; 5] {
    let p = coeff_kara_8(&ps[0..16].try_into().unwrap());
    let q = coeff_kara_8(&ps[16..32].try_into().unwrap());
    let p0 = [p[0], p[1], p[2]];
    let p1 = [p[3], p[4], p[5]];
    let p2 = [p[6], p[7], p[8]];
    let q0 = [q[0], q[1], q[2]];
...
pub fn toom_3_inter<F: Field>(p: &[F; 5]) -> [F; 5] {
    let c0 = p[0];
    let c1 = -p[0] / F::from(2) + p[1] - p[2] / F::from(3) - p[3] / F::from(6) + p[4] * F::from(2);
    let c2 = -p[0] + p[1] / F::from(2) + p[2] / F::from(2) - p[4];
    let c3 = p[0] / F::from(2) - p[1] / F::from(2) - p[2] / F::from(6) + p[3] / F::from(6)
        - p[4] * F::from(2);
    let c4 = p[4];
    [c0, c1, c2, c3, c4]
}
...
pub fn toom_16_to_coeff<F: Field>(toom: &[[F; 5]; 5]) -> [F; 17] {
...
// (p[0] + p[1]x) * (q[0] + q[1]x)
#[inline(always)]
pub fn kara_2<F: Field>(p: &[F; 2], q: &[F; 2]) -> [F; 3] {
    let l = p[0] * q[0];
    let u = p[1] * q[1];
...
#[inline(always)]
pub fn kara_2_w_top<F: Field>(p: &[F; 2], q: &[F; 2], u: F) -> [F; 2] {
    let l = p[0] * q[0];
    let m = (p[0] + p[1]) * (q[0] + q[1]) - u - l;
    [l, m]
}
...
#[inline(always)]
fn kara_2x1<F: Field>(p: &[F; 3], q: &[F; 2]) -> [F; 4] {
...
#[inline(always)]
fn kara_3x2<F: Field>(p: &[F; 4], q: &[F; 3]) -> [F; 6] {
...
// etc.
#[inline(always)]
pub fn kara_3<F: Field>(p: &[F; 3], q: &[F; 3]) -> [F; 5] {
    let l = [p[0] * q[0]];
...
#[inline(always)]
pub fn kara_3_w_top<F: Field>(p: &[F; 3], q: &[F; 3], uu: F) -> [F; 4] {
    let l = [p[0] * q[0]];
...
#[inline(always)]
pub fn kara_4<F: Field>(p: &[F; 4], q: &[F; 4]) -> [F; 7] {
...
#[inline(always)]
pub fn kara_5<F: Field>(p: &[F; 5], q: &[F; 5]) -> [F; 9] {
    let l = kara_2(p[0..2].try_into().unwrap(), q[0..2].try_into().unwrap());
...
#[inline(always)]
pub fn kara_5_w_top<F: Field>(p: &[F; 5], q: &[F; 5], uu: F) -> [F; 8] {
    let l = kara_2(p[0..2].try_into().unwrap(), q[0..2].try_into().unwrap());
...
#[inline(always)]
pub fn kara_8<F: Field>(p: &[F; 8], q: &[F; 8]) -> [F; 15] {
...
#[inline(always)]
pub fn kara_9<F: Field>(p: &[F; 9], q: &[F; 9]) -> [F; 17] {
    let l = kara_4(p[0..4].try_into().unwrap(), q[0..4].try_into().unwrap());
...
#[inline(always)]
pub fn kara_9_w_top<F: Field>(p: &[F; 9], q: &[F; 9], uu: F) -> [F; 16] {
    let l = kara_4(p[0..4].try_into().unwrap(), q[0..4].try_into().unwrap());
...
#[inline(always)]
pub fn kara_17<F: Field>(p: &[F; 17], q: &[F; 17]) -> [F; 33] {
...
mod tests {
    ...
    use crate::subprotocols::quang_optimization::{
        coeff_kara_3, coeff_kara_32, coeff_kara_5, coeff_naive,
    };
    ...
    fn test_coeff_kara_3() {
        ...
        let naive_res = coeff_naive(&polys);
        let kara_res = coeff_kara_3(&flat);
    ...
    #[test]
    fn test_coeff_kara_5() {
        let mut rng = ark_std::rand::rngs::StdRng::seed_from_u64(123);
        let polys: Vec<Vec<Fr>> = (0..5)
            .map(|_| vec![Fr::rand(&mut rng), Fr::rand(&mut rng)])
            .collect();
        let flat: [Fr; 10] = polys
            .iter()
            .flat_map(|p| [p[0], p[1]])
            .collect::<Vec<_>>()
            .try_into()
        ...
        let naive_res = coeff_naive(&polys);
        let kara_res = coeff_kara_5(&flat);
