...
pub trait FieldMulSmall: JoltField + Sized + Copy {
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod2<F: FieldMulSmall>((p0, p1): (F, F), (q0, q1): (F, F)) -> (F, F, F) {
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod2_2<F: FieldMulSmall>(p: [F; 4], q: [F; 4]) -> [F; 9] {
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod3_2<F: FieldMulSmall>(polys: [[F; 4]; 3]) -> [F; 16] {
    // Step 1: quadratic grid from first two polys
    let a = prod2_2(polys[0], polys[1]);
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod2_n<F: FieldMulSmall>(p_vals: &[F], q_vals: &[F]) -> Vec<F> {
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod3<F: FieldMulSmall>(p: [(F, F); 3]) -> (F, F, F, F) {
    // ---- step 1: combine first two linear terms into quadratic ----
    let (a0, a1, a_inf) = prod2(p[0], p[1]); // quadratic A on {0,1,∞}
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod4<F: FieldMulSmall>(p: [(F, F); 4]) -> (F, F, F, F, F) {
    // ---- first level:  two independent quadratic products ----
    let (a0, a1, a_inf) = prod2(p[0], p[1]); // A on {0,1,∞}
    let (b0, b1, b_inf) = prod2(p[2], p[3]); // B on {0,1,∞}
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod4_2<F: FieldMulSmall>(polys: [[F; 4]; 4]) -> [F; 25] {
    ...
    let a = prod2_2(polys[0], polys[1]); // A(x,y) values (9)
    let b = prod2_2(polys[2], polys[3]); // B(x,y) values (9)
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod5<F: FieldMulSmall>(p: [(F, F); 5]) -> (F, F, F, F, F, F) {
    // ---- step 1: compute sub-products ----
    let (a0, a1, a_inf) = prod2(p[0], p[1]); // quadratic A on {0,1,∞}
    let (b0, b1, b_m1, b_inf) = prod3([p[2], p[3], p[4]]); // cubic B on {0,1,-1,∞}
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod8<F: FieldMulSmall>(p: [(F, F); 8]) -> [F; 9] {
    // ---- first level: two quartic blocks ----------------------------------
    let (c0, c1, c_m1, c2, c_inf) = prod4(p[0..4].try_into().unwrap());
    let (d0, d1, d_m1, d2, d_inf) = prod4(p[4..8].try_into().unwrap());
    ...
    fn quartic_eval_batch<F: FieldMulSmall>(
        ...
        let f_inf120 = f_inf.mul_u64(120);

        // Compute all 4 points at once (start with positive terms to save negations)
        let x_m2 = f_inf24 + (f1_4 + fm1_4) - f2 - f0.mul_u64(6);
        let x_3 = f_inf24 + f0_4 + f2_4 - f_m1 - f1.mul_u64(6);
        let x_m3 = f_inf120 + f1.mul_u64(15) + f_m1.mul_u64(10) - f0.mul_u64(20) - f2.mul_u64(4);
        let x_4 = f_inf120 + f0.mul_u64(15) + f2.mul_u64(10) - f1.mul_u64(20) - f_m1.mul_u64(4);
...
// -----------------------------------------------------------------------------
#[inline]
pub fn prod16<F: FieldMulSmall>(p: [(F, F); 16]) -> [F; 17] {
    // ---- first level: two octic blocks ------------------------------------
    let c = prod8(p[0..8].try_into().unwrap()); // C(x) on 9 points
    let d = prod8(p[8..16].try_into().unwrap()); // D(x) on 9 points
