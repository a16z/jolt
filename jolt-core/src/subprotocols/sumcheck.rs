...
impl<F: JoltField, ProofTranscript: Transcript> SumcheckInstanceProof<F, ProofTranscript> {
    ...
    pub fn update_evals(evals: &mut Vec<F>, r: F, size: usize) {
        for i in 0..size {
            let temp = evals[i];
            evals[size + i] = temp * r;
            ...
            Func2: Fn(&[F]) -> F + std::marker::Sync
    {
        ...
        for round in 0..num_rounds / 2 {
                ...
                for idx_in_shard in 0..shard_length {
                    ...
                    for k in 0..num_polys {
                        for s in 0..=degree {
                            // if round == 1 {
                            //     println!(
                            //         "idx_in_poly % (1 << round) = {}, evals_1[idx_in_poly % (1 << round)] = {}",
                            //         idx_in_poly % (1 << round),
            ...
            Self::update_evals(&mut evals_1, r_i, 1 << round);
        ...
        for round in num_rounds / 2..num_rounds {
                ...
                for idx_in_shard in 0..shard_length {
                    ...
                    for k in 0..num_polys {
                        for s in 0..=degree {
                            int_witness_eval[k][s] +=
                                evals_1[idx_in_poly % (1 << (num_rounds / 2))] *
                                eq_eval_idx_s_vec[s] *
                                polys[k].get_coeff(idx_in_shard);
                        }
                    }
                    ...
                    if (idx_in_poly + 1) % (1 << (num_rounds / 2)) == 0 {
                        ...
                        for k in 0..num_polys {
                            for s in 0..=degree {
                                witness_eval[k][s] +=
                                    evals_2[temp % (1 << (round - num_rounds / 2))] *
                                    int_witness_eval[k][s];
                            }
                        }
            ...
            Self::update_evals(&mut evals_2, r_i, 1 << (round - num_rounds / 2));
