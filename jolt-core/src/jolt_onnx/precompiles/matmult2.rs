...
where
    F: JoltField,
{
    fn new<ProofTranscript>(a: &Matrix<F>, b: &Matrix<F>, transcript: &mut ProofTranscript) -> Self
    where
        ProofTranscript: Transcript,
    {
        ...
        for i in 0..m {
            for j in 0..k {
                A_rx[j] += a.entries[i * k + j] * eq_rx[i];
            }
        }
    ...
    F: JoltField,
{
    ...
    /// # Panics
    ///
    /// - panics if lhs's number of columns (lhs n) does not equal rhs's number of
    ///   rows (rhs m)
    pub fn matmult(a: &Self, b: &Self) -> Self {
        assert_eq!(a.n, b.m);
        let m = a.m;
        let n = b.n;
    ...
    pub fn random(mut rng: impl RngCore, m: usize, n: usize) -> Self {
        let entries = (0..m * n).map(|_| F::random(&mut rng)).collect_vec();
        Self { entries, m, n }
    }
    ...
    pub fn pad(&self) -> Self {
        ...
        entries.resize(padded_m * padded_n, F::zero());
        Self {
            entries,
            m: padded_m,
            n: padded_n,
