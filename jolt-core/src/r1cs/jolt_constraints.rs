...
pub enum JoltIn {
    ...
    IF_Virt_Adv,
    IF_Virt_Assert_LTE,
    IF_Virt_Assert_VALID_REMAINDER,
}
...
impl<F: JoltField> R1CSConstraintBuilder<F> for UniformJoltConstraints {
    ...
    fn build_constraints(&self, cs: &mut R1CSBuilder<F, Self::Inputs>) {
        let flags = input_range!(
            JoltIn::OpFlags_IsRs1Rs2,
            JoltIn::IF_Virt_Assert_VALID_REMAINDER
        );
        for flag in flags {
...
mod tests {
    ...
    fn instruction_flags_length() {
        assert_eq!(
            input_range!(JoltIn::IF_Add, JoltIn::IF_Virt_Assert_VALID_REMAINDER).len(),
            RV32I::COUNT
        );
    }
