...
use crate::{
    jolt::vm::{
        bytecode::BytecodeStuff, instruction_lookups::InstructionLookupStuff,
        read_write_memory::ReadWriteMemoryStuff, timestamp_range_check::TimestampRangeCheckStuff,
        JoltStuff,
    },
    poly::commitment::hyperkzg::HyperKZGCommitment,
    r1cs::inputs::{AuxVariableStuff, R1CSStuff},
};
...
pub fn convert_from_byte_code_stuff_to_circom(
    byte_code_stuff: &BytecodeStuff<HyperKZGCommitment<Bn254>>,
) -> ByteCodeStuffCircom {
    let mut v_read_write = Vec::new();
    for i in 0..byte_code_stuff.v_read_write.len() {
        v_read_write.push(convert_hyperkzg_commitment_to_circom(
            &byte_code_stuff.v_read_write[i].clone(),
        ))
    }
    ByteCodeStuffCircom {
...
pub fn convert_from_read_write_mem_stuff_to_circom(
    rw_stuff: &ReadWriteMemoryStuff<HyperKZGCommitment<Bn254>>,
...
pub fn convert_from_ins_lookup_stuff_to_circom(
    ins_lookup_stuff: &InstructionLookupStuff<HyperKZGCommitment<Bn254>>,
    ...
    for i in 0..ins_lookup_stuff.dim.len() {
        dim.push(convert_hyperkzg_commitment_to_circom(
            &ins_lookup_stuff.dim[i].clone(),
        ));
    }
    for i in 0..ins_lookup_stuff.read_cts.len() {
        read_cts.push(convert_hyperkzg_commitment_to_circom(
            &ins_lookup_stuff.read_cts[i].clone(),
        ));
    }
    for i in 0..ins_lookup_stuff.final_cts.len() {
        final_cts.push(convert_hyperkzg_commitment_to_circom(
            &ins_lookup_stuff.final_cts[i].clone(),
        ))
    }
    for i in 0..ins_lookup_stuff.E_polys.len() {
        E_polys.push(convert_hyperkzg_commitment_to_circom(
            &ins_lookup_stuff.E_polys[i].clone(),
        ));
    }
    for i in 0..ins_lookup_stuff.instruction_flags.len() {
        instruction_flags.push(convert_hyperkzg_commitment_to_circom(
            &ins_lookup_stuff.instruction_flags[i].clone(),
        ));
    }
...
pub fn convert_from_ts_lookup_stuff_to_circom(
    ts_lookup_stuff: &TimestampRangeCheckStuff<HyperKZGCommitment<Bn254>>,
    ...
    for i in 0..ts_lookup_stuff.read_cts_read_timestamp.len() {
        read_cts_read_timestamp.push(convert_hyperkzg_commitment_to_circom(
            &ts_lookup_stuff.read_cts_read_timestamp[i].clone(),
        ))
    }
    for i in 0..ts_lookup_stuff.read_cts_global_minus_read.len() {
        read_cts_global_minus_read.push(convert_hyperkzg_commitment_to_circom(
            &ts_lookup_stuff.read_cts_global_minus_read[i].clone(),
        ));
    }
    for i in 0..ts_lookup_stuff.final_cts_read_timestamp.len() {
        final_cts_read_timestamp.push(convert_hyperkzg_commitment_to_circom(
            &ts_lookup_stuff.final_cts_read_timestamp[i].clone(),
        ));
    }
    for i in 0..ts_lookup_stuff.final_cts_global_minus_read.len() {
        final_cts_global_minus_read.push(convert_hyperkzg_commitment_to_circom(
            &ts_lookup_stuff.final_cts_global_minus_read[i].clone(),
        ));
    }
...
pub fn convert_from_aux_stuff_to_circom(
    aux_stuff: &AuxVariableStuff<HyperKZGCommitment<Bn254>>,
) -> AuxVariableStuffCircom {
    let mut relevant_y_chunks = Vec::new();
    for i in 0..aux_stuff.relevant_y_chunks.len() {
        relevant_y_chunks.push(convert_hyperkzg_commitment_to_circom(
            &aux_stuff.relevant_y_chunks[i].clone(),
        ))
    }
    AuxVariableStuffCircom {
        left_lookup_operand: convert_hyperkzg_commitment_to_circom(&aux_stuff.left_lookup_operand),
        right_lookup_operand: convert_hyperkzg_commitment_to_circom(
            &aux_stuff.right_lookup_operand,
        ),
        product: convert_hyperkzg_commitment_to_circom(&aux_stuff.product),
        relevant_y_chunks: relevant_y_chunks,
        write_lookup_output_to_rd: convert_hyperkzg_commitment_to_circom(
            &aux_stuff.write_lookup_output_to_rd,
        ),
...
pub fn convert_from_r1cs_stuff_to_circom(
    r1cs_stuff: &R1CSStuff<HyperKZGCommitment<Bn254>>,
) -> R1CSStuffCircom {
    let mut chunks_x = Vec::new();
    let mut chunks_y = Vec::new();
    let mut circuit_flags = Vec::new();
    for i in 0..r1cs_stuff.chunks_x.len() {
        chunks_x.push(convert_hyperkzg_commitment_to_circom(
            &&r1cs_stuff.chunks_x[i].clone(),
        ));
    }
    for i in 0..r1cs_stuff.chunks_y.len() {
        chunks_y.push(convert_hyperkzg_commitment_to_circom(
            &r1cs_stuff.chunks_y[i].clone(),
        ));
    }
    for i in 0..r1cs_stuff.circuit_flags.len() {
        circuit_flags.push(convert_hyperkzg_commitment_to_circom(
            &r1cs_stuff.circuit_flags[i].clone(),
        ));
    }
    R1CSStuffCircom {
        chunks_x,
...
pub fn convert_from_jolt_stuff_to_circom(
    jolt_stuff: &JoltStuff<HyperKZGCommitment<Bn254>>,
) -> JoltStuffCircom {
    JoltStuffCircom {
        bytecode: convert_from_byte_code_stuff_to_circom(&jolt_stuff.bytecode),
        read_write_memory: convert_from_read_write_mem_stuff_to_circom(
            &jolt_stuff.read_write_memory,
        ),
        instruction_lookups: convert_from_ins_lookup_stuff_to_circom(
            &jolt_stuff.instruction_lookups,
        ),
        timestamp_range_check: convert_from_ts_lookup_stuff_to_circom(
            &jolt_stuff.timestamp_range_check,
        ),
