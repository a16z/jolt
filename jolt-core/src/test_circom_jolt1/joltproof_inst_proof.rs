...
pub fn convert_from_primary_sum_check_opening_to_circom(
    prim_s_c_openings: &PrimarySumcheckOpenings<Scalar>,
) -> PrimarySumcheckOpeningsCircom {
    let mut E_poly_openings = Vec::new();
    for i in 0..prim_s_c_openings.E_poly_openings.len() {
        E_poly_openings.push(FqCircom(prim_s_c_openings.E_poly_openings[i]))
    }
    let mut flag_openings = Vec::new();
    for i in 0..prim_s_c_openings.flag_openings.len() {
        flag_openings.push(FqCircom(prim_s_c_openings.flag_openings[i]));
    }

    PrimarySumcheckOpeningsCircom {
        E_poly_openings,
        flag_openings,
        lookup_outputs_opening: FqCircom(prim_s_c_openings.lookup_outputs_opening),
    }
}
...
pub fn convert_from_inst_lookups_proof_to_circom(
    inst_lookup_proof: InstructionLookupsProof<
        { C },
        { M },
        Scalar,
    ...
    let primary_sum_check = PrimarySumcheckCircom {
        sumcheck_proof: convert_sum_check_proof_to_circom(
            &inst_lookup_proof.primary_sumcheck.sumcheck_proof,
        ),
        openings: convert_from_primary_sum_check_opening_to_circom(
            &inst_lookup_proof.primary_sumcheck.openings,
        ),
    };
    ...
    for i in 0..lookup_openings.dim.len() {
        openings.push(FqCircom(lookup_openings.dim[i]));
    }
    for i in 0..lookup_openings.read_cts.len() {
        openings.push(FqCircom(lookup_openings.read_cts[i]))
    }
    for i in 0..lookup_openings.final_cts.len() {
        openings.push(FqCircom(lookup_openings.final_cts[i]))
    }
    for i in 0..lookup_openings.E_polys.len() {
        openings.push(FqCircom(lookup_openings.E_polys[i]))
    }
    for i in 0..lookup_openings.instruction_flags.len() {
        openings.push(FqCircom(lookup_openings.instruction_flags[i]))
    }
    openings.push(FqCircom(lookup_openings.lookup_outputs));

    let mem_checking_proof = InstMemoryCheckingProofCircom {
        multiset_hashes: convert_multiset_hashes_to_circom(
            &inst_lookup_proof.memory_checking.multiset_hashes,
        ),
        read_write_grand_product: convert_from_batched_GKRProof_to_circom(
            &inst_lookup_proof.memory_checking.read_write_grand_product,
        ),
        init_final_grand_product: convert_from_batched_GKRProof_to_circom(
            &inst_lookup_proof.memory_checking.init_final_grand_product,
        ),
        openings: openings,
    };
