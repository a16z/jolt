...
pub fn compute_uniform_spartan_to_circom(
    uni_spartan_proof: UniformSpartanProof<
        C,
        JoltR1CSInputs,
        Scalar,
    ...
    let mut outer_s_c_claims: [FqCircom; 3] = [FqCircom(Scalar::ZERO); 3];

    outer_s_c_claims[0] = FqCircom(uni_spartan_proof.outer_sumcheck_claims.0);
    ...
    for i in 0..uni_spartan_proof.claimed_witness_evals.len() {
        claimed_witness_evals.push(FqCircom(uni_spartan_proof.claimed_witness_evals[i]));
    }

    UniformSpartanProofCircom {
        outer_sumcheck_proof: convert_sum_check_proof_to_circom(
            &uni_spartan_proof.outer_sumcheck_proof,
        ),
        outer_sumcheck_claims: outer_s_c_claims,
        inner_sumcheck_proof: convert_sum_check_proof_to_circom(
            &uni_spartan_proof.inner_sumcheck_proof,
        ),
