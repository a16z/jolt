...
pub fn setup_F2<F: JoltField, ProofTranscript: Transcript>(
    lookup_table: &[F],
    read_addresses: &[usize],
    transcript: &mut ProofTranscript,
) -> Vec<F> {
...
#[cfg(feature = "icicle")]
fn icicle_sumcheck<F>(mut prover_transcript: &mut KeccakTranscript, K: usize, mle_polys: &[Vec<F>], sumcheck_claim: F)
where
    F: JoltField
{
    ...
    c.bench_function(&id, |b| {
        b.iter(|| {
            match benchmark_mode {
                ...
                BenchmarkMode::F2 => { setup_F2(&lookup_table, &read_addresses, &mut prover_transcript); },
...
fn main() {
    ...
    benchmark_sumcheck::<Fr, KeccakTranscript>(
        &mut criterion,
        "Sumcheck::shout_setup_F_map_reduce",
        BenchmarkMode::F2
    );
