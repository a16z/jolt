(* Exact Linux AArch64 correctness proof for scalar Fp64 subtraction. *)

needs (Filename.concat (Sys.getenv "JOLT_FP64_PROOF_DIR")
        "fp64_sub_aarch64_linux_object.ml");;

let JOLT_FP64_SUB_AARCH64_LINUX_CORRECT = time prove
 (`!a b pc.
        ensures arm
          (\s. aligned_bytes_loaded s (word pc)
                 jolt_fp64_sub_aarch64_linux_mc /\
  ...
  ARM_ACCSTEPS_TAC JOLT_FP64_SUB_AARCH64_LINUX_EXEC [2] (1--4) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN STRIP_TAC THEN
  SUBGOAL_THEN
   `2 EXP 64 * bitval carry_s2 + val(a:int64) =
    val(b:int64) + val(sum_s2:int64)`
  ASSUME_TAC THENL
   [REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    ACCUMULATOR_ASSUM_LIST(MP_TAC o end_itlist CONJ o DECARRY_RULE) THEN
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN REAL_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN `val(sum_s2:int64) < 2 EXP 64` ASSUME_TAC THENL
...
let JOLT_FP64_SUB_AARCH64_LINUX_SUBROUTINE_CORRECT = time prove
 (`!a b pc returnaddress.
  ...
  ARM_ADD_RETURN_NOSTACK_TAC JOLT_FP64_SUB_AARCH64_LINUX_EXEC
    JOLT_FP64_SUB_AARCH64_LINUX_CORRECT);;
