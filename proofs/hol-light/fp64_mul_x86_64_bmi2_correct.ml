(* Exact BMI2 x86-64 correctness proof for scalar Fp64 multiplication. *)

needs (Filename.concat (Sys.getenv "JOLT_FP64_PROOF_DIR")
        "fp64_mul_x86_64_bmi2_object.ml");;

let JOLT_FP64_MUL_X86_64_BMI2_CORRECT = time prove
 (`!a b pc.
        ensures x86
          (\s. bytes_loaded s (word pc)
                 (BUTLAST jolt_fp64_mul_x86_64_bmi2_mc) /\
          ...
          (MAYCHANGE [RIP; RAX; RCX; RDX; RDI; RSI] ,,
           MAYCHANGE SOME_FLAGS ,, MAYCHANGE [events])`,
  MAP_EVERY X_GEN_TAC [`a:int64`; `b:int64`; `pc:num`] THEN
  REWRITE_TAC[SOME_FLAGS] THEN ENSURES_INIT_TAC "s0" THEN
  X86_ACCSTEPS_TAC JOLT_FP64_MUL_X86_64_BMI2_EXEC
   [2;4;5;6;7;9;10;11;12;14;15] (1--16) THEN
  ...
  SUBGOAL_THEN
   `2 EXP 64 * val(sum_s12:int64) + val(sum_s11:int64) =
    val(sum_s6:int64) + 59 * val(sum_s7:int64)`
  (LABEL_TAC "fold2") THENL
   [ASM_CASES_TAC `carry_s10:bool` THEN
    ...
    MAP_EVERY (fun t -> MP_TAC(SPEC t VAL_BOUND_64))
     [`sum_s7:int64`; `mulhi_s9:int64`; `sum_s10:int64`;
      `sum_s11:int64`; `sum_s12:int64`] THEN
    MAP_EVERY MP_TAC
     [SPEC `val(sum_s7:int64)` REAL_POS;
  ...
  SUBGOAL_THEN `val(sum_s7:int64) <= 59` (LABEL_TAC "fold1_bound") THENL
   [USE_THEN "fold1" MP_TAC THEN
    MAP_EVERY (fun t -> MP_TAC(SPEC t VAL_BOUND_64))
     [`sum_s6:int64`; `mullo_s2:int64`; `mulhi_s2:int64`] THEN
    CONV_TAC NUM_REDUCE_CONV THEN ARITH_TAC;
    ALL_TAC] THEN
  ABBREV_TAC `t = 2 EXP 64 * val(sum_s12:int64) + val(sum_s11:int64)` THEN
  SUBGOAL_THEN `t < 2 * jolt_fp64_p` (LABEL_TAC "reduced_bound") THENL
   [USE_THEN "fold2" MP_TAC THEN USE_THEN "fold1_bound" MP_TAC THEN
    MP_TAC(SPEC `sum_s6:int64` VAL_BOUND_64) THEN
   ...
   [EXPAND_TAC "t" THEN
    MATCH_MP_TAC(SPECL
     [`val(a:int64) * val(b:int64)`;
      ...
      `val(sum_s7:int64)`; `val(sum_s6:int64)`;
  ...
  (LABEL_TAC "tail14") THENL
   [REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
   `2 EXP 64 * bitval carry_s15 + val(sum_s12:int64) =
    bitval carry_s14 + val(sum_s15:int64)`
  (LABEL_TAC "tail15") THENL
   [REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
   `val(sum_s11:int64) < 2 EXP 64 /\ val(sum_s12:int64) < 2 EXP 64 /\
  ...
  MATCH_MP_TAC(SPECL
   [`val(sum_s12:int64)`; `val(sum_s11:int64)`;
    `val(sum_s14:int64)`; `val(sum_s15:int64)`;
    `~carry_s14`; `carry_s15:bool`] JOLT_FP64_FINAL_REDUCTION) THEN
  REPEAT CONJ_TAC THENL
   [USE_THEN "reduced_bound" MP_TAC THEN EXPAND_TAC "t" THEN ARITH_TAC;
    USE_THEN "tail14" ACCEPT_TAC;
    USE_THEN "tail15" MP_TAC THEN ARITH_TAC;
    ASM_REWRITE_TAC[];
    ASM_REWRITE_TAC[];
    ASM_REWRITE_TAC[];
    ASM_REWRITE_TAC[]]);;

let JOLT_FP64_MUL_X86_64_BMI2_SUBROUTINE_CORRECT = time prove
 (`!a b pc stackpointer returnaddress.
        ensures x86
          (\s. bytes_loaded s (word pc) jolt_fp64_mul_x86_64_bmi2_mc /\
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC
    jolt_fp64_mul_x86_64_bmi2_mc JOLT_FP64_MUL_X86_64_BMI2_CORRECT);;
