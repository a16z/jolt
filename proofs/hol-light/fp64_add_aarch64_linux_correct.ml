(* Exact Linux AArch64 correctness proof for scalar Fp64 addition. *)

needs (Filename.concat (Sys.getenv "JOLT_FP64_PROOF_DIR")
        "fp64_add_aarch64_linux_object.ml");;

let JOLT_FP64_ADD_AARCH64_LINUX_CORRECT = time prove
 (`!a b pc.
        ensures arm
          (\s. aligned_bytes_loaded s (word pc)
                 jolt_fp64_add_aarch64_linux_mc /\
  ...
  ARM_STEPS_TAC JOLT_FP64_ADD_AARCH64_LINUX_EXEC (1--1) THEN
  ARM_ACCSTEPS_TAC JOLT_FP64_ADD_AARCH64_LINUX_EXEC [2] (2--2) THEN
  SUBGOAL_THEN
   `2 EXP 64 * bitval carry_s2 + val(sum_s2:int64) =
    val(a:int64) + val(b:int64)`
  (LABEL_TAC "sum2") THENL
   [REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    ACCUMULATOR_ASSUM_LIST(MP_TAC o end_itlist CONJ o DECARRY_RULE) THEN
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN REAL_ARITH_TAC;
    ALL_TAC] THEN
  ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC) THEN
  ARM_STEPS_TAC JOLT_FP64_ADD_AARCH64_LINUX_EXEC (3--3) THEN
  ARM_ACCSTEPS_TAC JOLT_FP64_ADD_AARCH64_LINUX_EXEC [4] (4--4) THEN
  SUBGOAL_THEN
   `2 EXP 64 * bitval carry_s4 + val(sum_s4:int64) =
    val(sum_s2:int64) + 59 * bitval carry_s2`
  (LABEL_TAC "sum4") THENL
   [REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    ...
    ALL_TAC] THEN
  ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC) THEN
  ARM_ACCSTEPS_TAC JOLT_FP64_ADD_AARCH64_LINUX_EXEC [5] (5--5) THEN
  SUBGOAL_THEN
  ...
  (LABEL_TAC "sum5") THENL
   [REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    ...
    ALL_TAC] THEN
  ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC) THEN
  ARM_STEPS_TAC JOLT_FP64_ADD_AARCH64_LINUX_EXEC (6--7) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN STRIP_TAC THEN
  SUBGOAL_THEN
   `val(sum_s2:int64) < 2 EXP 64 /\
...
let JOLT_FP64_ADD_AARCH64_LINUX_SUBROUTINE_CORRECT = time prove
 (`!a b pc returnaddress.
  ...
  ARM_ADD_RETURN_NOSTACK_TAC JOLT_FP64_ADD_AARCH64_LINUX_EXEC
    JOLT_FP64_ADD_AARCH64_LINUX_CORRECT);;
