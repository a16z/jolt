(* Exact Linux AArch64 correctness proof for scalar Fp64 multiplication. *)

needs (Filename.concat (Sys.getenv "JOLT_FP64_PROOF_DIR")
        "fp64_mul_aarch64_linux_object.ml");;

let JOLT_FP64_MUL_AARCH64_LINUX_CORRECT = time prove
 (`!a b pc.
        ensures arm
          (\s. aligned_bytes_loaded s (word pc)
                 jolt_fp64_mul_aarch64_linux_mc /\
  ...
  ARM_ACCSTEPS_TAC JOLT_FP64_MUL_AARCH64_LINUX_EXEC (1--16) (1--17) THEN
  ...
  (LABEL_TAC "fold1") THENL
   [ASM_CASES_TAC `carry_s6:bool` THEN ASM_CASES_TAC `carry_s7:bool` THEN
    ASM_CASES_TAC `carry_s8:bool` THEN
    UNDISCH_TAC `val(mulhi_s4:int64) < 59` THEN
    MAP_EVERY (fun t -> MP_TAC(SPEC t VAL_BOUND_64))
     [`mulhi_s1:int64`; `mulhi_s4:int64`; `sum_s6:int64`; `sum_s8:int64`] THEN
    MAP_EVERY MP_TAC
  ...
  (LABEL_TAC "fold2") THENL
   [ASM_CASES_TAC `carry_s11:bool` THEN ASM_CASES_TAC `carry_s12:bool` THEN
    ASM_CASES_TAC `carry_s13:bool` THEN
    UNDISCH_TAC `val(mulhi_s9:int64) < 59` THEN
    MAP_EVERY (fun t -> MP_TAC(SPEC t VAL_BOUND_64))
     [`sum_s8:int64`; `mulhi_s9:int64`; `sum_s11:int64`; `sum_s13:int64`] THEN
    MAP_EVERY MP_TAC
  ...
  SUBGOAL_THEN
   `2 EXP 64 * bitval carry_s16 + val(sum_s13:int64) =
    val(sum_s16:int64) + bitval(~carry_s14)`
  (LABEL_TAC "tail16") THENL
   [ASM_CASES_TAC `carry_s14:bool` THEN ASM_CASES_TAC `carry_s16:bool` THEN
    REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    ACCUMULATOR_ASSUM_LIST(MP_TAC o end_itlist CONJ o DESUM_RULE) THEN
    ASM_REWRITE_TAC[BITVAL_CLAUSES] THEN
    CONV_TAC REAL_RAT_REDUCE_CONV THEN REAL_ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
   `2 EXP 64 * bitval carry_s14 + val(sum_s15:int64) =
    val(sum_s12:int64) + 59`
  ...
  MATCH_MP_TAC(SPECL
   [`val(sum_s13:int64)`; `val(sum_s12:int64)`;
    `val(sum_s15:int64)`; `val(sum_s16:int64)`;
    `carry_s14:bool`; `carry_s16:bool`]
...
let JOLT_FP64_MUL_AARCH64_LINUX_SUBROUTINE_CORRECT = time prove
 (`!a b pc returnaddress.
  ...
  ARM_ADD_RETURN_NOSTACK_TAC JOLT_FP64_MUL_AARCH64_LINUX_EXEC
    JOLT_FP64_MUL_AARCH64_LINUX_CORRECT);;
