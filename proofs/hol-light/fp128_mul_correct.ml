(* Functional correctness proof for the fixed A7F7 AArch64 multiply body. *)
...
let JOLT_FP128_MUL_CORRECT = time prove
 (`!a0 a1 b0 b1 pc.
          ...
          (\s. read PC s = word (pc + 0x8c) /\
               (bignum_of_wordlist [a0; a1] < jolt_fp128_a7f7_p /\
                ...
                ==> bignum_of_wordlist [read X0 s; read X1 s] =
                    ...
                    jolt_fp128_a7f7_p))
          (MAYCHANGE [PC; X0; X1; X5; X6; X7; X8; X9; X10; X11; X12] ,,
  ...
  ARM_ACCSTEPS_TAC JOLT_FP128_MUL_EXEC (1--35) (1--35) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN STRIP_TAC THEN

  RULE_ASSUM_TAC(REWRITE_RULE[COND_SWAP; GSYM WORD_BITVAL]) THEN

  ABBREV_TAC
   `mn4 = bignum_of_wordlist [mullo_s1; sum_s15; sum_s16; sum_s17]` THEN
  SUBGOAL_THEN `m * n < 2 EXP 256` ASSUME_TAC THENL
   [REWRITE_TAC[ARITH_RULE `2 EXP 256 = 2 EXP 128 * 2 EXP 128`] THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
   `2 EXP 256 * bitval carry_s17 + mn4 = m * n`
  ASSUME_TAC THENL
  ...
  SUBGOAL_THEN `mn4 = m * n` ASSUME_TAC THENL
   [MP_TAC(SPECL
      [`bitval carry_s17`; `2 EXP 256`; `mn4:num`; `m * n`]
      JOLT_FP128_NO_TOP_CARRY) THEN ASM_REWRITE_TAC[] THEN
    CONV_TAC NUM_REDUCE_CONV;
    ALL_TAC] THEN

  ABBREV_TAC `lo0 = bignum_of_wordlist [mullo_s1; sum_s15]` THEN
  ABBREV_TAC `hi0 = bignum_of_wordlist [sum_s16; sum_s17]` THEN
  SUBGOAL_THEN `mn4 = 2 EXP 128 * hi0 + lo0` ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["mn4"; "lo0"; "hi0"] THEN
    ...
    ALL_TAC] THEN
  ABBREV_TAC `t = bignum_of_wordlist [sum_s22; sum_s25; sum_s26]` THEN
  SUBGOAL_THEN `lo0 + 4294944759 * hi0 < 2 EXP 192` ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["lo0"; "hi0"] THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
   `2 EXP 192 * bitval carry_s26 + t = lo0 + 4294944759 * hi0`
  ASSUME_TAC THENL
  ...
  STRIP_ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["lo0"; "hi0"; "lo1"] THEN
    REWRITE_TAC[bignum_of_wordlist] THEN BOUNDER_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN `t = 2 EXP 128 * val(sum_s26:int64) + lo1` ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["t"; "lo1"] THEN
    REWRITE_TAC[bignum_of_wordlist] THEN ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN `val(sum_s26:int64) <= 4294944759` ASSUME_TAC THENL
   [MAP_EVERY UNDISCH_TAC
     [`t = lo0 + 4294944759 * hi0`;
      `t = 2 EXP 128 * val(sum_s26:int64) + lo1`;
      `lo0 < 2 EXP 128`; `hi0 < 2 EXP 128`; `lo1 < 2 EXP 128`] THEN
    CONV_TAC NUM_REDUCE_CONV THEN ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN `val(sum_s26:int64) * 4294944759 < 2 EXP 64`
  ASSUME_TAC THENL
   [UNDISCH_TAC `val(sum_s26:int64) <= 4294944759` THEN
    CONV_TAC NUM_REDUCE_CONV THEN ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
   `2 EXP 64 * val(mulhi_s27:int64) + val(mullo_s27:int64) =
    val(sum_s26:int64) * 4294944759`
  ASSUME_TAC THENL
   [REWRITE_TAC[GSYM REAL_OF_NUM_EQ; GSYM REAL_OF_NUM_CLAUSES] THEN
    ...
    CONV_TAC REAL_RAT_REDUCE_CONV THEN CONV_TAC REAL_RING;
    ALL_TAC] THEN
  SUBGOAL_THEN `val(mullo_s27:int64) = val(sum_s26:int64) * 4294944759`
  ASSUME_TAC THENL
   [MP_TAC(SPECL
      [`val(mulhi_s27:int64)`; `2 EXP 64`; `val(mullo_s27:int64)`;
       `val(sum_s26:int64) * 4294944759`] JOLT_FP128_NO_TOP_CARRY) THEN
    ASM_REWRITE_TAC[] THEN CONV_TAC NUM_REDUCE_CONV;
    ALL_TAC] THEN

  ABBREV_TAC `r = bignum_of_wordlist [sum_s28; sum_s29]` THEN
  ABBREV_TAC `u = bignum_of_wordlist [sum_s31; sum_s32]` THEN
  ABBREV_TAC `v = 2 EXP 128 * bitval carry_s29 + r` THEN
  SUBGOAL_THEN
   `2 EXP 128 * bitval carry_s29 + r =
    lo1 + val(mullo_s27:int64)`
  ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["r"; "lo1"] THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN `v = lo1 + 4294944759 * val(sum_s26:int64)` ASSUME_TAC THENL
   [EXPAND_TAC "v" THEN
    MP_TAC(ASSUME
     `2 EXP 128 * bitval carry_s29 + r =
      lo1 + val(mullo_s27:int64)`) THEN
    MP_TAC(ASSUME
     `val(mullo_s27:int64) = val(sum_s26:int64) * 4294944759`) THEN
    ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN `v < 2 * jolt_fp128_a7f7_p` ASSUME_TAC THENL
   [MATCH_MP_TAC(SPECL
     [`lo1:num`; `val(sum_s26:int64)`; `v:num`]
     JOLT_FP128_SECOND_FOLD_BOUND) THEN
    REPEAT CONJ_TAC THENL
     [ACCEPT_TAC(ASSUME `lo1 < 2 EXP 128`);
      ACCEPT_TAC(ASSUME `val(sum_s26:int64) <= 4294944759`);
      ACCEPT_TAC(ASSUME
       `v = lo1 + 4294944759 * val(sum_s26:int64)`)];
    ALL_TAC] THEN
   ...
   [ONCE_REWRITE_TAC[GSYM(ASSUME `mn4 = m * n`)] THEN
    MATCH_MP_TAC(SPECL
     [`mn4:num`; `t:num`; `v:num`; `hi0:num`; `lo0:num`;
      `val(sum_s26:int64)`; `lo1:num`] JOLT_FP128_TWO_FOLDS) THEN
    REPEAT CONJ_TAC THENL
  ...
  SUBGOAL_THEN `r < 2 EXP 128 /\ u < 2 EXP 128` STRIP_ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["r"; "u"] THEN BOUNDER_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN
   `2 EXP 128 * bitval carry_s32 + u = r + 4294944759`
  ASSUME_TAC THENL
  ...
  MATCH_MP_TAC EQ_TRANS THEN
  EXISTS_TAC `(if carry_s29 \/ carry_s32 then u else r):num` THEN
  CONJ_TAC THENL
   [MAP_EVERY EXPAND_TAC ["u"; "r"] THEN
    ASM_CASES_TAC `carry_s29:bool` THEN
    ASM_CASES_TAC `carry_s32:bool` THEN
    ASM_REWRITE_TAC
    ...
    MATCH_MP_TAC(SPECL
     [`v:num`; `r:num`; `u:num`; `carry_s29:bool`; `carry_s32:bool`]
