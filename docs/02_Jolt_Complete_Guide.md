---
title: "Jolt zkVM: Complete Implementation Guide"
abstract: "End-to-end guide to Jolt's zkVM implementation from cryptographic foundations through prover and verifier code. Covers preprocessing, execution, proof generation, and verification with moderate mathematical detail and complete code paths."
author: "Parti"
date: "2025-10-23"
lang: en-US
titlepage: true
...
# Jolt zkVM: Complete Implementation Guide

> ** Document Purpose**: This guide covers the **entire Jolt system** - foundations, proving, and verification. It's structured as an implementation journey from theory to code.
>
> ** Quick Navigation**:
> - **Learning the system?** Read sequentially (Part 0 \rightarrow  Part 4)
> - **Implementing prover?** Focus on Parts 0-3
> - **Implementing verifier?** Read Part 0, then Part 4, then [Verifier_Mathematics_and_Code.md](Verifier_Mathematics_and_Code.md)
> - **Need deep mathematics?** See [01_Jolt_Theory.md](../01_Jolt_Theory.md) for full derivations and security proofs
> - **Need just cryptography overview?** Part 0 is self-contained
>
> ** Math Detail Level**: This guide provides **moderate mathematical detail** - enough equations to understand what's happening, but not full proofs. For complete derivations, see 01_Jolt_Theory.md.

***

# Jolt Code Flow: The Complete Pipeline

This document traces the complete execution flow in Jolt, showing exactly which files are called, what they do, and the mathematical operations being performed.
...
- Jolt's proof system is structured as a graph where:
  ...
  - **Edges** = Polynomial evaluations (output of one sumcheck \rightarrow  input to another)
...
- `#[jolt::provable] fn sha3(...)` \rightarrow  generates `prove_sha3()`, `verify_sha3()`
- `#[jolt::provable] fn fibonacci(...)` \rightarrow  generates `prove_fibonacci()`, `verify_fibonacci()`
- `#[jolt::provable] fn my_func(...)` \rightarrow  generates `prove_my_func()`, `verify_my_func()`
...
### Detailed Execution Flow with File Paths

```

+-------------------------------------------------------------+
| 1. PREPROCESSING (once per program)                        |
|    examples/*/host/src/main.rs                             |

+-------------------------------------------------------------+
| compile_{fn}(target_dir)                                   |
|   \rightarrow  Compile guest Rust \rightarrow  RISC-V ELF                       |
|   \rightarrow  Parse bytecode, create Program struct                  |
|                                                             |
| preprocess_prover_{fn}(&mut program)                       |
|   \rightarrow  jolt-core/src/zkvm/mod.rs                             |
|   \rightarrow  Generate Dory SRS (generators)                         |
|   \rightarrow  Commit to bytecode polynomial                          |
|   \rightarrow  Returns JoltProverPreprocessing                        |

+-------------------------------------------------------------+
                          ↓

+-------------------------------------------------------------+
| 2. EXECUTION & WITNESS GENERATION                          |
|    prove_{fn}(input)                                       |

+-------------------------------------------------------------+
| RISC-V Emulation                                           |
|   \rightarrow  tracer/src/emulator/                                   |
|   \rightarrow  Execute each instruction via RISCVInstruction::execute()|
|   \rightarrow  Record: registers, memory, PC per cycle                |
|   \rightarrow  Output: Vec<Cycle> (execution trace)                   |
|                                                             |
| Witness Generation                                          |
|   \rightarrow  jolt-core/src/zkvm/witness.rs                         |
|   \rightarrow  Convert trace \rightarrow  multilinear polynomial extensions      |
|   \rightarrow  Create MLEs for: instructions, registers, memory       |

+-------------------------------------------------------------+
                          ↓

+-------------------------------------------------------------+
| 3. PROOF GENERATION (The DAG Execution)                    |
|    jolt-core/src/zkvm/dag/jolt_dag.rs                     |

+-------------------------------------------------------------+
| JoltDAG::prove()                                           |
|   |                                                         |
|   +-\rightarrow  Create StateManager                                  |
|   |   \rightarrow  jolt-core/src/zkvm/dag/state_manager.rs          |
|   |   \rightarrow  Central hub: transcript, accumulator, proofs      |
|   |                                                         |
|   +-\rightarrow  Fiat-Shamir preamble (bind public inputs)          |
|   |                                                         |
|   +-\rightarrow  Generate & commit polynomials                        |
|   |   \rightarrow  jolt-core/src/zkvm/witness.rs                     |
|   |   \rightarrow  jolt-core/src/poly/commitment/dory.rs            |
|   |   \rightarrow  PCS::batch_commit()                               |
|   |                                                         |
|   +-\rightarrow  STAGE 1: Spartan Outer Sumcheck                     |
|   |   \rightarrow  jolt-core/src/zkvm/r1cs/                          |
|   |   \rightarrow  Prove: \Sigma eq(\tau,x)·(Az(x)·Bz(x)-Cz(x))=0          |
|   |   \rightarrow  Output: Claims for Az(r), Bz(r), Cz(r)           |
|   |                                                         |
|   +-\rightarrow  STAGE 2: Batched Sumchecks                          |
|   |   \rightarrow  Spartan product sumchecks (3x)                    |
|   |   \rightarrow  Registers read-checking (Twist)                   |
|   |   \rightarrow  RAM read-checking (Twist)                         |
|   |   \rightarrow  Instruction lookups begin (Shout)                 |
|   |   \rightarrow  Bytecode lookups begin (Shout)                    |
|   |   \rightarrow  All run in parallel, batched interaction          |
|   |                                                         |
|   +-\rightarrow  STAGE 3: Evaluation Sumchecks                       |
|   |   \rightarrow  Spartan SPARK (sparse polynomial evaluation)      |
|   |   \rightarrow  Registers write-checking + evaluation (Twist)     |
|   |   \rightarrow  RAM write-checking + evaluation (Twist)           |
|   |   \rightarrow  Instruction/Bytecode lookups continue             |
|   |                                                         |
|   +-\rightarrow  STAGE 4: Final Sumchecks                            |
|   |   \rightarrow  RAM final evaluation (Twist)                      |
|   |   \rightarrow  Bytecode final evaluation (Shout)                 |
|   |   \rightarrow  Instruction lookups finalize                      |
|   |                                                         |
|   +-\rightarrow  STAGE 5: Batched Opening Proof (Dory)              |
|       \rightarrow  jolt-core/src/poly/opening_proof.rs               |
|       \rightarrow  Accumulate all polynomial evaluation claims       |
|       \rightarrow  Batch into single opening via random linear comb. |
|       \rightarrow  Dory-Reduce: recursive folding (log rounds)       |
|       \rightarrow  Output: Opening proof (~10 KB)                    |
...
+-------------------------------------------------------------+
| 4. VERIFICATION                                             |
|    verify_{fn}(input, output, proof)                       |

+-------------------------------------------------------------+
| JoltDAG::verify()                                          |
|   \rightarrow  jolt-core/src/zkvm/dag/jolt_dag.rs                   |
|   \rightarrow  Replays Fiat-Shamir challenges                        |
|   \rightarrow  Verifies each sumcheck (Stages 1-4)                   |
|   \rightarrow  Verifies Dory opening proof (Stage 5)                 |
|   \rightarrow  Cost: O(log N) per stage, 4-5 pairings                |
|   \rightarrow  Returns: true (accept) or false (reject)              |
...
|-----------|--------------|
| **Sumcheck Engine** | `jolt-core/src/subprotocols/sumcheck.rs` |
| **R1CS (Spartan)** | `jolt-core/src/zkvm/r1cs/` |
| **Memory (Twist)** | `jolt-core/src/zkvm/ram/`, `jolt-core/src/zkvm/registers/` |
| **Lookups (Shout)** | `jolt-core/src/zkvm/instruction_lookups/`, `jolt-core/src/zkvm/bytecode/` |
| **Polynomial Commitment** | `jolt-core/src/poly/commitment/dory.rs` |
| **DAG Orchestration** | `jolt-core/src/zkvm/dag/jolt_dag.rs`, `state_manager.rs` |
| **Emulator** | `tracer/src/emulator/`, `tracer/src/instruction/` |
| **Witness Generation** | `jolt-core/src/zkvm/witness.rs` |

**Critical Flow Insight**: The StateManager's opening accumulator creates the DAG structure. When Stage 1's sumcheck claims "Az(r) = v", this claim gets added to the accumulator. Stage 2 then proves this claim via product sumchecks, which in turn create new claims. This cascading dependency defines the 5-stage structure—each stage depends on the outputs of the previous.
...
- `compile_{your_function}()` compiles guest Rust \rightarrow  RISC-V ELF binary
...
**File**: [jolt-core/src/zkvm/dag/jolt_dag.rs](jolt-core/src/zkvm/dag/jolt_dag.rs) \rightarrow  `JoltDAG::prove()`
...
**File**: [jolt-core/src/zkvm/dag/jolt_dag.rs:574](jolt-core/src/zkvm/dag/jolt_dag.rs#L574) \rightarrow  `generate_and_commit_polynomials()`
...
**File**: [jolt-core/src/zkvm/spartan/mod.rs](jolt-core/src/zkvm/spartan/mod.rs) \rightarrow  `SpartanDag::stage1_prove()`
...
- Any mismatch \rightarrow  verification fails
...
- **Directed**: Data flows in one direction (stage 1 \rightarrow  2 \rightarrow  3 \rightarrow  4 \rightarrow  5)
...
- **Virtual edges** \rightarrow  Resolved by another sumcheck (creates graph dependency)
- **Committed edges** \rightarrow  Resolved by PCS opening proof in Stage 5 (terminal edges)
...
- [jolt-core/src/zkvm/witness.rs](jolt-core/src/zkvm/witness.rs): Converts trace \rightarrow  polynomials
...
**Sub-table**: `ADD_1` performs 1-bit full addition (3 inputs \rightarrow  2 outputs):
...
>
> - **Case 1**: $0 + 1 = 1$ \rightarrow  sum is `1`, carry `0` to next column
> - **Case 2**: $1 + 1 = 2 = 10_2$ \rightarrow  sum is `0` (for current position), carry `1` to next column
>
...
- Lookup: $\text{ADD}_1(a_1=0, b_1=1, \text{carry}_1=0)$ \leftarrow  **Uses carry from previous step!**
...
>
> - `sub_opcode = 1` \rightarrow  `ADD_4`
> - `sub_opcode = 2` \rightarrow  `XOR_4`
> - etc.
>
...
> ** For Deep Mathematics**: This part provides an overview of the six cryptographic protocols with key equations. For complete mathematical derivations, security proofs, and the full 5-stage DAG mathematical structure, see [01_Jolt_Theory.md](../01_Jolt_Theory.md).
...
> **Mathematical foundation**: See [01_Jolt_Theory.md Section 2.1](../01_Jolt_Theory.md#21-r1cs-constraints-spartan) for complete derivation with examples.

**Quick refresher**: Spartan proves R1CS constraints $Az \circ Bz = Cz$ where:
- $A, B, C$: Constraint matrices (sparse in Jolt)
- $z$: Witness vector (execution trace)
- $\circ$: Element-wise multiplication

**Key idea**: Instead of checking all $m$ constraints individually, use random linear combination + three sumchecks to verify all constraints hold with overwhelming probability.

**In Jolt's implementation**:
- **Constraint count**: ~30 constraints per cycle (e.g., PC update, register consistency, instruction linking)
- **Witness size**: ~100T elements (T = number of cycles)
- **Matrix sparsity**: 97%+ zeros (only ~30 non-zero entries per row)
- **Optimization**: SPARK protocol for efficient sparse polynomial handling
- **Code location**: [jolt-core/src/zkvm/r1cs/](../jolt-core/src/zkvm/r1cs/)

**Why Spartan + Lookups?**: R1CS handles arithmetic well but struggles with bitwise operations. A 64-bit XOR would require 64+ R1CS constraints. Jolt's solution: prove XOR via **16 lookups** into a pre-computed 4-bit XOR table (256 entries). Much faster!

***

## Protocol 2: SPARK - Efficient Sparse Polynomial Commitments

> **Mathematical foundation**: See [01_Jolt_Theory.md Section 3.1-3.3](../01_Jolt_Theory.md#sumcheck-31-33-spartan-matrix-evaluation-spark) for complete SPARK protocol details.

**Quick refresher**: SPARK makes polynomial commitments efficient for sparse polynomials by representing them as (vals, idxs) pairs.

**The problem**: Jolt's R1CS matrices are 97%+ sparse (only ~30 non-zeros per row out of 1024+ entries). Standard PCS costs $O(T^2)$ - wasteful!

**SPARK's solution**:
1. Represent sparse polynomial with $s$ non-zeros as two dense vectors: `vals[s]` and `idxs[s]`
2. Commit to these small dense representations instead of full sparse matrix
3. Evaluation $\widetilde{P}(r) = v$ becomes summation: $\sum_{j} \widetilde{vals}(j) \cdot \text{eq}(\widetilde{idxs}(j), r)$
4. Verify via sumcheck

**Result**: Prover work $O(s \log s)$ instead of $O(2^k)$!

**In Jolt's implementation**:
- Used for Spartan matrix evaluations $\widetilde{A}(r, s)$, $\widetilde{B}(r, s)$, $\widetilde{C}(r, s)$
- Code location: [jolt-core/src/subprotocols/spark.rs](../jolt-core/src/subprotocols/spark.rs)

***

## Protocol 3: Lasso - Lookup Arguments for Structured Tables

> **Mathematical foundation**: See [01_Jolt_Theory.md Section 2.2](../01_Jolt_Theory.md#22-instruction-execution-shout-lookup-argument) for complete Lasso/Shout derivation with examples.

**Quick refresher**: Lasso proves that $m$ lookups into a table of size $N$ all return correct values, with prover work $O(m \log m)$ instead of $O(N)$.

**The problem**: For 64-bit operations, $N = 2^{128}$ (two 64-bit operands). Table too large to materialize!

**Lasso's solution - Sparse "hit" vector**:
1. Create sparse polynomial $\widetilde{E}(y)$ over table indices (0 everywhere except at accessed indices)
2. Prove: $\sum_{y} \widetilde{E}(y) \cdot (\widetilde{T}(y) - \widetilde{a}(y)) = 0$
   - For every accessed index ($\widetilde{E}(y) \neq 0$), claimed value ($\widetilde{a}(y)$) must match table ($\widetilde{T}(y)$)
3. Run sumcheck over $\log N$ variables
4. Key insight: Sparsity of $\widetilde{E}$ means prover work scales with lookups $m$, not table size $N$!

**Table decomposition for 64-bit ops**: A 64-bit ADD becomes **16 lookups into a tiny 4-bit ADD table** (256 entries). This decomposes the massive $2^{128}$ table into manageable $2^8$ sub-tables, with wiring polynomials $\widetilde{M}_{V,j}$ proving correct bit-chunking.

**In Jolt's implementation**:
- Used for instruction semantics (ADD, XOR, SLL, etc.)
- Each instruction defines its efficiently-evaluable table MLE
- Code location: [jolt-core/src/zkvm/instruction_lookups/](../jolt-core/src/zkvm/instruction_lookups/)
- Prefix-suffix sumcheck algorithm enables tractable proving over $2^{128}$ space
...
> **See [01_Jolt_Theory.md](../01_Jolt_Theory.md) Section 2.3** for detailed mathematical derivation with concrete examples of time-ordered vs address-ordered traces, fingerprinting, and the grand product approach.

**Quick summary**:
- Memory operations represented as tuples $(addr, ts, val, op)$
- Two orderings: time-ordered (as executed) and address-ordered (sorted by address)
- Prove they're permutations via fingerprint grand product
- Address-ordered checking is local and efficient
- Twist uses incremental formulation with chunking parameter $d$
...
> **Mathematical foundation**: See [01_Jolt_Theory.md Section 2.3](../01_Jolt_Theory.md#23-bytecode-shout-offline-memory-checking) for complete Shout derivation with examples.

**Quick refresher**: Shout proves correct reads from **pre-committed, read-only memory**—simpler than Lasso because all values are fixed during preprocessing.

**Key distinction from Lasso**:
- **Lasso**: Dynamic table lookups, values computed during execution, table may be virtual
- **Shout**: Fixed pre-committed memory, just prove correct read access pattern
- **Analogy**: Lasso = "I computed f(x) correctly". Shout = "I read location x from this sealed document"

**Shout's approach for bytecode checking**:
1. **Preprocessing**: Decode bytecode into components (opcode, rs1, rs2, rd, imm, flags), commit each as polynomial
2. **Execution**: Prover claims to read instruction at PC = p with specific decoded values
3. **Verification**: Prove $\sum_{p} \widetilde{E}_{\text{bc}}(p) \cdot (\widetilde{\text{BC}}(p) - \widetilde{\text{claimed}}(p)) = 0$
   - For every executed PC ($\widetilde{E}_{\text{bc}}(p) \neq 0$), claimed instruction matches committed bytecode
4. **Sparse evaluation claims** resolved via SPARK

**In Jolt's implementation**:
- Used for bytecode verification (proves execution trace matches committed program)
- Also used for instruction lookups with efficiently-evaluable table MLEs
- Code location: [jolt-core/src/zkvm/bytecode/](../jolt-core/src/zkvm/bytecode/)

***

## Protocol 6: Dory - Polynomial Commitment Scheme

> **Mathematical foundation**: See [01_Jolt_Theory.md Section 3.5](../01_Jolt_Theory.md#35-dory---the-polynomial-commitment-scheme) for complete Dory derivation with detailed 4-element example showing full folding protocol.

**Quick refresher**: Dory is a transparent polynomial commitment scheme achieving logarithmic verifier complexity through bilinear pairings and recursive inner product arguments.

**Core idea - Two-tiered commitment**:
1. **Layer 1**: Commit to polynomial coefficients arranged as $\sqrt{N} \times \sqrt{N}$ matrix using Pedersen commitments (one per row)
2. **Layer 2**: Commit to vector of row commitments using AFGHO pairing-based commitment
3. **Result**: Single $\mathbb{G}_T$ element (192 bytes) binding prover to entire $N$-coefficient polynomial

**Key properties**:
- **Transparent**: No trusted setup (generates public SRS)
- **Succinct commitment**: Constant size regardless of polynomial degree
- **Logarithmic verification**: $O(\log N)$ verifier operations
- **Homomorphic**: Can compute $C_{aP+bQ} = C_P^a \cdot C_Q^b$ without knowing $P$ or $Q$

**Opening protocol - Dory-Reduce**:
- Transforms polynomial evaluation $f(\vec{x}) = y$ into three inner product relations
- Recursively folds vectors from size $n$ to $n/2$ to $n/4$ ... to 1
- Two challenges per round: $\beta$ (witness randomization), $\alpha$ (folding)
...
+- G_T exponentiations: ~1.2B cycles (80%) \leftarrow  THE BOTTLENECK
|  +- ~40 exponentiations in F_q^12 (3072-bit elements)

+- Everything else: ~300M cycles (20%)
```

**Key optimization**: Batched multi-exponentiation across all rounds reduces ~200 individual exponentiations to ~40 effective exponentiations.

**In Jolt's implementation**:
- **Stage 5**: Batched opening proof for all polynomial evaluations from Stages 1-4
- **Batching**: Random linear combination of ~30-50 evaluation claims into single opening
- **Proof size**: $O(\log N)$ group elements ≈ 10 KB
- Code location: [jolt-core/src/poly/commitment/dory.rs](../jolt-core/src/poly/commitment/dory.rs)

**Implications for on-chain verification**:
- Native Dory verification: ~4M gas for 40 $\mathbb{G}_T$ exponentiations (not viable)
- Jolt's solution: SNARK composition with BN254 \leftrightarrow  Grumpkin cycle reduces to <30M cycles
...
1. **Spartan** (Stages 1-3): Proves R1CS constraints \rightarrow  outputs sparse matrix evaluation claims
2. **SPARK** (embedded): Proves sparse evaluations \rightarrow  outputs dense (vals, idxs) evaluation claims
3. **Twist** (Stages 2-3): Proves memory consistency (registers + RAM) \rightarrow  outputs increment polynomial evaluation claims
   - Stage 2: Time-ordered verification (read-checking, write-checking)
   - Stage 3: Address-ordered verification (evaluation sumchecks)
4. **Shout** (Stages 2-4): Proves instruction lookups and bytecode \rightarrow  outputs sparse hit vector evaluation claims
5. **SPARK** (again, embedded): Proves Shout's sparse claims \rightarrow  outputs dense evaluation claims
6. **Dory** (Stage 5): Proves all accumulated dense evaluation claims \rightarrow  cryptographic verification
...
#### Concrete Example: 2\times 2 Bytecode Matrix
...
**Result**: Single $\mathbb{G}_T$ element (192 bytes) commits to entire 2\times 2 matrix!
...
- Bytecode: 1024 instructions \times  4 bytes = 4KB
- Commitment: 192 bytes (~20\times  compression)
...
Matrix dimensions: √16,384 \times  √16,384 = 128 \times  128
...
Savings: 64\times  reduction in SRS size
```
...
Total size: N = 16 \times  65,536 = 1,048,576 = 2^20

Matrix: √1,048,576 \times  √1,048,576 = 1,024 \times  1,024

SRS size:

- 1,024 G_1 points ≈ 1024 \times  48 bytes = 49 KB
- 1,024 G_2 points ≈ 1024 \times  96 bytes = 98 KB
- Total prover SRS: ~147 KB

Verifier SRS: Much smaller (only needs subset for verification)

- Typically 10-20\times  smaller than prover SRS
```
...
- **But**: ~40 G_T exponentiations dominate the cost
  - **Batched recursive folding**: ~30 G_T exps (would be ~10 per round \times  23 rounds = 230 naively, but verifier accumulates and computes once at end)
...
- Dory views polynomials as **matrices** (rows \times  columns)
- For a polynomial of degree K·T, Dory uses a sqrt(K·T) \times  sqrt(K·T) matrix
...
1. Convert bytecode vector \rightarrow  multiple MLEs (one per field: opcode_mle, rd_mle, rs1_mle, etc.)
...
**Size**: Typically ~25 KB (10-20\times  smaller than prover setup)
...
1. Convert bytecode \rightarrow  MLEs \rightarrow  commit each to get $C_{\text{opcode}}, C_{\text{rd}}, \ldots \in \mathbb{G}_T$
2. Convert trace \rightarrow  MLEs \rightarrow  commit to get $C_{\text{registers}}, C_{\text{RAM}}, \ldots \in \mathbb{G}_T$
...
2. **Witness remapping**: Guest addresses \rightarrow  witness polynomial indices
...
pub struct BytecodePreprocessing {
    ...
    pub pc_map: BytecodePCMapper,       // Maps real addresses \rightarrow  virtual PC
...
pub fn preprocess(memory_init: Vec<(u64, u8)>) -> Self {
    ...
    // Convert bytes \rightarrow  64-bit words (RISC-V is 64-bit)
...
|    |       - Convert bytes \rightarrow  words                          |
...
- [x] Create PC mapper (address \rightarrow  virtual PC)
...
- [x] Calculate matrix dimensions (sqrt(K·T) \times  sqrt(K·T))
...
- Different inputs \rightarrow  same preprocessing, different proofs
- Change program \rightarrow  must recompute preprocessing
...
2. **Witness generation** ([jolt-core/src/zkvm/witness.rs](jolt-core/src/zkvm/witness.rs)):
   - Convert execution trace \rightarrow  MLEs
   - Uses `MemoryLayout` to remap guest addresses \rightarrow  witness indices
   - Convert bytecode \rightarrow  MLE (using `BytecodePreprocessing`)
...
Think of it as: **Run the program \rightarrow  Record what happened \rightarrow  Convert to math**
...
## Part 4: Witness Generation (Trace \rightarrow  Polynomials)
...
- **Inputs**: $(a_1, b_1), (a_2, b_2), \ldots, (a_T, b_T)$ \rightarrow  MLEs
- **Outputs**: $(c_1, \text{flags}_1), (c_2, \text{flags}_2), \ldots$ \rightarrow  MLEs
- **Lookup indices**: Decomposed chunks for Shout protocol \rightarrow  One-hot MLEs
...
For a read-only address: sum of $\Delta$ is zero (no writes) \rightarrow  final = initial 
...
- **Overwriting same value**: Cycle 5 writes 350 to address already containing 350 \rightarrow  $\Delta = 0$
...
- Sum is **non-zero** \rightarrow  Verifier rejects!
...
- Pre-computed table: $\text{ADD\_table}(10, 20) = 30$ \leftarrow  This is `value_written(k)` (the "ground truth")
...
**Important**: Addresses are typically chunked (e.g., 64-bit address \rightarrow  16 chunks of 4 bits). For simplicity, assume 2 possible addresses.
...
- One-hot witnesses: $\mathbb{F}^{\log T} \times \mathbb{F}^{\log N}$ (cycle index \times  table index)
...
struct WitnessData {
    ...
    instruction_ra: [Vec<Option<u8>>; 16],   // 16 chunks \times  T entries
    bytecode_ra: Vec<Vec<Option<u8>>>,       // d chunks \times  T entries
    ram_ra: Vec<Vec<Option<u8>>>,            // d chunks \times  T entries
}
```
...
### **Conversion Type 1: Simple Vectors (Already Dense) \rightarrow  MLE**
...
```rust
// No sparse\rightarrow dense conversion needed! Just field conversion
let coeffs: Vec<F> = data.left_instruction_input
    .iter()
    .map(|&x| F::from(x))    // u64 \rightarrow  field element
    .collect();
...
### **Conversion Type 2: Sparse One-Hot Indices \rightarrow  Dense Matrix \rightarrow  MLE**

**Applies to**: `instruction_ra`, `bytecode_ra`, `ram_ra`

 **KEY TRANSFORMATION: Sparse \rightarrow  Dense**
...
**Why this matters**: When we say "Step 10C converts sparse \rightarrow  dense", we mean:
...
- 10C: 256 KB (storing full T\times 256 matrix with mostly zeros)
...
// Step 10C: Convert sparse \rightarrow  dense
let mut dense_matrix = vec![vec![0u8; TABLE_SIZE]; T];  // Size T \times  N
for (j, &maybe_k) in sparse_indices.iter().enumerate() {
    if let Some(k) = maybe_k {
        dense_matrix[j][k as usize] = 1;  // Expand: one index \rightarrow  full one-hot row
    }
}

// Convert dense matrix \rightarrow  MLE
let mle_ra = MultilinearPolynomial::from_2d_matrix(dense_matrix);
```

**Why convert sparse \rightarrow  dense?**
...
3. **Memory trade-off**:
   ...
   - Dense: $T \times N$ entries (e.g., 1024 \times  256 = 256 KB)
...
If cycles looked up: cycle 0 \rightarrow  index 10, cycle 1 \rightarrow  index 15, cycle 2 \rightarrow  no lookup, cycle 3 \rightarrow  index 10:
...
| **Conversion needed?** |  No (already dense) |  Yes (sparse \rightarrow  dense) |
...
| **Example size** | 1 KB \rightarrow  1 KB | 1 KB \rightarrow  256 KB |
...
**Conversion**: Already dense in Step 10B \rightarrow  Just type convert $\mathbb{Z} \to \mathbb{F}$ in Step 10C
...
**Conversion**: Sparse indices in Step 10B \rightarrow  Expand to dense $T \times N$ matrix in Step 10C \rightarrow  MLE
...
HashMap<CommittedPolynomial, MultilinearPolynomial<F>> {
    // Type 1: 8 simple witness polynomials (8 \times  32 KB = 256 KB total)
...
- **After Dory commitment**: ~7 KB (35 commitments \times  192 bytes each)

**The compression**: 220 MB of witness data \rightarrow  7 KB of commitments!
...
- **Type 2**: One-hot MLE $\mathbb{F}^{\log T} \times \mathbb{F}^{\log N} \to \mathbb{F}$ (sparse \rightarrow  dense in Step 10C) - 27 polynomials
...
1. **Reshape to matrix**: $N$ coefficients \rightarrow  $\sqrt{N} \times \sqrt{N}$ matrix $M$
...
MLE: L̃(j) : F^10 \rightarrow  F where L̃(0,0,...,0,0) = 100, L̃(1,0,...,0,0) = 250, etc.
```
...
Step 1: Reshape L̃ evaluations into 128\times 128 matrix M_L
...
Total commitments sent to verifier: 35 \times  192 bytes ≈ 7 KB

Compare to:

- Raw MLEs: ~220 MB (dense matrix data)
- Raw trace: 1024 cycles \times  ~100 bytes/cycle ≈ 100 KB
- Commitments: 7 KB

Compression ratio: ~30,000\times  compared to MLEs! ~14\times  compared to raw trace!
```
...
|  |   |     - Convert vectors \rightarrow  MLEs                     |       |
|  |   |                                                   |       |
|  |   |  2. PCS::batch_commit()                          |       |
...
- [x] Convert vectors \rightarrow  MLEs
- [x] Convert one-hot indices \rightarrow  MLEs
...
**Result**: **35 commitments** sent to verifier, each 192 bytes \rightarrow  **~7 KB total**
...
- $A, B, C \in \mathbb{F}^{m \times n}$ are constraint matrices (size: $m$ constraints \times  $n$ variables)
...
- $T = 1024$ cycles \rightarrow  $m = 30 \times 1024 = 30,720$ total constraints
...
**Total**: 6 categories \times  ~5 constraints each ≈ **30 constraints per cycle**
...
2. **Temporary computation**
   - $Az$ is only needed during Stage 1 \rightarrow  Stage 2 transition
...
- **64 MB commitment** per matrix \times  3 matrices = **192 MB of commitments!**
- **~18 KB opening proof** per matrix \times  3 matrices = **54 KB of proof overhead**
...
1. **Witness is secret/large**
   ...
   - 35 polynomials \times  average size = ~200 MB of data
...
- 10 polynomials \times  4 coefficients each = **40 field elements** (~1.3 KB)
...
- $\text{eq}(\tau, \vec{r})$ \leftarrow  Verifier computes directly (public)
- $\widetilde{Az}(\vec{r})$ \leftarrow  **VIRTUAL POLYNOMIAL CLAIM**
- $\widetilde{Bz}(\vec{r})$ \leftarrow  **VIRTUAL POLYNOMIAL CLAIM**
- $\widetilde{Cz}(\vec{r})$ \leftarrow  **VIRTUAL POLYNOMIAL CLAIM**
...
- $\widetilde{A}_{\vec{r}}(\vec{r}')$ \leftarrow  Need to compute
- $\widetilde{z}(\vec{r}')$ \leftarrow  **COMMITTED POLYNOMIAL CLAIM** (witness!)
...
- **Memory footprint**: ~7 million \times  32 bytes = ~224 MB for the witness vector
...
- **Level 1 \rightarrow  Level 2**: We computed MLEs of each trace component separately
...
- **Level 1 \rightarrow  Level 3**: We concatenate all trace vectors into one giant vector $z$
  - $z = [\text{public}, L_0, \ldots, L_{1023}, R_0, \ldots, R_{1023}, \ldots]$
  - This is a **different data structure** - a flat vector, not a polynomial

- **Level 3 \rightarrow  Level 4**: We compute the MLE of the giant vector $z$
...
2. **Instruction lookup addresses** (Type 2 - one-hot matrices):
   ...
   For chunk 0 (bit position 0): operands are $(L_0, R_0) = (1, 1)$ \rightarrow  lookup index $k = 3$ (binary: `11`)
   ...
   - Chunk 1: $(L_1, R_1) = (0, 1)$ \rightarrow  $k = 1$ \rightarrow  $\widetilde{\text{ra}}_1(0, 1) = 1$
   - Chunk 2: $(L_2, R_2) = (1, 1)$ \rightarrow  $k = 3$ \rightarrow  $\widetilde{\text{ra}}_2(0, 3) = 1$
   - Chunk 3: $(L_3, R_3) = (0, 0)$ \rightarrow  $k = 0$ \rightarrow  $\widetilde{\text{ra}}_3(0, 0) = 1$
...
**Verifier checks**: $v_A \cdot v_B - v_C \stackrel{?}{=} 0$ \rightarrow  $42.7 \times 31.2 - 1332.24 = 0$ 
...
- $\text{ADD\_table}(0) = 0 + 0 = 0$ (binary: `00` \rightarrow  0)
- $\text{ADD\_table}(1) = 0 + 1 = 1$ (binary: `01` \rightarrow  1)
- $\text{ADD\_table}(2) = 1 + 0 = 1$ (binary: `10` \rightarrow  1)
- $\text{ADD\_table}(3) = 1 + 1 = 0$ (binary: `11` \rightarrow  0, with carry)
...
- $\text{bit\_extractor}_0(j)$ extracts bit 0 from $\widetilde{L}(j)$ \rightarrow  from 5 = `0101`, bit 0 = 1
- $\text{bit}_0(k)$ extracts bit 0 from index $k$ \rightarrow  from 3 = `11`, bit 0 = 1
...
- No Spartan \rightarrow  Can't verify constraints
- No Twist \rightarrow  Can't verify register operations
- No Shout \rightarrow  Can't verify ADD semantics
- No Linking \rightarrow  Can't connect operands to lookups
- No Dory \rightarrow  Can't verify commitments are honest
...
**In Jolt**: $m = 30 \times 1024 = 30,720$, $n = 35$ \rightarrow  ~1M operations (milliseconds!)
...
- $m = 2$ constraints \rightarrow  $\log m = 1$
- $n = 4$ variables \rightarrow  $\log n = 2$
- $z = (1, 5, 7, 35)$
...
- $\widetilde{z}(456, 789)$ is **committed** (witness) \rightarrow  added to `committed_openings`
- $\widetilde{A}_{123}(456, 789)$ is **virtual** (public matrix) \rightarrow  verify in Stage 3
...
Both compute the same value \rightarrow  virtual claim verified!
...
4. **Three-stage nesting enables efficiency**:
   - Stage 1: $m$ constraints \rightarrow  1 random point
   - Stage 2: $m \times n$ matrix-vector product \rightarrow  1 random point
...
- **Stage 1**: Proves R1CS constraints satisfied \rightarrow  outputs claims about $Az, Bz, Cz$
- **Stage 2**: Proves those claims \rightarrow  outputs claims about matrix $A, B, C$ and witness $z$
- **Stage 3**: Proves matrix claims \rightarrow  finally reduces to committed witness polynomials
...
| `InstructionRa(0)` | $\widetilde{\text{ra}}_0(j,k)$ | $C_{ra_0} \in \mathbb{G}_T$ | T\times 256 coefficients |
| `InstructionRa(1)` | $\widetilde{\text{ra}}_1(j,k)$ | $C_{ra_1} \in \mathbb{G}_T$ | T\times 256 coefficients |
| ... | ... | ... | ... |
...
// committed_openings = {
//     LeftInstructionInput     \rightarrow  (r_1, 100),
//     RightInstructionInput    \rightarrow  (r_2, 250),
//     RdInc                    \rightarrow  (r_3, 350),
//     InstructionRa(0)         \rightarrow  (r_4, 1),
//     ...
//     WitnessZ                 \rightarrow  (r', 14),
// }
...
**File**: [jolt-core/src/zkvm/witness.rs](../jolt-core/src/zkvm/witness.rs) \rightarrow  `CommittedPolynomial::generate_witness_batch()`
...
**File**: [jolt-core/src/poly/commitment/dory.rs](../jolt-core/src/poly/commitment/dory.rs) \rightarrow  `PCS::batch_commit()`
...
|------|---------------------|-------------------------|
| Witness polynomials | $\widetilde{P}_1, \ldots, \widetilde{P}_m \in \mathbb{F}^N$ | ~50 \times  8KB = 400 KB |
| Commitments | $C_1, \ldots, C_m \in \mathbb{G}_T$ | ~50 \times  192 bytes ≈ 10 KB |
| Opening hints | Blinding factors for later | ~50 \times  64 bytes ≈ 3 KB |
| Updated transcript | Hash state $\mathcal{O}$ | 32 bytes |

**Compression achieved**: 400 KB witness \rightarrow  10 KB commitments (40\times  compression!)
...
**Critical transition**: Virtual polynomial $Az$ \rightarrow  Committed polynomial $z$
...
All committed polynomials \rightarrow  added to `committed_openings`.
...
2. **Different dimensionalities**: Evaluation points have different dimensions
   ...
   - $\widetilde{\text{ra}}_0(\vec{r}) \in \mathbb{F}^{18}$ (Type 2: cycle \times  table-indexed)
...
- Cost: 36 \times  6 KB = **216 KB** just for openings!
- Verifier work: 36 separate verification procedures

**Jolt's approach**: **Batch all 36 openings together** \rightarrow  **~6 KB total** (36\times  reduction!)
...
**File**: [jolt-core/src/poly/opening_proof.rs](../jolt-core/src/poly/opening_proof.rs) \rightarrow  `reduce_and_prove()`
...
- $\widetilde{\text{ra}}_0: \{0,1\}^{18} \to \mathbb{F}$ (1024 cycles \times  256 table entries)
...
**Sumcheck proof size**: 23 rounds \times  4 coefficients \times  32 bytes = **2.9 KB**
...
**File**: [jolt-core/src/poly/commitment/dory.rs](../jolt-core/src/poly/commitment/dory.rs) \rightarrow  `prove_evaluation()`
...
**Total Dory proof size**: 23 rounds \times  (2 commitments + 2 values) = 23 \times  448 bytes = **~10 KB**
...
**Compare to naïve approach**: 36 \times  10 KB = **360 KB**

**Savings**: **~28\times  reduction** in proof size!

**Verifier work**:

- Reduction sumcheck: 23 rounds \times  $O(1)$ = $O(23)$ field operations
- Compute eq values: 36 \times  $O(23)$ = $O(828)$ field operations
- Dory verification: 23 rounds \times  1 pairing check = $O(23)$ pairings
...
**Savings**: $\sim 50 \times 10\text{ KB} = 500\text{ KB} \rightarrow 10\text{ KB}$ (50\times  reduction!)
...
> ** For Detailed Implementation Reference**: This section provides a high-level overview of verification. For complete stage-by-stage mathematical analysis with line-by-line code mapping, see [Verifier_Mathematics_and_Code.md](Verifier_Mathematics_and_Code.md).
...
pub struct JoltProof {
    ...
    commitments: Vec<GroupElement>,  // 36 \times  192 bytes = ~7 KB
...
Verifier transcript:
  ...
  outputs: [12]
  T = 1024
  C_L = 0x1a2b3c...  \leftarrow  Must match exactly!
```

Even one bit difference \rightarrow  complete verification failure.

***

## The Five Verification Stages

> ** Detailed Analysis**: For complete mathematical equations and code walkthrough of each stage, see [Verifier_Mathematics_and_Code.md - Stage-by-Stage Analysis](Verifier_Mathematics_and_Code.md#stage-by-stage-mathematical-analysis).
...
**Output**: Evaluation points for $Az$, $Bz$, $Cz$ \rightarrow  passed to Stage 2
...
**Stage 2: Product Sumchecks + Read/Write Checking** (6 sumchecks batched)
- **Spartan InnerSumcheck**: Batches Az, Bz, Cz evaluation with RLC (1 instance for 3 claims)
- **Registers ReadWriteChecking**: Twist time-ordered memory checking for registers
- **RAM**: RafEvaluation, ReadWriteChecking, OutputSumcheck (3 instances)
- **Instruction Lookups BooleanitySumcheck**: Proves lookup indices well-formed
- **All verified together**: Single batched sumcheck with random combination $\rho_i$

**Stage 3: Evaluation Sumchecks + Write Checking** (8 sumchecks batched)
- **Spartan**: PCSumcheck + ProductVirtualizationSumcheck (2 instances)
- **Registers ValEvaluationSumcheck**: Twist address-ordered memory checking
- **RAM**: ValEvaluation, ValFinal, HammingBooleanity (3 instances)
- **Instruction Lookups**: ReadRafSumcheck, HammingWeightSumcheck (2 instances)

**Stage 4: Final Component Sumchecks** (7 sumchecks batched)
- **RAM**: HammingWeightSumcheck, BooleanitySumcheck, RaSumcheck (3 instances)
- **Bytecode**: ReadRafSumcheck, BooleanitySumcheck, HammingWeightSumcheck (3 instances)
- **Instruction Lookups RaSumcheck**: Final virtualization (1 instance)
...
```rust
JoltProof {
    commitments: [C_z, C_L, C_R, C_\Delta_rd, C_ra_0, C_ra_1, C_ra_2, C_ra_3],  // 8 commitments
    sumcheck_proof_stage1: \pi_1,  // Spartan outer
    sumcheck_proof_stage2: \pi_2,  // Registers + Instructions Booleanity
    sumcheck_proof_stage3: \pi_3,  // Instructions Read-checking + Hamming
    sumcheck_proof_stage4: \pi_4,  // R1CS Linking
    opening_proof: \pi_Dory,       // Batched Dory opening
}
...
- Proof size: ~10 KB
- Prover time: ~100 ms (10\times  slower)
- Trace size: ~1 KB
...
- Operands: $(L_0, R_0) = (1, 1)$ \rightarrow  table index $k = 3$
...
|-------|------------|------------|
| Stage 1 | 15 rounds \times  O(1) | O(log m) = O(log T) |
| Stage 2 | 23 rounds \times  O(1) | O(log N) |
| Stage 3 | 18 rounds \times  O(1) | O(log N) |
| Stage 4 | 18 rounds \times  O(1) | O(log N) |
| Stage 5 Reduction | 23 rounds \times  O(1) | O(log N) |
...
3. **Fiat-Shamir is critical**: Same transcript \rightarrow  same challenges \rightarrow  binding
