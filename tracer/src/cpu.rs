...
pub enum TraceStep {
    ...
    Memory {
        ...
        // rs1_mem_pre_value: Option<u64>,
        // rd_pre_value: Option<u64>,

        // rs1_mem_post_value: Option<u64>,
        // rd_post_value: Option<u64>,
    },
}
...
fn trace_r(inst: &Instruction, cpu: &mut Cpu, word: u32, _address: u64) {
    let f = parse_format_r(word);
    let t = TraceStep::Arithmetic { 
        opcode: inst.name,
        imm: None,
        rs1: Some(f.rs1),
        rs2: Some(f.rs2),
...
fn trace_i(inst: &Instruction, cpu: &mut Cpu, word: u32, _address: u64) {
    let f = parse_format_i(word);
    let t = TraceStep::Arithmetic { 
        opcode: inst.name,
        imm: Some(f.imm.into()),
...
fn trace_i_mem(inst: &Instruction, cpu: &mut Cpu, _word: u32, _address: u64) {
    // let f = parse_format_i(word);
    let t = TraceStep::Memory { opcode: inst.name };

    cpu.trace.push(t)
}

fn trace_s(inst: &Instruction, cpu: &mut Cpu, word: u32, _address: u64) {
    let f = parse_format_s(word);
    let t = TraceStep::Arithmetic { 
        opcode: inst.name,
        imm: Some(f.imm.into()),
...
fn trace_b(inst: &Instruction, cpu: &mut Cpu, word: u32, _address: u64) {
    let f = parse_format_b(word);
    let t = TraceStep::Arithmetic { 
        opcode: inst.name,
        imm: Some(f.imm.into()),
...
fn trace_u(inst: &Instruction, cpu: &mut Cpu, word: u32, _address: u64) {
    let f = parse_format_u(word);
    let t = TraceStep::Arithmetic { 
        opcode: inst.name,
        imm: Some(f.imm.into()),
...
fn trace_j(inst: &Instruction, cpu: &mut Cpu, word: u32, _address: u64) {
    let f = parse_format_u(word);
    let t = TraceStep::Arithmetic { 
        opcode: inst.name,
        imm: Some(f.imm.into()),
