...
/// A single FieldReg access for downstream witness generation. At `cycle_index`
/// on slot `slot`, the register transitioned from `old` to `new` (natural-form
/// `[u64;4]`). Read-only accesses emit an event with `old == new`. Events
/// feed the FR Twist witness on the host side.
...
pub struct Cpu {
    ...
    /// Mutated by FieldOp / FieldMov / FieldSLL* instructions.
    pub field_regs: [[u64; 4]; FIELD_REG_COUNT],
    /// Per-cycle FieldReg event stream, drained by the host to build the
    /// FR Twist witness.
