...
declare_riscv_instr!(name = VirtualLaneMaskH, mask = 0, match = 0, format = FormatI, ram = ());

impl VirtualLaneMaskH {
    fn exec(&self, cpu: &mut Cpu, _: &mut <Self as RISCVInstruction>::RAMAccess) {
        ...
        cpu.write_register(
            ...
            0xffffu64.wrapping_shl(8 * (address as u32 & 6)) as i64,
        );
    }
}

impl RISCVTrace for VirtualLaneMaskH {}
