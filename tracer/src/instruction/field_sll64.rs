//! FieldSLL64 (funct3 = 0x08) — integer→field shift-left-64 on-ramp.
//!
//! Writes `FReg[frd] = (XReg[rs1] as Fr) · 2⁶⁴`, which in natural-form limbs
//! places the integer in limb 1:
//!
//!   FReg[frd] = [0, rs1_value, 0, 0]
//!
//! Single-cycle R-type, opcode `0x0B` (custom-0), funct7 `0x41` (SLL sub-family).
//! funct3 `0x00` within funct7=0x41 selects SLL64. (funct7=0x40 is saturated
//! by the 2-input FReg↔FReg ops; the 1-input XReg→FReg SLL ops live under 0x41.)
...
use super::{
    field_arith_common::FIELD_OP_OPCODE,
    format::{format_r::FormatR, InstructionFormat},
    Cycle, NormalizedInstruction, RISCVCycle, RISCVInstruction, RISCVTrace,
};

/// funct7 sub-family for the three integer→field shift-left ops. Distinct from
/// the `0x40` family used by the 2-input FReg↔FReg arithmetic + assertion ops
/// because the 3-bit funct3 space under `0x40` is saturated.
pub const BN254_FR_SLL_FUNCT7: u32 = 0x41;

/// funct3 within the SLL sub-family (0x41) selecting the 64-bit shift.
pub const FUNCT3_FIELD_SLL64: u8 = 0x00;

#[derive(Debug, Clone, Copy, Default, Serialize, Deserialize, PartialEq)]
pub struct FieldSLL64 {
    pub address: u64,
...
impl RISCVInstruction for FieldSLL64 {
    const MASK: u32 = 0xfe00_707f;
    const MATCH: u32 =
        (BN254_FR_SLL_FUNCT7 << 25) | ((FUNCT3_FIELD_SLL64 as u32) << 12) | FIELD_OP_OPCODE;
...
impl RISCVTrace for FieldSLL64 {
...
impl From<NormalizedInstruction> for FieldSLL64 {
...
impl From<FieldSLL64> for NormalizedInstruction {
    fn from(instr: FieldSLL64) -> NormalizedInstruction {
        NormalizedInstruction {
            instruction_kind: JoltInstructionKind::FieldSLL64,
...
mod tests {
    ...
    #[test]
    fn field_sll64_lands_in_limb_one() {
        let mut cpu = test_cpu();
        cpu.x[5] = 0xabcd_ef01_2345_6789_u64 as i64;
        let word = encode_r(
            FIELD_OP_OPCODE,
            FUNCT3_FIELD_SLL64 as u32,
            BN254_FR_SLL_FUNCT7,
            3,
            5,
            0,
        );
        let op = FieldSLL64::new(word, 0x1000, true, false);
        op.execute(&mut cpu, &mut ());
        assert_eq!(cpu.field_regs[3], [0, 0xabcd_ef01_2345_6789, 0, 0]);
    }
}
