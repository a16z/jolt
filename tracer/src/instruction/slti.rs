...
pub struct SLTI<const WORD_SIZE: usize> {
    pub address: u64,
...
impl<const WORD_SIZE: usize> RISCVInstruction for SLTI<WORD_SIZE> {
    ...
    fn execute(&self, cpu: &mut Cpu, _: &mut Self::RAMAccess) {
        cpu.x[self.operands.rd] = match cpu.x[self.operands.rs1] < self.operands.imm {
            true => 1,
            false => 0,
        };
    }
