...
const coordinator=artifact('SpartanLeafCoordinator');
const selector=Buffer.from(coordinator.evm.methodIdentifiers['checkIncompleteSparseLeaves(bytes,bytes,bytes,bytes,uint256[4][2])'],'hex');
...
async function configured(f){const key=digest(f.key).toString('hex');if(!configs.has(key))configs.set(key,await deploy('SpartanLeafCoordinator',Buffer.concat([digest(f.key),digest(f.setup),...modules.map(addressWord)])));return configs.get(key);}
const records=[];
...
for(const name of ['toy','empty','empty-public']){
 const f=fixture(name),native=JSON.parse(fs.readFileSync('test/spartan/'+name+'/leaves.json'));
 assert.equal(native.accepted,true);assert.equal(native.witness_mutations_rejected,true);const result=await run(name,f);const root=Number(at(result,0));
 assert.equal(result.subarray(root+32,root+64).toString('hex'),native.state,name+' native complete sparse transcript');
 if(name==='toy')honestLeaf=Buffer.from(leafData);
}
assert.equal(JSON.parse(fs.readFileSync('test/spartan/zero-products/leaves.json')).accepted,false);
...
// Final witness value and PCS remain outside this incomplete gate.
{const f=clone(toy);const offset=slabs(f)[2].end;le((little(f.proof,offset,32)+1n)%21888242871839275222246405745257275088548364400416034343698204186575808495617n).copy(f.proof,offset);await run('witness-product-still-pending',f);}
{const f=clone(toy);const w=BigInt(little(f.key,108)).toString(2).length-1;const opening=slabs(f)[2].end+32;f.setup.subarray(64,96).copy(f.proof,opening+32*(w-1)+96*w);await run('witness-pairing-still-pending',f);}
...
await deploy('SpartanLeafCoordinator',Buffer.concat([digest(toy.key),digest(toy.setup),addressWord(modules[1]),addressWord(modules[0]),addressWord(modules[2]),addressWord(modules[3])]),true);
...
fs.mkdirSync('evidence/leaves',{recursive:true});fs.writeFileSync('evidence/leaves/honest-leaf-calldata.hex',honestLeaf.toString('hex')+'\n');fs.writeFileSync('evidence/leaves/results.json',JSON.stringify({scope:'same-call fixed-code modules through three sparse PCS and leaf equations; final witness product/PCS pending',settings:build.settings,node:process.version,codeHashes:build.hashes,sourceHashes:Object.fromEntries(Object.entries(build.sources).map(([name,v])=>[name,hash(Buffer.from(v.content))])),runtimeBytes:Object.fromEntries([...build.names,'SpartanLeafCoordinator'].map(n=>[n,artifact(n).evm.deployedBytecode.object.length/2])),deployments,records,gasLimit:gasLimit.toString(),warmness:'Each call resets journal; precompiles05/06/07/08/09 explicitly warm; module accounts initially cold, EXTCODEHASH warms them before STATICCALL',gasScope:'actual EVM CREATE/code-deposit and call execution; excludes transaction intrinsic/calldata gas, no chain deployment'},null,2)+'\n');
