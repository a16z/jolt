...
#[cfg(test)]
mod tests {
    #![expect(clippy::panic, reason = "template drift tests should fail loudly")]

    use super::{GUEST_LIB, HOST_MAIN};

    const QUICKSTART: &str = include_str!("../book/src/usage/quickstart.md");
    const HOSTS: &str = include_str!("../book/src/usage/guests_hosts/hosts.md");

    fn normalized(source: &str) -> String {
        source.replace("\r\n", "\n").trim_matches('\n').to_owned()
    }

    fn rust_block_after<'a>(markdown: &'a str, marker: &str) -> &'a str {
        let Some(marker_start) = markdown.find(marker) else {
            panic!("marker not found in markdown: {marker}");
        };
        let after_marker = &markdown[marker_start + marker.len()..];
        let Some(fence_start) = after_marker.find("```rust") else {
            panic!("rust code fence not found after marker: {marker}");
        };
        let code = &after_marker[fence_start + "```rust".len()..];
        let Some(fence_end) = code.find("```") else {
            panic!("closing code fence not found after marker: {marker}");
        };
        &code[..fence_end]
    }

    #[test]
    fn quickstart_guest_snippet_tracks_scaffold_template() {
        let snippet = rust_block_after(
            QUICKSTART,
            "We can view the guest code in `guest/src/lib.rs`.",
        );
        assert_eq!(normalized(snippet), normalized(GUEST_LIB));
    }

    #[test]
    fn quickstart_host_snippet_tracks_scaffold_template() {
        let snippet = rust_block_after(QUICKSTART, "Next let's take a look at the host code");
        assert_eq!(normalized(snippet), normalized(HOST_MAIN));
    }

    #[test]
    fn hosts_guide_documents_current_verifier_preprocess_signature() {
        assert!(
            HOSTS.contains(
                "`preprocess_prover_sha3(shared)` and \
                 `preprocess_verifier_sha3(shared, verifier_setup, blindfold_setup)`"
            ),
            "hosts guide must document the current verifier preprocessing arguments"
        );
        assert!(
            HOSTS.contains(
                "guest::preprocess_verifier_sha3(shared_preprocessing, verifier_setup, None)"
            ),
            "hosts guide example must pass the non-ZK BlindFold setup argument"
        );
    }
}
