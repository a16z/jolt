...
"#;
    ...
    #[test]
    fn wasm_verifier_always_forwards_optional_trusted_advice() {
        let generated = wasm_verify_source(&["fib".to_string()]);
    ...
    #[test]
    fn guest_source_discovers_top_level_provable_functions() {
        ...
        assert_eq!(functions, vec!["fib", "merkle"]);
    }

    #[test]
    fn index_html_always_passes_trusted_advice_bytes() {
        let html = index_html_source(&["fib".to_string()], "sample_project");
        assert!(html.contains("trustedAdviceFile_fib"));
        assert!(html.contains("verify_fib(ppData, proofData, ioData, trustedData)"));
    }

    #[test]
    fn wasm_preprocess_helper_uses_project_guest_apis() {
        let generated = wasm_preprocess_source(&["fib".to_string()]);
        syn::parse_file(&generated).expect("generated preprocessing helper must parse");
        assert!(generated.contains("guest::compile_fib"));
        assert!(generated.contains("guest::preprocess_shared_fib"));
        assert!(generated.contains("guest::preprocess_prover_fib"));
        assert!(generated.contains("guest::verifier_preprocessing_from_prover_fib"));
        assert!(generated.contains("preprocessed_fib_verifier.bin"));
    }

    #[test]
    fn modify_cargo_toml_sets_wasm_lib_path() {
        let dir = std::env::temp_dir().join(format!(
            "jolt-build-wasm-toml-{}",
        ...
        fs::write(dir.join("Cargo.toml"), host_cargo_toml()).unwrap();
        fs::write(dir.join("guest/Cargo.toml"), GUEST_CARGO_TOML).unwrap();

        modify_cargo_toml(dir.to_str().unwrap()).unwrap();
