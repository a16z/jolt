...
mod test {
  ...
  use ark_bn254::{Bn254, Fr, Fq, G1Affine, G1Projective};
  ...
  fn quotient_construction() {
    ...
    let multilinear_f = DensePolynomial::new((0..N).into_iter().map(|_| Fr::rand(&mut rng)).collect::<Vec<_>>());
    let u_challenge = (0..log_N).into_iter().map(|_| Fr::rand(&mut rng)).collect::<Vec<_>>();
    let v_evaluation = multilinear_f.evaluate(&u_challenge);
    
    // Compute multilinear quotients `qₖ(𝑋₀, …, 𝑋ₖ₋₁)`
    let quotients = Zeromorph::<N, Bn254>::compute_multilinear_quotients(&multilinear_f, &u_challenge);
    ...
    let z_challenge = (0..log_N).into_iter().map(|_| Fr::rand(&mut rng)).collect::<Vec<_>>();
  ...
  fn batched_lifted_degree_quotient() {
    ...
    let data_2 = vec![ Fr::from(4u64), Fr::from(5u64), Fr::from(6u64), Fr::from(7u64)];
    ...
    let batched_quotient = Zeromorph::<N, Bn254>::compute_batched_lifted_degree_quotient(&quotients, &y_challenge);
    ...
    let data_0_lifted = vec![Fr::zero(), Fr::zero(), Fr::zero(), Fr::zero(), Fr::zero(), Fr::zero(), Fr::zero(), Fr::one()];
    let data_1_lifted = vec![Fr::zero(), Fr::zero(), Fr::zero(), Fr::zero(), Fr::zero(), Fr::zero(), Fr::from(2u64), Fr::from(3u64)];
    let data_2_lifted = vec![Fr::zero(), Fr::zero(), Fr::zero(), Fr::zero(), Fr::from(4u64), Fr::from(5u64), Fr::from(6u64), Fr::from(7u64)];
  ...
  fn partially_evaluated_quotient_zeta() {
    ...
    let zeta_x = Zeromorph::<N, Bn254>::compute_partially_evaluated_degree_check_polynomial(&batched_quotient, &quotients, &y_challenge, &x_challenge);
    ...
    for i in 0..zeta_x_expected.len() {
      zeta_x_expected[i] += q_1[i] * (-y_challenge * x_challenge.pow(BigInt::<1>::from((N - 1 - 1) as u64)));
    }

    for i in 0..zeta_x_expected.len() {
      zeta_x_expected[i] += q_1[i] * (-y_challenge * y_challenge * x_challenge.pow(BigInt::<1>::from((N - 3 - 1) as u64)));
    }
  ...
  fn phi_n_x_evaluation() {
    ...
    let efficient = (x_challenge.pow(BigInt::<1>::from((1 << log_N) as u64)) - Fr::one()) / (x_challenge - Fr::one());
  ...
  fn phi_n_k_1_x_evaluation() {
    ...
    //(𝑥²^ⁿ − 1) / (𝑥²^ᵏ⁺¹ − 1)
    let efficient = (x_challenge.pow(BigInt::<1>::from((1 << log_N) as u64)) - Fr::one()) / (x_pow - Fr::one()); 
  ...
  fn partially_evaluated_quotient_z_x() {
    ...
    // Construct a random multilinear polynomial f, and (u,v) such that f(u) = v.
    let mut rng = test_rng();            
    let multilinear_f = (0..N).into_iter().map(|_| Fr::rand(&mut rng)).collect::<Vec<_>>();
    let mut multilinear_g = (0..N).into_iter().map(|_| Fr::rand(&mut rng)).collect::<Vec<_>>();
    ...
    let q_0 = UniPoly::from_coeff((0..(1 << 0)).into_iter().map(|_| Fr::rand(&mut rng)).collect::<Vec<_>>());
    let q_1 = UniPoly::from_coeff((0..(1 << 1)).into_iter().map(|_| Fr::rand(&mut rng)).collect::<Vec<_>>());
    let q_2 = UniPoly::from_coeff((0..(1 << 2)).into_iter().map(|_| Fr::rand(&mut rng)).collect::<Vec<_>>());
    let quotients = vec![q_0.clone(), q_1.clone(), q_2.clone()];

    let x_challenge = Fr::rand(&mut rng);

    // Construct Z_x using the prover method
    let Z_x = Zeromorph::<N,Bn254>::compute_partially_evaluated_zeromorph_identity_polynomial(&f_batched, &g_batched, &quotients, &v_evaluation, &u_challenge, &x_challenge);
    ...
    Z_x_expected[0] = Z_x_expected[0] - v_batched * x_challenge * &phi::<Bn254>(&x_challenge, log_N);
    ...
    for k in 0..log_N {
      ...
      // x^{2^k} * \Phi_{n-k-1}(x^{2^{k+1}}) - u_k *  \Phi_{n-k}(x^{2^k})
      let mut scalar = x_pow_2k * &phi::<Bn254>(&x_pow_2kp1, log_N - k - 1) - u_challenge[k] * &phi::<Bn254>(&x_pow_2k, log_N  - k);
      scalar *= x_challenge;
      scalar *= Fr::from(-1);
      for i in 0..Z_x_expected.len() {
