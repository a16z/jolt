...
use ark_ec::short_weierstrass::{SWCurveConfig, Affine};
use ark_ec::{CurveConfig, Group, CurveGroup};
...
impl<const N: usize, C: CurveGroup + PrimeField> Zeromorph<N, C> {
    ...
    fn compute_multilinear_quotients(polynimial: DensePolynomial<C::ScalarField>, u_challenge: &[C::ScalarField] ) -> Vec<DensePolynomial<C::ScalarField>> {
        ...
        let q = DensePolynomial::new(
            (0..size_q).fold(Vec::new(), |mut acc, l| {
    ...
    fn compute_batched_lifted_degree_quotient(quotients: Vec<DensePolynomial<C::ScalarField>>, y_challenge: C::ScalarField) -> DensePolynomial<C::ScalarField> {
...
mod test {
    ...
    fn phi<C: CurveGroup>(challenge: C::BaseField, subscript: usize) -> C::BaseField {
        ...
        (0..len).into_iter().fold(C::BaseField::zero(),|mut acc, i| { 
