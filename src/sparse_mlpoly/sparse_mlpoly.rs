...
impl<G: CurveGroup> SparsePolyCommitmentGens<G> {
  pub fn new(
    ...
    // E_1, ..., E_c
    // log_2(cs)
    let num_vars_derefs = (k * c * s).next_power_of_two().log_2();
...
#[derive(Debug, CanonicalSerialize, CanonicalDeserialize)]
struct PrimarySumcheck<G: CurveGroup, const C: usize, const K: usize>
where
  [(); K * C]:,
{
  ...
  [(); K * C]:,
{
  ...
  primary_sumcheck: PrimarySumcheck<G, C, K>,
  ...
  [(); K * C]:,
{
  ...
  /// - `claimed_eval`: Claimed evaluation of \widetilde{M}(r = (r_1, ..., r_logM))
  ...
  pub fn prove(
    ...
    claimed_eval: &G::ScalarField, // a evaluation of \widetilde{M}(r = (r_1, ..., r_logM))
    ...
    // Special case for Lasso
    // TOOD(moodlezoup): make generic
    let eq_evals: [Vec<G::ScalarField>; K * C] =
      std::array::from_fn(|i| EqPolynomial::new(r[i].clone()).evals());
    assert_eq!(K, 1);
    let materialize_subtable = |i| {
      let subtable: &Vec<G::ScalarField> = &eq_evals[i];
      subtable.clone()
    };

    let subtable_evaluations = SubtableEvaluations::<G::ScalarField, C, K>::new(
      &materialize_subtable,
      &dense.dim_usize,
      dense.s,
    );
    ...
    let comm_derefs = {
      let comm = subtable_evaluations.commit(&gens.gens_derefs);
      comm.append_to_transcript(b"comm_poly_row_col_ops_val", transcript);
      comm
    };

    let scalar_product_operands = subtable_evaluations.subtable_lookup_polys.clone();
    let scalar_product: GeneralizedScalarProduct<G::ScalarField, C, K> =
      GeneralizedScalarProduct::new(scalar_product_operands.clone());
    // TODO(moodlezoup): \tilde{eq}(r,k)
    let multiply_all = |vals: [G::ScalarField; K * C]| vals.iter().product();
    let eval_scalar_product = scalar_product.evaluate(&multiply_all);
    ...
    assert_eq!(eval_scalar_product, *claimed_eval);

    let (primary_sumcheck_proof, r_z, _) =
      SumcheckInstanceProof::<G::ScalarField>::prove_arbitrary::<_, G, Transcript>(
        &eval_scalar_product,
        dense.s.log_2(),
        &mut scalar_product_operands.to_vec(),
        |vals| -> G::ScalarField { vals.iter().product() }, // TODO(moodlezoup): replace with function parameter g
        scalar_product_operands.len(),
        transcript,
      );
    ...
    let eval_derefs: [G::ScalarField; K * C] =
      std::array::from_fn(|i| subtable_evaluations.subtable_lookup_polys[i].evaluate(&r_z));
...
mod tests {
  ...
  fn check_evaluation_helper<G: CurveGroup>() {
    ...
    let gens = SparsePolyCommitmentGens::<G>::new(b"gens_sparse_poly", c, k, s, log_m);
  ...
  fn prove_3d() {
    ...
    let gens = SparsePolyCommitmentGens::<G1Projective>::new(b"gens_sparse_poly", C, K, s, log_M);
    ...
    let mut prover_transcript = Transcript::new(b"example");
    let proof = SparsePolynomialEvaluationProof::<G1Projective, C, K>::prove(
      &mut dense,
      &r,
      &eval,
      &gens,
  ...
  fn evaluate_over_known_indices() {
    ...
    let (_, _, _, log_m, lookup_matrix) = construct_2d_small::<G1Projective>();
  ...
  fn prove_2d() {
    ...
    let (_, s, _, log_m, lookup_matrix) = construct_2d_small::<G1Projective>();
