...
use tracer::instruction::{advice_sd::AdviceSD, RISCVCycle};

use crate::zkvm::lookup_table::{range_check::RangeCheckTable, LookupTables};

use super::{CircuitFlags, Flags, InstructionLookup, LookupQuery, NUM_CIRCUIT_FLAGS};

impl<const XLEN: usize> InstructionLookup<XLEN> for AdviceSD {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(RangeCheckTable.into())
    }
}

impl Flags for AdviceSD {
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<AdviceSD> {
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, AdviceSD>();
    }
}
