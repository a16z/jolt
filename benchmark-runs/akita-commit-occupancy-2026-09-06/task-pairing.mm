...
int main(int argc, const char **argv) {
    require(argc == 5, "usage: pairing production.metal capture-directory map.u32le archive-prefix");
    @autoreleasepool {
        NSError *error = nil;
        ...
        body = [body stringByReplacingOccurrencesOfString:@"akita_packed_onehot_commit_fp128_d128_rank3"
            withString:@"diagnostic_paired_commit"];
        ...
        const NSArray<NSString *> *names = @[@"akita_packed_onehot_commit_fp128_d128_rank3", @"diagnostic_paired_commit"];
        ...
        for (unsigned variant = 0; variant < 2; ++variant) {
            ...
            if (variant) small.task_mapping = [device newBufferWithBytes:small_map length:sizeof(small_map)
        ...
        for (unsigned variant = 0; variant < 2; ++variant) {
            target.task_mapping = variant ? mapping_buffer : nil;
        ...
        std::printf("PAIRING_COMPLETE target_observations=4 parity=pass active_coefficients=%llu\n",
            (unsigned long long)reference.size());
    }
}
