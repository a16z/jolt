...
def census(pairing=False, fragments=False):
    capture = ROOT / "runs/d2-capture"
    output = ROOT / ("runs/d4-fragments.out" if fragments else "runs/d3-pairing-price.out" if pairing else "runs/d2-census.out")
    metadata = json.loads((capture / "metadata.json").read_text())
    command = ["/usr/bin/time", "-l", str(ROOT / ("bin/selector-fragments" if fragments else "bin/selector-pairing" if pairing else "bin/selector-census")),
    ...
    try:
        ...
        record("d4_fragments_complete" if fragments else "d3_pairing_price_complete" if pairing else "d2_census_complete",
               raw_sha256=hashlib.sha256(output.read_bytes()).hexdigest())
    except BaseException as error:
        record("d4_fragments_failure" if fragments else "d3_pairing_price_failure" if pairing else "d2_census_failure", reason=str(error))
        raise
    finally:
        LOCK.rmdir()
...
if __name__ == "__main__":
    ...
    census(args.price_pairing, args.fragments) if args.census or args.price_pairing or args.fragments else main()
