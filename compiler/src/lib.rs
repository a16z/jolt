...
pub fn compile_example(example_name: &str) {
    ...
    if !cargo_build_status.status.success() {
        println!(
            "Failed to build example: {}. Error: {}",
            example_name,
            String::from_utf8_lossy(&cargo_build_status.stderr)
        );
        std::process::exit(1);
    }
    ...
    let device_destination = JoltPaths::jolt_device_path(example_name);
    match run_tracer_with_paths(
        elf_location,
        trace_destination,
        ...
        Err(e) => {
            println!(
                "Failed to run tracer on example: {}. Error: {}",
                example_name, e
            );
            std::process::exit(1);
        }
