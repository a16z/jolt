...
impl PackedLinearLayout for TestLayout {
    ...
    fn rank(&self, address: PackedLinearAddress) -> Result<usize, OpeningsError> {
        if address.family != FAMILY || address.limb != 0 || address.row >= 4 || address.symbol >= 2
        {
...
fn packed_view_eval(poly: &Polynomial<Fr>, row_point: &[Fr], terms: &[PackedLinearTerm<Fr>]) -> Fr {
    ...
    terms.iter().fold(fr(0), |acc, term| {
        acc + row_weights
            ...
            .copied()
            .enumerate()
            .fold(fr(0), |term_acc, (row, row_weight)| {
                term_acc + term.coefficient * row_weight * poly.evaluations()[row * 2 + term.symbol]
            })
    })
}
