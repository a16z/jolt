...
fn main() {
    ...
    // baseline 19 to 29 so Spartan's outer uniskip actually samples the
    // FADD/FSUB/FMUL/FINV gates (matrix rows 19-26) plus the FMov-I2F /
    // FMov-F2I security gates (rows 27-28). This intentionally diverges
...
struct Polys {
    ...
    op_flags: Vec<PolynomialId>, // [0..NUM_CIRCUIT_FLAGS) — 20 for FR module

    // BN254 Fr FieldOp R1CS columns (matrix vars 42, 43, 44).
    ...
    // FMov limb-binding R1CS columns (matrix vars 45, 46).
    // Referenced by GroupSplitR1CSEval for security-fix rows 27-28.
    field_reg_read_limb: PolynomialId,
    field_reg_write_limb: PolynomialId,
    ...
    // FieldReg coprocessor (Phase 2 addition).
    //
    // `field_reg_read_value` / `field_reg_write_value` are Committed; they
    // carry the per-cycle Fr coprocessor read/write trace and are opened at
    // rw_cycle via CollectOpeningClaimAt in the FieldReg Twist stage.
    //
    // `field_reg_inc` / `field_reg_ra` / `field_reg_val` / `field_reg_eq_cycle`
    // are Virtual — the Twist sumcheck consumes them without PCS commitments.
    // Committing them is Phase 1 / Phase 1f work (segmented-reduce final-bind
    // point reconciliation) not yet resolved.
    field_reg_inc: PolynomialId,
    field_reg_ra: PolynomialId,
...
fn register_polys(pt: &mut PolyTable, p: &ModuleParams) -> Polys {
    ...
    use PolynomialId::{
        ...
        FieldOpOperandB, FieldOpResultValue, FieldRegReadLimb, FieldRegWriteLimb, HammingG,
        HammingWeight, Imm, InstructionRa,
    ...
    // FieldReg-extended modules include six additional circuit flags
    // (IsFieldMul/Add/Sub/Inv at CircuitFlags indices 14-17, IsFMovI2F/F2I at
    // 18-19) so the outer Spartan remaining sumcheck outputs their evaluations
    // for the GroupSplitR1CSEval reconstruction of rows 19-28.
    let op_flags: Vec<_> = (0..20)
        .map(|i| pt.add(OpFlag(i), &format!("OpFlag_{i}"), Virtual, p.log_t))
        .collect();

    // BN254 Fr FieldOp operand/result columns (R1CS witness vars 42, 43, 44).
    ...
    // FMov limb-binding columns (R1CS witness vars 45, 46).
    let field_reg_read_limb = pt.add(FieldRegReadLimb, "FieldRegReadLimb", Virtual, p.log_t);
    let field_reg_write_limb = pt.add(FieldRegWriteLimb, "FieldRegWriteLimb", Virtual, p.log_t);
    ...
    // FieldReg coprocessor polynomials (Phase 2). ReadValue / WriteValue are
    // committed at log_t and opened at rw_cycle_challenges via
    // CollectOpeningClaimAt inside the FieldReg Twist stage. Others are
    // Virtual inputs to that stage's 2-phase sumcheck.
    let field_reg_inc = pt.add(
        PolynomialId::FieldRegInc,
        "FieldRegInc",
        Virtual,
        p.log_t,
    );
    let field_reg_ra = pt.add(
        PolynomialId::FieldRegRa,
        "FieldRegRa",
        Virtual,
        p.log_k_chunk + p.log_t,
    );
    let field_reg_val = pt.add(
        PolynomialId::FieldRegVal,
        "FieldRegVal",
        Virtual,
        p.log_k_chunk + p.log_t,
    );
    let field_reg_read_value = pt.add_committed(
    ...
    let field_reg_write_value = pt.add_committed(
...
fn r1cs_input_polys(p: &Polys) -> [PolynomialId; NUM_R1CS_INPUTS_FR] {
    [
        ...
        p.op_flags[18],            // 39: OpFlags(IsFMovI2F)
        p.op_flags[19],            // 40: OpFlags(IsFMovF2I)
        p.field_op_a,              // 41: FieldOpOperandA
        p.field_op_b,              // 42: FieldOpOperandB
...
fn build_module(params: &ModuleParams) -> Module {
    ...
    //
    // NOTE: FieldReg Twist sumcheck stage is NOT YET added here. Adding a
    // new BeginStage requires reconciling the prover/verifier stage-count
    // asymmetry (8 prover BeginStages → 4 verifier BeginStages in the base
    // module; the mapping is many-to-one and would need untangling before a
    // new FR sumcheck stage can slot in cleanly). For now, RV/WV are
    // committed but the Twist identity is not yet enforced through the main
    // protocol; the standalone Twist in
    // `crates/jolt-equivalence/tests/field_register_twist_standalone.rs`
    // demonstrates the sumcheck side in isolation.
...
fn build_commitment_phase(p: &Polys, params: &ModuleParams, ops: &mut Vec<Op>) {
    ...
    // Barrier 4: FieldReg coprocessor ReadValue / WriteValue (Phase 2). Both
    // committed at log_t, padded to log_k_chunk + log_t for the main witness
    // grid. Absorbed into the transcript before Stage 1 so downstream stages
    // can reference their commitments.
    ops.push(Op::Commit {
        polys: vec![p.field_reg_read_value, p.field_reg_write_value],
        tag: DomainSeparator::Commitment,
        num_vars: params.log_k_chunk + params.log_t,
    });
}
...
fn build_stage1(
    ...
    // module widens to 29 rows by appending the FADD/FSUB/FMUL/FINV gates
...
fn build_verifier_stage1_ops(
    ...
    // Barrier 4: FieldReg ReadValue / WriteValue (Phase 2)
    commit_pairs.push((p.field_reg_read_value, DomainSeparator::Commitment));
    commit_pairs.push((p.field_reg_write_value, DomainSeparator::Commitment));
    ...
    // FieldReg module widens from the baseline 10+9 to 15+14 so Spartan's
...
fn build_stage2(
    ...
    // γ_fr (FieldReg Twist mixing challenge)
    let ch_gamma_fr = ch.add("gamma_fr", ChallengeSource::FiatShamir { after_stage: 1 });
    ops.push(Op::Squeeze {
    ...
    //
    // stage1_cycle_challenges is BE (reversed from declaration order).
    // LowToHigh bind pairs the low bit of the index with the first challenge
    // passed, so iterate in LE order (reverse of the BE-stored list).
    ops.push(Op::Materialize {
        binding: InputBinding::Provided {
            poly: p.field_reg_read_value,
        },
    });
    ops.push(Op::Materialize {
        binding: InputBinding::Provided {
            poly: p.field_reg_write_value,
        },
    });
    for &c in stage1_cycle_challenges.iter().rev() {
        ops.push(Op::Bind {
            polys: vec![p.field_reg_read_value, p.field_reg_write_value],
            challenge: c,
            order: BindingOrder::LowToHigh,
    ...
    // [5] FieldReg Twist: field_reg_read_value + γ_fr * field_reg_write_value
    let fr_input_claim = ClaimFormula {
        terms: vec![
            ClaimTerm {
                coeff: 1,
                factors: vec![ClaimFactor::Eval(p.field_reg_read_value)],
            },
            ClaimTerm {
                coeff: 1,
                factors: vec![
                    ClaimFactor::Challenge(ch_gamma_fr),
                    ClaimFactor::Eval(p.field_reg_write_value),
                ],
            },
        ],
    };
    ops.push(Op::AbsorbInputClaim {
        formula: fr_input_claim.clone(),
        tag: DomainSeparator::SumcheckClaim,
        batch: batch_idx,
    ...
    // FieldReg Twist externals (phase 1→2 scalar captures).
    ...
    // [5] FieldReg Twist — 2-phase mirror of RamRW, K = 1<<log_k_chunk.
    //
    // Phase 1 (cycle, log_t rounds, degree 3, segmented+Gruen):
    //   eq_cycle·ra·val + γ_fr·eq_cycle·ra·val + γ_fr·eq_cycle·ra·inc
    //   Inputs: eq_cycle(T) inner, ra(K×T), val(K×T), inc(T) inner.
    //
    // Phase 2 (address, log_k_chunk rounds, degree 2):
    //   eq_bound·ra·val + γ_fr·eq_bound·ra·val + γ_fr·eq_bound·inc_bound·ra
    //   Inputs: ra(K), val(K) post phase-1 binding.
    let fr_phase1_kernel_idx = kernels.len();
    kernels.push(KernelDef {
        spec: KernelSpec {
            formula: Formula::from_terms(vec![
                ProductTerm {
                ...
                ProductTerm {
                    coefficient: 1,
                    factors: vec![
                        Factor::Challenge(ch_gamma_fr.0 as u32),
                        Factor::Input(0),
                        Factor::Input(1),
                        Factor::Input(2),
                    ],
            ...
            num_evals: params.fr_checking_degree + 1,
            iteration: Iteration::Dense,
            binding_order: BindingOrder::LowToHigh,
            gruen_hint: Some(GruenHint {
                eq_input: 0,
                ...
                q_lincombo: LinComboQ {
                    ...
                    b_input: 2,
                    c_input: 3,
                    gamma_challenge: ch_gamma_fr,
                },
            }),
        },
        inputs: vec![
            InputBinding::EqTable {
                poly: p.field_reg_eq_cycle,
                challenges: rw_cycle_eq_challenges.clone(),
            },
            InputBinding::Provided {
                poly: p.field_reg_ra,
    ...
    let fr_phase2_kernel_idx = kernels.len();
    kernels.push(KernelDef {
        spec: KernelSpec {
            formula: Formula::from_terms(vec![
                ProductTerm {
    ...
    batched_sumchecks.push(BatchedSumcheckDef {
        instances: vec![
            ...
            // [5] FieldReg Twist — 2 phases: cycle (log_t) + address (log_k_chunk).
            BatchedInstance {
                phases: vec![
                    InstancePhase {
                        kernel: fr_phase1_kernel_idx,
                        num_rounds: params.log_t,
                        scalar_captures: vec![],
                        segmented: Some(SegmentedConfig {
                            inner_num_vars: params.log_t,
                            outer_num_vars: params.log_k_chunk,
...
fn build_verifier_stage2_ops(
    ...
    // τ_high is at s2_ch_base, r0 at s2_ch_base+1, then γ_rw, γ_instruction,
    // r_address, γ_fr.
    ...
    // Evaluation list positions in the deduplicated flush layout.
    // Used by StageEval in output_check formulas.
    // sp.evals[0] = product_uniskip_eval (recorded before the batch).
    // sp.evals[1] = field_reg_read_value, sp.evals[2] = field_reg_write_value
    //   (recorded pre-sumcheck for FR input_claim to reference).
    // sp.evals[3..21] = the 18 unique batched-sumcheck final-bind evals.
    //
    ...
    // Instance 5: FieldReg Twist
    //
    // input_claim = field_reg_read_value + γ_fr * field_reg_write_value
    // output_check = eq(r_cycle, r') * ra * ((1+γ_fr)*val + γ_fr*inc)
    //   Expanded: eq*ra*val + γ_fr*eq*ra*val + γ_fr*eq*ra*inc
    //
    // normalize: Segments { sizes: [log_t, log_k_chunk], output_order: [1, 0] }
    //   Active rounds (last log_t + log_k_chunk) have:
    //     [phase1_cycle(log_t), phase2_addr(log_k_chunk)]
    //   After normalize: [r_addr_BE(log_k_chunk), r_cycle_BE(log_t)]
    let fr_eq = ClaimFactor::EqEvalSlice {
        challenges: stage1_cycle_challenges.clone(),
        at_stage: VerifierStageIndex(1),
        offset: params.log_k_chunk, // cycle starts after address portion
    };
    let fr_input_claim = ClaimFormula {
        terms: vec![
            ClaimTerm {
    ...
    let fr_output_check = ClaimFormula {
        terms: vec![
            ClaimTerm {
                coeff: 1,
                factors: vec![
                    fr_eq.clone(),
                    ClaimFactor::StageEval(SE_FR_RA),
                    ClaimFactor::StageEval(SE_FR_VAL),
                ],
            },
            ClaimTerm {
                coeff: 1,
            ...
            ClaimTerm {
                ...
                factors: vec![
                    ...
                    fr_eq,
                    ClaimFactor::StageEval(SE_FR_RA),
                    ClaimFactor::StageEval(SE_FR_INC),
                ],
            },
        ],
    };
    ...
    let instances = vec![
        ...
        // [5] FieldReg Twist — (log_t + log_k_chunk) rounds, degree 3
        SumcheckInstance {
            input_claim: fr_input_claim,
            output_check: fr_output_check,
            num_rounds: params.fr_checking_rounds,
            degree: params.fr_checking_degree,
            normalize: Some(PointNormalization::Segments {
                sizes: vec![params.log_t, params.log_k_chunk],
                output_order: vec![1, 0],
            }),
        },
    ...
    // γ_fr (FieldReg mixing challenge)
    ops.push(VerifierOp::Squeeze {
        challenge: ch_gamma_fr,
    });
    ...
    let stage2_round_base = ch_batch_base + STAGE2_NUM_INSTANCES_FR;
