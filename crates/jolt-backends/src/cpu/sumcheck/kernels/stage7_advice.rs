...
use jolt_claims::protocols::jolt::formulas::{
    claim_reductions::advice::{advice_cycle_phase_dummy_scale, advice_index_to_address_cycle},
    ...
    N: jolt_witness::WitnessNamespace,
{
    ...
    let mut state = SumcheckStage7AdviceAddressState::new(
        ...
        advice,
        Polynomial::from(eq),
        advice_cycle_phase_dummy_scale::<F>(request.dummy_cycle_phase_rounds),
    );
...
fn compare_address_cycle(
    ...
    let trace_order = TracePolynomialOrder::from(trace_order);
