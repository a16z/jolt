    ...
    <F as WithAccumulator>::Accumulator: RingAccumulator<Element = F>,
{
    ...
    pub fn bind(
        ...
        rayon::scope(|scope| {
            scope.spawn(|_| {
                stage_eq.iter_mut().for_each(|poly| poly.bind(challenge));
            });
            scope.spawn(|_| {
                extra_stage_values
                    .par_iter_mut()
                ...
                for x_in in 0..eq_layout.e_in_current_len() {
                    ...
                    for (point_index, &point) in points[..degree].iter().enumerate() {
                        ...
                        for &(pair, coefficient) in
                            extra_pairs[..cycle_value.extra_stage_values.len()].iter()
                        {
        ...
        let evals = (0..rows)
            ...
            .map(|row| {
                ...
                for point in 0..=degree {
                    ...
                    for stage in 0..BYTECODE_STAGES {
                        let [eq0, eq1] =
                            poly_eval_pair(&stage_eq[stage], row, BindingOrder::HighToLow);
