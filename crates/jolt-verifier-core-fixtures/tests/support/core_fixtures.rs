...
use jolt_core_verifier_bridge::{
    proof_into_verifier, verifier_preprocessing_from_core, CorePcsBridge, ImportedCoreProof,
};
...
impl CorePrecompatVerifierCase {
    ...
    pub fn verify_after_compat(&self) -> Result<(), VerifierError> {
        let proof = proof_into_verifier(self.proof.clone_via_bytes())?;
        verify::<Fr, DoryScheme, Pedersen<Bn254G1>, Blake2bTranscript>(
            &self.preprocessing,
            &self.public_io,
            &proof,
...
fn zk_case_from_parts(fixture: GeneratedCoreFixture) -> CoreZkVerifierCase {
    CoreZkVerifierCase {
        preprocessing: verifier_preprocessing_from_core(&fixture.core_preprocessing),
        public_io: fixture.public_io,
        proof: proof_into_verifier(fixture.proof).expect("convert ZK core proof"),
    }
}
...
fn case_from_parts(fixture: GeneratedCoreFixture, public_io: JoltDevice) -> CoreVerifierCase {
    CoreVerifierCase {
        preprocessing: verifier_preprocessing_from_core(&fixture.core_preprocessing),
        public_io,
        proof: proof_into_verifier(fixture.proof).expect("convert standard core proof"),
...
fn deserialize_core_commitment(bytes: &[u8]) -> CoreCommitment {
    ArkGT(
        jolt_core::ark_bn254::Fq12::deserialize_compressed(Cursor::new(bytes))
