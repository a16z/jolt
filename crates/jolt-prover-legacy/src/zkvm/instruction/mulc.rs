use tracer::instruction::{mulc::MULC, RISCVCycle};
...
impl<const XLEN: usize> InstructionLookup<XLEN> for MULC {
...
impl Flags for MULC {
    fn circuit_flags(&self) -> [bool; NUM_CIRCUIT_FLAGS] {
        ...
        flags[CircuitFlags::UsesCarry] = true;
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<MULC> {
    fn to_lookup_operands(&self) -> (u64, u128) {
        let (x, y) = LookupQuery::<XLEN>::to_instruction_inputs(self);
        // Unsigned widening product plus carry-in: (2^64-1)^2 + (2^64-1) < 2^128.
        (0, x as u128 * (y as u64 as u128) + self.carry as u128)
    }
    ...
    fn to_lookup_output(&self) -> u64 {
        ...
        x.wrapping_mul(y as u64).wrapping_add(self.carry) & mask
    }
}
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, MULC>();
    }

    #[test]
    fn lookup_output_matches_trace() {
        lookup_output_matches_trace_test::<MULC>();
    }
}
