...
impl<const XLEN: usize> InstructionLookup<XLEN> for VirtualAdviceLen {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(RangeCheckTable.into())
    }
}

impl Flags for VirtualAdviceLen {
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<VirtualAdviceLen> {
    fn to_instruction_inputs(&self) -> (u64, i128) {
        (0, 0)
    }

    fn to_lookup_operands(&self) -> (u64, u128) {
        // Use the value written to rd (after execution)
        match XLEN {
            #[cfg(test)]
    ...
    fn to_lookup_output(&self) -> u64 {
        ...
        match XLEN {
            #[cfg(test)]
            8 => (self.register_state.rd.1 as u8).into(),
            32 => (self.register_state.rd.1 as u32).into(),
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, VirtualAdviceLen>();
    }
}
