...
use tracer::instruction::{virtual_window_mask_h::VirtualWindowMaskH, RISCVCycle};

use crate::zkvm::lookup_table::{window_mask_h::WindowMaskHTable, LookupTables};

use super::{CircuitFlags, Flags, InstructionLookup, LookupQuery, NUM_CIRCUIT_FLAGS};

impl<const XLEN: usize> InstructionLookup<XLEN> for VirtualWindowMaskH {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(WindowMaskHTable.into())
    }
}

impl Flags for VirtualWindowMaskH {
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<VirtualWindowMaskH> {
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, VirtualWindowMaskH>();
    }

    #[test]
    fn lookup_output_matches_trace() {
        lookup_output_matches_trace_test::<VirtualWindowMaskH>();
    }
}
