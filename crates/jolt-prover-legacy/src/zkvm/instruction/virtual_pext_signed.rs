...
impl<const XLEN: usize> InstructionLookup<XLEN> for VirtualPextSigned {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(PextSignedTable.into())
    }
}

impl Flags for VirtualPextSigned {
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<VirtualPextSigned> {
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, VirtualPextSigned>();
    }

    #[test]
    fn lookup_output_matches_trace() {
        lookup_output_matches_trace_test::<VirtualPextSigned>();
    }
}
