...
use crate::zkvm::lookup_table::lane_extract_s::{signed_extract, LaneExtractSTable};
use crate::zkvm::lookup_table::lane_mask::lane_value;
use crate::zkvm::lookup_table::LookupTables;
use tracer::instruction::{
    virtual_align_addr::VirtualAlignAddr, virtual_lane_extract_s::VirtualLaneExtractS,
    virtual_lane_mask_b::VirtualLaneMaskB, virtual_lane_mask_h::VirtualLaneMaskH,
    virtual_lane_mask_w::VirtualLaneMaskW, virtual_pow2_lane::VirtualPow2Lane, RISCVCycle,
};
...
impl<const XLEN: usize> InstructionLookup<XLEN> for VirtualLaneExtractS {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(LaneExtractSTable.into())
    }
}

impl Flags for VirtualLaneExtractS {
    fn circuit_flags(&self) -> [bool; NUM_CIRCUIT_FLAGS] {
        let mut flags = [false; NUM_CIRCUIT_FLAGS];
        flags[CircuitFlags::WriteLookupOutputToRD] = true;
        flags[CircuitFlags::VirtualInstruction] = self.virtual_sequence_remaining.is_some();
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<VirtualLaneExtractS> {
    fn to_instruction_inputs(&self) -> (u64, i128) {
        match XLEN {
            #[cfg(test)]
            8 => (
...
mod tests {
    ...
    instruction_tests!(lane_extract_s, VirtualLaneExtractS);
}
