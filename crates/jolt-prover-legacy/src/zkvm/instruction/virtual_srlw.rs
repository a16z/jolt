...
use tracer::instruction::{virtual_srlw::VirtualSRLW, RISCVCycle};

use crate::zkvm::lookup_table::{virtual_srlw::VirtualSRLWTable, LookupTables};

use super::{CircuitFlags, Flags, InstructionLookup, LookupQuery, NUM_CIRCUIT_FLAGS};

impl<const XLEN: usize> InstructionLookup<XLEN> for VirtualSRLW {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(VirtualSRLWTable.into())
    }
}

impl Flags for VirtualSRLW {
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<VirtualSRLW> {
...
mod tests {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, VirtualSRLW>();
    }

    #[test]
    fn lookup_output_matches_trace() {
        lookup_output_matches_trace_test::<VirtualSRLW>();
    }
}
