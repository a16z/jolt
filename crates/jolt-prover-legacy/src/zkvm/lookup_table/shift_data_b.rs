...
impl<const XLEN: usize> JoltLookupTable for ShiftDataBTable<XLEN> {
...
impl<const XLEN: usize> PrefixSuffixDecomposition<XLEN> for ShiftDataBTable<XLEN> {
    fn suffixes(&self) -> Vec<Suffixes> {
        vec![Suffixes::OffsetScaleB, Suffixes::ShiftDataB]
    }
...
mod test {
    ...
    fn mle_full_hypercube() {
        lookup_table_mle_full_hypercube_test::<Fr, ShiftDataBTable<8>>();
    }

    #[test]
    fn mle_random() {
        lookup_table_mle_random_test::<Fr, ShiftDataBTable<XLEN>>();
    }
    ...
    fn prefix_suffix_small_phases() {
        prefix_suffix_test_with_phase_size::<XLEN, Fr, ShiftDataBTable<XLEN>>(2, 20);
    }
}
