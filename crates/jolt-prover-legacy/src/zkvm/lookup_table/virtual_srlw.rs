...
impl<const XLEN: usize> JoltLookupTable for VirtualSRLWTable<XLEN> {
    fn materialize_entry(&self, index: u128) -> u64 {
        ...
        entry + sign_bit * y_0 * ((1u128 << XLEN) - (1u128 << half)) as u64
    }
        ...
        F: JoltField + FieldChallengeOps<C>,
    {
        ...
        for i in half..XLEN {
            result *= F::one() + r[2 * i + 1];
            result += r[2 * i] * r[2 * i + 1];
        }
...
mod tests {
    ...
    #[test]
    fn mle_random() {
        lookup_table_mle_random_test::<Fr, VirtualSRLWTable<XLEN>>();
    }
    ...
    fn mle_full_hypercube() {
        lookup_table_mle_full_hypercube_test::<Fr, VirtualSRLWTable<8>>();
    }
