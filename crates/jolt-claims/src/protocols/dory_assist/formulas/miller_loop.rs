...
fn double_step<F>(
    state: G2MapState<F>,
) -> (G2MapState<F>, [Fq2Expr<F>; MILLER_LOOP_LINE_COEFFICIENTS])
where
    F: RingCore + FromPrimitiveInt,
{
...
fn line_state<F>(relation: DoryAssistRelationId) -> G2MapState<F>
where
    F: RingCore,
{
    G2MapState {
...
fn line_shifted_state<F>(relation: DoryAssistRelationId) -> G2MapState<F>
where
    F: RingCore,
{
    G2MapState {
