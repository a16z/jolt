...
use jolt_field::{Fr, Ring, CanonicalEncoding};
...
fuzz_target!(|data: &[u8]| {
    ...
    let values: Vec<Fr> = (0..4)
        .map(|i| <Fr as CanonicalEncoding>::from_bytes_le_reduced(&data[i * 32..(i + 1) * 32]))
        .collect();
    let blinding = <Fr as CanonicalEncoding>::from_bytes_le_reduced(&data[128..160]);
