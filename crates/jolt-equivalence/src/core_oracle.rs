...
use crate::checks::{
    ...
    assert_core_stage3_sumcheck_proof_matches_bolt, assert_core_stage5_sumcheck_proof_matches_bolt,
    assert_core_stage6_opening_claims_match_bolt, assert_core_stage6_sumcheck_proof_matches_bolt,
};
...
pub fn core_lookup_table_to_modular_index<const XLEN: usize>(
    table: &CoreLookupTables<XLEN>,
) -> usize {
    match table {
        ...
        CoreLookupTables::And(_) => LookupTableKind::<XLEN>::And(Default::default()).index(),
        CoreLookupTables::Andn(_) => LookupTableKind::<XLEN>::Andn(Default::default()).index(),
        CoreLookupTables::Or(_) => LookupTableKind::<XLEN>::Or(Default::default()).index(),
        CoreLookupTables::Xor(_) => LookupTableKind::<XLEN>::Xor(Default::default()).index(),
        CoreLookupTables::Equal(_) => LookupTableKind::<XLEN>::Equal(Default::default()).index(),
        ...
        CoreLookupTables::Pow2(_) => LookupTableKind::<XLEN>::Pow2(Default::default()).index(),
        CoreLookupTables::Pow2W(_) => LookupTableKind::<XLEN>::Pow2W(Default::default()).index(),
...
fn core_guest_commitment_fixture(
    ...
    let mut trace_backend = tracer::TracerBackend::new();
    let trace_output = core_program
        .trace_with_backend(&mut trace_backend, &inputs, &[], &[])
        .expect("trace guest with program tracer backend");
    let trace = trace_output.trace.into_rows();
    let host_io_device = trace_output.device;
    ...
    padded_trace.resize(proof.trace_length, TraceRow::default());
    ...
    let stage6_entry_bytecode_index = bytecode.entry_bytecode_index().unwrap_or(0);
    ...
    let rv64_cycles = stage1_rv64_cycles(&padded_trace, proof.trace_length, &bytecode);
    let stage3_cycles = stage3_cycles(&padded_trace, proof.trace_length, &bytecode);
    let stage4_register_accesses = stage4_register_accesses(&padded_trace, proof.trace_length);
    let stage5_trace = stage5_lookup_trace(&padded_trace, proof.trace_length, |cycle| {
        InstructionLookup::<XLEN>::lookup_table(&cycle.instruction())
            .map(|table| core_lookup_table_to_modular_index(&table))
    });
...
pub(crate) fn assert_core_accepts_bolt_stage5(
    ...
    assert_core_stage5_sumcheck_proof_matches_bolt(&fixture.proof, &stage5_proof.sumchecks[0]);
