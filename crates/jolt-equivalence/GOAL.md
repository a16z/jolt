...
- Stage 6 bytecode-entry materialization also moved into
  `jolt-kernels::trace`, using the shared `jolt-program` bytecode rows and
  `jolt-riscv` instruction flag helpers. The core oracle now supplies only the
  core lookup-table index callback, keeping bytecode witness row construction
  out of the harness.
