...
use jolt_witness::{WitnessBuilder, WitnessConfig};
use tracer::instruction::{Cycle, RAMAccess};

use crate::witness::bytecode::BytecodePreprocessing;
use crate::witness::cycle_data::trace_to_cycle_data;
use crate::witness::r1cs_inputs::trace_to_witnesses;
use crate::witness::store::WitnessStore;
use crate::witness::store_sink::StoreSink;
...
pub struct WitnessOutput<F: Field> {
    ...
    pub witness_store: WitnessStore<F>,
...
///    [`WitnessBuilder`] → [`StoreSink`] → [`WitnessStore`]
///
...
pub fn generate_witnesses<F: Field>(trace: &[Cycle]) -> WitnessOutput<F> {
    ...
    let mut store = WitnessStore::new();
    {
        ...
        builder.build(&cycle_data, &mut sink);
    }
