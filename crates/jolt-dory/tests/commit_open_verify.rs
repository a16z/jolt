...
/// Generic over the spongefish sponge. jolt-dory is a symmetric consumer:
/// both prover and verifier `absorb` every value through the same adapter, so
/// both sides build an independent `ProverState` over the same domain and
...
fn streaming_zk_commitment_is_blinded_and_verifies() {
    ...
    let mut pt = prover_transcript(b"stream-zk", INSTANCE, Blake2b512::default());
    let (proof, eval_com, _blind) =
        DoryScheme::open_zk(&poly, &point, eval, &prover_setup, hint, &mut pt);

    let mut vt = prover_transcript(b"stream-zk", INSTANCE, Blake2b512::default());
...
fn wrong_eval_rejected() {
    ...
    let mut pt = prover_transcript(b"wrong-eval", INSTANCE, Blake2b512::default());
    let proof = DoryScheme::open(&poly, &point, eval, &prover_setup, Some(hint), &mut pt);

    let tampered_eval = eval + Fr::from_u64(1);
    let mut vt = prover_transcript(b"wrong-eval", INSTANCE, Blake2b512::default());
...
fn wrong_point_rejected() {
    ...
    let mut pt = prover_transcript(b"wrong-point", INSTANCE, Blake2b512::default());
    let proof = DoryScheme::open(&poly, &point, eval, &prover_setup, Some(hint), &mut pt);

    let mut tampered_point = point.clone();
    tampered_point[0] += Fr::from_u64(1);
    let mut vt = prover_transcript(b"wrong-point", INSTANCE, Blake2b512::default());
    let result = DoryScheme::verify(
        &commitment,
        &tampered_point,
        eval,
...
fn wrong_commitment_rejected() {
    ...
    let mut pt = prover_transcript(b"wrong-commit", INSTANCE, Blake2b512::default());
    let proof = DoryScheme::open(&poly, &point, eval, &prover_setup, Some(hint), &mut pt);
    ...
    let mut vt = prover_transcript(b"wrong-commit", INSTANCE, Blake2b512::default());
    let result = DoryScheme::verify(
        &wrong_commitment,
        &point,
        eval,
...
fn wrong_transcript_domain_rejected() {
    ...
    let mut pt = prover_transcript(b"correct-domain", INSTANCE, Blake2b512::default());
    let proof = DoryScheme::open(&poly, &point, eval, &prover_setup, Some(hint), &mut pt);

    let mut vt = prover_transcript(b"wrong-domain", INSTANCE, Blake2b512::default());
...
fn transparent_verify_rejects_zk_opening_proof() {
    ...
    let mut pt = prover_transcript(
        b"zk-proof-transparent-verify",
        INSTANCE,
        Blake2b512::default(),
    );
...
fn zk_wrong_commitment_rejected() {
    ...
    let mut pt = prover_transcript(b"zk-wrong-commit", INSTANCE, Blake2b512::default());
    let (proof, _eval_com, _blind) =
        DoryScheme::open_zk(&poly, &point, eval, &prover_setup, hint, &mut pt);
    ...
    let mut vt = prover_transcript(b"zk-wrong-commit", INSTANCE, Blake2b512::default());
...
fn transparent_commitment_rejected_for_zk_blinded_proof() {
    ...
    let mut pt = prover_transcript(b"zk-transparent-reject", INSTANCE, Blake2b512::default());
    let (proof, _eval_com, _blind) =
        DoryScheme::open_zk(&poly, &point, eval, &prover_setup, hint, &mut pt);

    let mut vt = prover_transcript(b"zk-transparent-reject", INSTANCE, Blake2b512::default());
...
fn zk_combined_commitment_and_hint_verify() {
    ...
    let mut pt = prover_transcript(b"zk-combined", INSTANCE, Blake2b512::default());
    ...
    let mut vt = prover_transcript(b"zk-combined", INSTANCE, Blake2b512::default());
...
fn wrong_eval_commitment_rejected_zk() {
    ...
    let mut pt = prover_transcript(b"zk-tampered-y-com", INSTANCE, Blake2b512::default());
    ...
    let mut vt = prover_transcript(b"zk-tampered-y-com", INSTANCE, Blake2b512::default());
...
fn zk_wrong_transcript_domain_rejected() {
    ...
    let mut pt = prover_transcript(b"zk-correct-domain", INSTANCE, Blake2b512::default());
    let (proof, _eval_com, _blind) =
        DoryScheme::open_zk(&poly, &point, eval, &prover_setup, hint, &mut pt);

    let mut vt = prover_transcript(b"zk-wrong-domain", INSTANCE, Blake2b512::default());
    let result = DoryScheme::verify_zk(&commitment, &point, &proof, &verifier_setup, &mut vt);
    assert!(
        result.is_err(),
