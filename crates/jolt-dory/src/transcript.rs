...
pub struct JoltToDoryTranscript<'a, T: FsTranscript<Fr>> {
    transcript: &'a mut T,
}

impl<'a, T: FsTranscript<Fr>> JoltToDoryTranscript<'a, T> {
    pub fn new(transcript: &'a mut T) -> Self {
        Self { transcript }
    }
}

impl<T: FsTranscript<Fr>> DoryTranscript for JoltToDoryTranscript<'_, T> {
    type Curve = BN254;
    ...
    fn append_field(&mut self, _label: &[u8], x: &ArkFr) {
        ...
        self.transcript.absorb_field(&jolt_scalar);
    }
    ...
    fn append_group<G: DoryGroup>(&mut self, _label: &[u8], g: &G) {
        ...
        self.transcript.absorb_bytes(&buffer);
    }
