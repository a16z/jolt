...
impl DoryProof {
    ...
    {
        let mut dory_transcript = crate::transcript::JoltToDoryTranscript::new(&mut transcript);
        let proof = &self.0;
...
mod tests {
    ...
    fn verifier_transcript_scalars_replay_dory_challenge_schedule() {
        ...
        let again = proof.verifier_transcript_scalars(
            jolt_transcript::Blake2bTranscript::<Fr>::new(b"scalar-replay"),
            &point,
        );

        assert_eq!(
            before,
            jolt_transcript::Blake2bTranscript::<Fr>::new(b"scalar-replay").state()
        );
    ...
    fn verifier_transcript_scalars_detect_invalid_inverse_relations() {
        ...
        let mut scalars = proof.verifier_transcript_scalars(
            jolt_transcript::Blake2bTranscript::<Fr>::new(b"scalar-inverses"),
            &point,
        );
        assert!(scalars.has_valid_inverse_relations());
        assert!(scalars.has_valid_replay_relations_for_point(&point));
