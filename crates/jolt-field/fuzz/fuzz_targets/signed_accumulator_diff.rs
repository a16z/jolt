...
fuzz_target!(|data: &[u8]| {
    ...
    while cursor < data.len() && ops < MAX_OPS {
        ...
        match tag % 4 {
            ...
            _ => {
                if cursor + 33 > data.len() {
                    break;
                }
                let limbs: [u64; 4] = std::array::from_fn(|limb| {
                    u64::from_le_bytes(
                        data[cursor + limb * 8..cursor + (limb + 1) * 8]
                            .try_into()
                            .unwrap(),
                    )
                });
                let is_positive = data[cursor + 32] % 2 == 0;
                cursor += 33;
                let scalar = S256::from_limbs(Limbs::new(limbs), is_positive);
