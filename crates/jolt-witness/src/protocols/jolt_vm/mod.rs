...
pub struct JoltVmProductUniskipRow {
...
pub trait JoltVmProductUniskipRows {
...
impl<T: TraceSource + Clone> JoltVmProductUniskipRows for TraceBackedJoltVmWitness<'_, T> {
    fn product_uniskip_rows(&self) -> Result<Vec<JoltVmProductUniskipRow>, WitnessError> {
        let rows = checked_pow2(self.config.log_t)?;
        ...
        for index in 0..rows {
            ...
            values.push(product_uniskip_row(&current, next.as_ref())?);
    ...
    pub fn committed_batch_stream(
            ...
            .any(|(_, kind)| matches!(kind, JoltVmCommittedStreamKind::Advice(_)))
        {
            return Err(WitnessError::UnsupportedFrontier {
                frontier: "batched Jolt VM advice streams",
            });
        }
...
impl<T: TraceSource + Clone> TraceBackedJoltVmWitness<'_, T> {
    ...
    fn evaluate_ram_val<F: Field>(&self, point: &[F]) -> Result<F, WitnessError> {
        ...
        for cycle in 0..cycles {
            ...
            result += cycle_eq[cycle] * cycle_eval;
        }
...
fn append_compact_chunk<F: Field>(
    ...
    match chunk {
        ...
        PolynomialChunk::OneHot(_) => {
            return Err(WitnessError::UnsupportedFrontier {
                frontier: "one-hot chunk materialization as compact field values",
            });
        }
    }
    Ok(())
...
fn product_uniskip_row(
    row: &TraceRow,
    next: Option<&TraceRow>,
) -> Result<JoltVmProductUniskipRow, WitnessError> {
    let instruction = JoltInstruction::try_from(row.instruction).map_err(|kind| {
        WitnessError::InvalidWitnessData {
            namespace: JOLT_VM_NAMESPACE.name,
            reason: format!("unsupported Jolt instruction kind in trace row: {kind:?}"),
...
mod tests {
    ...
    fn virtual_oracle_views_materialize_stage1_r1cs_inputs() -> Result<(), String> {
        ...
        assert!(product_rows[3].next_is_noop);
