...
pub struct TraceRecordRow {
    /// [`crate::witnesses::Pc`] — bytecode-expanded PC (requires a mapping).
    pub pc: u64,
...
impl WitnessBundle for TraceRecordRow {
    fn from_row(
        ...
        Ok(Self {
            ...
            ram_address: ram_access_address(row.ram_access).unwrap_or(0),
            remapped_ram_address: ram_access_address(row.ram_access)
                .and_then(|address| {
                    env.preprocessing
                        .memory_layout
                ...
                .flatten(),
            ram_read_value: match row.ram_access {
                RamAccess::Read(read) => read.value,
                RamAccess::Write(write) => write.pre_value,
                RamAccess::NoOp => 0,
            },
            ram_write_value: match row.ram_access {
                RamAccess::Read(read) => read.value,
