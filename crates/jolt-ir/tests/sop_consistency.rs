...
fn assert_sop_consistent(expr: &Expr, openings: &[Fr], challenges: &[Fr]) {
    let direct = expr.evaluate(openings, challenges);
    let sop = expr.to_sum_of_products();
    let via_sop = sop.evaluate(openings, challenges);
...
fn distribution_a_plus_b_times_c() {
    ...
    for _ in 0..100 {
        assert_sop_consistent(&expr, &random_vals(&mut rng, 3), &[]);
    }
}
...
fn distribution_a_plus_b_times_c_plus_d() {
    ...
    for _ in 0..100 {
        assert_sop_consistent(&expr, &random_vals(&mut rng, 4), &[]);
    }
}
...
fn negation_of_product() {
    ...
    for _ in 0..100 {
        assert_sop_consistent(&expr, &random_vals(&mut rng, 2), &[]);
    }
}
...
fn booleanity_formula() {
    ...
    for _ in 0..100 {
        assert_sop_consistent(&expr, &random_vals(&mut rng, 1), &random_vals(&mut rng, 1));
    }
}
...
fn deeply_nested_distribution() {
    ...
    for _ in 0..100 {
        assert_sop_consistent(&expr, &random_vals(&mut rng, 6), &[]);
    }
}
...
fn weighted_sum_with_challenges() {
    ...
    for _ in 0..100 {
        assert_sop_consistent(&expr, &random_vals(&mut rng, 3), &random_vals(&mut rng, 3));
    }
}
