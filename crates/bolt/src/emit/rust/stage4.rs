    ...
    fn emit_field_expr_constants(&self) -> String {
        ...
        for (index, expr) in self.field_exprs.iter().enumerate() {
            source.push_str(&emit_str_array(
                &format!("STAGE4_FIELD_EXPR_{index}_OPERAND_NAMES"),
                &expr.operand_names,
            ));
            source.push_str(&emit_str_array(
                &format!("STAGE4_FIELD_EXPR_{index}_OPERANDS"),
                &expr.operands,
            ));
        }
        let exprs = self
            ...
            .map(|(index, expr)| {
                format!(
                    "    Stage4FieldExprPlan {{ symbol: {}, kind: {}, formula: {}, operand_names: STAGE4_FIELD_EXPR_{index}_OPERAND_NAMES, operands: STAGE4_FIELD_EXPR_{index}_OPERANDS }},",
