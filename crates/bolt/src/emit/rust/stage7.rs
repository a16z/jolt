...
"#,
    ...
    fn emit_field_expr_constants(&self) -> String {
        ...
        for (index, expr) in self.field_exprs.iter().enumerate() {
            source.push_str(&emit_str_array(
                &format!("STAGE7_FIELD_EXPR_{index}_OPERAND_NAMES"),
                &expr.operand_names,
            ));
            source.push_str(&emit_str_array(
                &format!("STAGE7_FIELD_EXPR_{index}_OPERANDS"),
                &expr.operands,
            ));
        }
        let exprs = self
            ...
            .map(|(index, expr)| {
                format!(
                    "    Stage7FieldExprPlan {{ symbol: {}, kind: {}, formula: {}, operand_names: STAGE7_FIELD_EXPR_{index}_OPERAND_NAMES, operands: STAGE7_FIELD_EXPR_{index}_OPERANDS }},",
    ...
    fn emit_sumcheck_eval_constants(&self) -> String {
        let evals = self
            ...
            .collect::<Vec<_>>()
            .join("\n");
        format!("pub const STAGE7_SUMCHECK_EVALS: &[Stage7SumcheckEvalPlan] = &[\n{evals}\n];\n\n")
    }
