...
pub(in crate::expand) fn expand_amo_d(
    ...
    asm.dispatch_r(op, v_rs2.operand(), v_rd.operand(), reg(rs2(instruction)?));
    asm.dispatch_s(
...
pub(in crate::expand) fn expand_amo_minmax_d(
    ...
    asm.dispatch_r(compare_op, v1.operand(), cmp_rs1, cmp_rs2);
    asm.dispatch_r(
...
pub(in crate::expand) fn expand_amo_minmax_w(
    ...
    asm.dispatch_i(extend_op, v_rs2.operand(), reg(rs2(instruction)?), 0);
    asm.dispatch_i(extend_op, v0.operand(), v_rd.operand(), 0);
    let (cmp_rs1, cmp_rs2) = if min {
        (v_rs2.operand(), v0.operand())
    } else {
        (v0.operand(), v_rs2.operand())
    };
    asm.dispatch_r(compare_op, v0.operand(), cmp_rs1, cmp_rs2);
    asm.dispatch_r(
        JoltInstructionKind::SUB,
        v_rs2.operand(),
        reg(rs2(instruction)?),
...
pub(in crate::expand) fn expand_amo_post64(
    ...
    asm.dispatch_i(JoltInstructionKind::ANDI, v_mask, rs1, format_i_imm(-8));
    asm.dispatch_s(JoltInstructionKind::SD, v_mask, v_dword, 0);
    asm.dispatch_i(JoltInstructionKind::VirtualSignExtendWord, rd, v_rd, 0);
    Ok(())
}
...
pub(in crate::expand) fn expand_narrow_store(
    ...
    if let Some(alignment) = alignment {
        asm.dispatch_address(alignment, reg(rs1(instruction)?), instruction.operands.imm);
    }
    asm.dispatch_i(
        JoltInstructionKind::ADDI,
        v0.operand(),
