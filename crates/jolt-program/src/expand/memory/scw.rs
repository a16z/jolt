...
pub(in crate::expand) fn expand_scw(
    ...
    asm.emit_i(JoltInstructionKind::ADDI, v_reservation, v_success, 0)?;
    asm.allocator().release(v_success)?;

    let v_mem = asm.allocator().allocate()?;
    asm.emit_i(JoltInstructionKind::LW, v_mem, rs1(instruction)?, 0)?;

    let v_diff = asm.allocator().allocate()?;
    asm.emit_r(JoltInstructionKind::SUB, v_diff, rs2(instruction)?, v_mem)?;
    asm.emit_r(JoltInstructionKind::MUL, v_diff, v_diff, v_reservation)?;
    asm.emit_r(JoltInstructionKind::ADD, v_diff, v_mem, v_diff)?;
    asm.allocator().release(v_mem)?;

    asm.emit_i(JoltInstructionKind::ADDI, v_reservation_d, v_diff, 0)?;
    ...
    asm.emit_i(JoltInstructionKind::ADDI, v_reservation_d, 0, 0)?;
