...
pub(in crate::expand) fn expand_scd(
    ...
    asm.expand_i(JoltInstructionKind::LD, v_mem, rs1(instruction)?, 0)?;

    let v_diff = asm.allocate()?;
    asm.expand_r(JoltInstructionKind::SUB, v_diff, rs2(instruction)?, v_mem)?;
    asm.expand_r(JoltInstructionKind::MUL, v_diff, v_diff, v_success)?;
    asm.expand_r(JoltInstructionKind::ADD, v_diff, v_mem, v_diff)?;
    asm.release(v_mem)?;
    asm.expand_s(JoltInstructionKind::SD, rs1(instruction)?, v_diff, 0)?;
    asm.release(v_diff)?;
    asm.expand_i(JoltInstructionKind::ADDI, v_reservation, 0, 0)?;
    asm.expand_i(JoltInstructionKind::ADDI, v_reservation_w, 0, 0)?;
    asm.expand_i(JoltInstructionKind::XORI, rd(instruction)?, v_success, 1)?;
