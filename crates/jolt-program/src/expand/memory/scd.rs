...
pub(in crate::expand) fn expand_scd(
    ...
    asm.emit_i(SourceInstructionKind::ADDI, v_one.operand(), reg(0), 1);
    asm.emit_b(
    ...
    asm.emit_i(SourceInstructionKind::ADDI, reg(v_reservation), reg(0), 0);
    asm.emit_i(SourceInstructionKind::ADDI, reg(v_reservation_w), reg(0), 0);
    asm.emit_i(
