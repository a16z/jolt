use crate::traits::impl_lookup_table;
use crate::traits::LookupQuery;
use jolt_riscv::instructions::Jal;
use jolt_riscv::JoltCycle;

impl_lookup_table!(Jal, Some(RangeCheck));

impl<const XLEN: usize, C: JoltCycle> LookupQuery<XLEN> for Jal<C> {
    fn to_lookup_operands(&self) -> (u64, u128) {
        let (pc, imm) = LookupQuery::<XLEN>::to_instruction_inputs(self);
        (0, (pc as i128 + imm) as u128)
    }

    fn to_lookup_index(&self) -> u128 {
        LookupQuery::<XLEN>::to_lookup_operands(self).1
    }

    fn to_instruction_inputs(&self) -> (u64, i128) {
        let mask = (1u128 << XLEN).wrapping_sub(1) as u64;
        (
            Into::<jolt_riscv::JoltInstructionRow>::into(self.0.instruction()).address as u64
                & mask,
            Into::<jolt_riscv::JoltInstructionRow>::into(self.0.instruction())
                .operands
                .imm
                & mask as i128,
        )
    }

    fn to_lookup_output(&self) -> u64 {
        let (pc, imm) = LookupQuery::<XLEN>::to_instruction_inputs(self);
        let mask = (1u128 << XLEN).wrapping_sub(1) as u64;
        pc.wrapping_add(imm as u64) & mask
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::{
        instruction_inputs_match_constraint_test, lookup_output_matches_trace_test,
        materialize_entry_test,
    };

    #[test]
    fn materialize_entry_jal() {
        materialize_entry_test!(Jal, tracer::instruction::jal::JAL);
    }

    #[test]
    fn instruction_inputs_match_constraint_jal() {
        instruction_inputs_match_constraint_test!(Jal, tracer::instruction::jal::JAL);
    }

    #[test]
    fn lookup_output_matches_trace_jal() {
        lookup_output_matches_trace_test!(Jal, tracer::instruction::jal::JAL);
    }
}
