...
fuzz_target!(|data: &[u8]| {
    ...
    let mut pt = prover_transcript(b"fuzz-wrong-eval", INSTANCE, Blake2b512::default());
    let proof =
        <TestScheme as CommitmentScheme>::open(&poly, &point, eval, &pk, None, &mut pt);

    let mut vt = verifier_transcript(b"fuzz-wrong-eval", INSTANCE, Blake2b512::default(), &[]);
