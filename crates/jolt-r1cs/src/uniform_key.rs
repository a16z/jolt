//! Uniform R1CS key for repeated-constraint instances.
//!
...
/// A key for uniform (repeated-constraint) R1CS instances.
///
...
#[serde(bound = "")]
pub struct UniformR1csKey<F: Field> {
...
impl<F: Field> UniformR1csKey<F> {
    /// Creates a new uniform R1CS key.
    ///
    /// # Arguments
    ///
...
mod tests {
    ...
    fn test_key(num_cycles: usize) -> UniformR1csKey<Fr> {
    ...
    fn dense_vs_key_mle_consistency() {
        ...
        for (num_cycles, num_constraints, num_vars) in configs {
            ...
            let combined_row = edge_transform::combined_partial_evaluate(
                &key,
                &r_x,
                rho_a,
                rho_b,
