...
//!
//! The fixtures are generated by the real ZK prover and include trusted and
//! untrusted advice plus a committed program. Every non-skipped input
...
fn mutate_compressed(
    proof: &mut CompressedSumcheckProof<Fr>,
    operation: u8,
    index: usize,
    delta: Fr,
) -> bool {
    match operation % 2 {
        ...
        _ => {
            ...
            let mut coefficients = proof.round_polynomials[round]
                .coeffs_except_linear_term()
                .to_vec();
            let coefficient = operation as usize / 2 % coefficients.len();
            coefficients[coefficient] += delta;
            proof.round_polynomials[round] = CompressedPoly::new(coefficients);
            true
        }
    }
}
...
fuzz_target!(|data: &[u8]| {
    ...
    let changed = match mutation {
        ...
        4 => mutate_committed_sumcheck(stage_proof(&mut proof, stage), operation, index),
        5 => {
        ...
        6 => {
            ...
            blindfold_proof.random_u += delta;
            true
        }
        7 => {
            let JoltProofClaims::Zk { blindfold_proof } = &mut proof.claims else {
            ...
            mutate_compressed(&mut blindfold_proof.outer_sumcheck, operation, index, delta)
        }
        8 => {
            let JoltProofClaims::Zk { blindfold_proof } = &mut proof.claims else {
                return;
            };
            mutate_compressed(&mut blindfold_proof.inner_sumcheck, operation, index, delta)
        }
        9 => {
            let JoltProofClaims::Zk { blindfold_proof } = &mut proof.claims else {
                return;
