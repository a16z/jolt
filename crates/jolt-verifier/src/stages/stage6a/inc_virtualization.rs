//! The inc-virtualization phase (packed path): a single-instance sumcheck run
//! at the head of the stage-6 region, before the address batch, that
//! virtualizes the four reduced `Inc` claims into the committed `FusedInc`
//! stream and its `OpFlags(Store)` destination selector. `RdInc`/`RamInc` are
//! never PCS-opened on the packed path; this phase is where their claims leave
//! the base PIOP. The store selector claim feeds this stage's read-raf sixth
//! val stage, and its cycle point (this phase's bound point) anchors the
//! stage-7 chunk reconstruction's decode legs.
...
/// The phase's verified openings: the `FusedInc` and store-selector claims at
/// the phase's bound cycle point, consumed by this stage's read-raf (store
