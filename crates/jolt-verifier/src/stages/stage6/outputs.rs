...
impl<F: Field, C> Stage6ZkOutput<F, C> {
    pub const fn advice_cycle_phase(
        &self,
        kind: JoltAdviceKind,
    ) -> Option<&AdviceCyclePhasePublicOutput<F>> {
        match kind {
            JoltAdviceKind::Trusted => self.trusted_advice_cycle_phase.as_ref(),
            JoltAdviceKind::Untrusted => self.untrusted_advice_cycle_phase.as_ref(),
        }
    }
}
