...
mod tests {
    ...
    fn precommitted_statements_use_lattice_commitment_layout_digest() {
        ...
        let commitment: jolt_akita::AkitaCommitment = serde_json::from_value(serde_json::json!({
            "layout_digest": digest,
            "num_vars": 1,
            "poly_count": 1,
            "native": [1],
        }))
        .expect("test Akita commitment should deserialize");
        ...
        let (opening_claims, statements) = precommitted_clear_statements(&[entry], default_digest)
            .expect("precommitted statement should build");

        assert_eq!(opening_claims.len(), 1);
        assert_eq!(
            opening_claims[0].evaluation.point,
            Point::high_to_low(vec![Fr::from_u64(2)])
        );
        assert_eq!(opening_claims[0].evaluation.value, Fr::from_u64(21));
        assert_eq!(statements.len(), 1);
        assert_eq!(statements[0].logical_point, vec![Fr::from_u64(2)]);
        assert_eq!(statements[0].pcs_point, vec![Fr::from_u64(2)]);
        assert_eq!(statements[0].layout_digest, digest);
        assert_ne!(statements[0].layout_digest, default_digest);
        assert_eq!(statements[0].claims[0].claim, Fr::from_u64(7));
