...
/// are equality-checked against the appendage by `stage2::verify` rather than
/// alias-elided, so they stay Fiat-Shamir-bound like any member output).
#[cfg(feature = "field-inline")]
impl<F: Field> Stage2BatchSumchecks<F> {
    ...
    pub fn opening_values(
        ...
        use crate::stages::relations::{absorbed_opening_values, OutputClaims as _};
...
mod tests {
    ...
    /// The FR claim reduction's `EqSpartan` mirrors the instruction claim
    /// reduction's derivation exactly: `Eq(reduced opening point, tau_low)`
    /// over the same reversed bound point. Both relations are built with the
    /// same `tau_low`, so at the same batch point the two publics are equal.
    #[cfg(feature = "field-inline")]
    #[test]
    fn field_registers_eq_spartan_matches_instruction_claim_reduction_derivation() {
        use crate::stages::stage2::field_registers_claim_reduction::FieldRegistersClaimReductionChallenges;
        use crate::stages::stage2::instruction_claim_reduction::InstructionClaimReductionChallenges;
        ...
        let log_t = 4usize;
        let tau_low: Vec<Fr> = (0..log_t as u64).map(|i| fr(60 + i)).collect();
        let instruction =
            InstructionClaimReduction::<Fr>::new(TraceDimensions::new(log_t), tau_low.clone());
        let field_registers = FieldRegistersClaimReduction::<Fr>::new(
            FieldRegistersTraceDimensions::new(log_t),
            tau_low,
        );

        let point: Vec<Fr> = (0..log_t as u64).map(|i| fr(80 + i)).collect();
        let instruction_input_points = Default::default();
        let instruction_points = instruction
            .derive_opening_points(&point, &instruction_input_points)
            .unwrap();
        let field_input_points = Default::default();
        let field_points = field_registers
            .derive_opening_points(&point, &field_input_points)
            .unwrap();

        let instruction_challenges =
            InstructionClaimReductionChallenges::from_transcript_values(std::iter::repeat(fr(1)))
                .unwrap();
        let field_challenges = FieldRegistersClaimReductionChallenges::from_transcript_values(
            std::iter::repeat(fr(1)),
        )
        .unwrap();

        let instruction_eq = instruction
            .derive_output_term(
                &JoltDerivedId::InstructionClaimReduction(
                    InstructionClaimReductionPublic::EqSpartan,
                ),
                &instruction_input_points,
                &instruction_points,
                &instruction_challenges,
            )
            .unwrap();
        let field_eq = field_registers
            .derive_output_term(
        ...
        assert_eq!(field_eq, instruction_eq);
    }
}
