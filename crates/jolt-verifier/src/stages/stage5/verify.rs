...
struct Stage5BatchInputClaims<F: Field> {
    ...
    ram_ra_claim_reduction: Option<F>,
    ...
    T: Transcript<Challenge = PCS::Field>,
{
    ...
    validate_ram_ra_claim_shape(lattice, claims.ram_ra_claim_reduction.as_ref())?;
    ...
    let input_claims = Stage5BatchInputClaims {
        ...
        ram_ra_claim_reduction: (!lattice)
            .then(|| {
                ram_claims.input.expression().try_evaluate(
                    |id| match *id {
                        ...
                        id if id == ram_ra_val_check => {
                            Ok(stage4.output_claims.ram_val_check.ram_ra)
                        }
                        id => Err(VerifierError::MissingOpeningClaim { id }),
                    },
                    |id| match id {
                        JoltChallengeId::RamRaClaimReduction(
                            RamRaClaimReductionChallenge::Gamma,
                        ) => Ok(ram_gamma),
                        _ => Err(VerifierError::MissingStageClaimChallenge { id: *id }),
                    },
                    |id| Err(VerifierError::MissingStageClaimPublic { id: *id }),
    ...
    if let Some(input_claim) = input_claims.ram_ra_claim_reduction {
    ...
    let ram_ra_claim_reduction =
        if !lattice {
            let output_claims = claims.ram_ra_claim_reduction.as_ref().ok_or(
                VerifierError::MissingOpeningClaim {
                    id: ram::ra_claim_reduction_output_openings()[0],
                },
            )?;
            let input_claim = input_claims
                .ram_ra_claim_reduction
                .ok_or(VerifierError::MissingOpeningClaim { id: ram_ra_raf })?;
            let ram_point = batch
                .try_instance_point(ram_claims.sumcheck.rounds)
                .map_err(|error| VerifierError::StageClaimSumcheckFailed {
                    stage: JoltRelationId::RamRaClaimReduction,
            ...
            let (ram_read_write_address, ram_read_write_cycle) =
                ram_read_write_opening_point.split_at(log_k);
            let (ram_val_check_address, ram_val_check_cycle) =
                ram_val_check_opening_point.split_at(log_k);
            if ram_raf_address != ram_read_write_address {
                return Err(VerifierError::StageClaimOpeningMismatch {
                    stage: JoltRelationId::RamRaClaimReduction,
            ...
            let ram_public_values = ram::RamRaClaimReductionPublicValues {
                ...
                eq_cycle_read_write: try_eq_mle(ram_read_write_cycle, &ram_cycle).map_err(
                    |error| VerifierError::StageClaimPublicInputFailed {
                        stage: JoltRelationId::RamRaClaimReduction,
                        reason: error.to_string(),
                    },
                )?,
                eq_cycle_val_check: try_eq_mle(ram_val_check_cycle, &ram_cycle).map_err(
                    |error| VerifierError::StageClaimPublicInputFailed {
                        stage: JoltRelationId::RamRaClaimReduction,
                        reason: error.to_string(),
                    },
            ...
            let expected_output_claim = ram_claims.output.expression().try_evaluate(
                |id| match *id {
                    id if id == ram_ra_reduced => Ok(output_claims.ram_ra),
                    id => Err(VerifierError::MissingOpeningClaim { id }),
                },
                |id| match id {
                ...
                |id| match id {
                    JoltPublicId::RamRaClaimReduction(public_id) => {
                        Ok(ram_public_values.value(*public_id))
                    }
                    _ => Err(VerifierError::MissingStageClaimPublic { id: *id }),
                },
            )?;
            Some(VerifiedStage5Sumcheck {
                input_claim,
                sumcheck_point: ram_point.to_vec(),
                opening_point: [ram_read_write_address, ram_cycle.as_slice()].concat(),
    ...
    let expected_outputs = Stage5BatchExpectedOutputClaims {
        ...
        ram_ra_claim_reduction: ram_ra_claim_reduction
            .as_ref()
    ...
    let mut expected_outputs_in_order = vec![expected_outputs.instruction_read_raf];
    if let Some(output_claim) = expected_outputs.ram_ra_claim_reduction {
        expected_outputs_in_order.push(output_claim);
    }
    expected_outputs_in_order.push(expected_outputs.registers_val_evaluation);
    #[cfg(feature = "field-inline")]
    expected_outputs_in_order.push(expected_outputs.field_registers_val_evaluation);
    if coefficients.len() != expected_outputs_in_order.len() {
        return Err(VerifierError::StageClaimSumcheckFailed {
            stage: JoltRelationId::InstructionReadRaf,
            reason: format!(
                "Stage 5 batch verifier returned {} coefficients for {} instances",
                coefficients.len(),
                expected_outputs_in_order.len()
            ),
        });
    }
    let expected_final_claim = coefficients
        .iter()
    ...
    T: Transcript<Challenge = F>,
{
    ...
    if let Some(claims) = &claims.ram_ra_claim_reduction {
        transcript.append_labeled(b"opening_claim", &claims.ram_ra);
    }
...
fn validate_ram_ra_claim_shape<F: Field>(
    lattice: bool,
    output_claims: Option<&super::inputs::RamRaClaimReductionOutputOpeningClaims<F>>,
) -> Result<(), VerifierError> {
    let id = ram::ra_claim_reduction_output_openings()[0];
    match (lattice, output_claims.is_some()) {
        (true, true) => Err(VerifierError::UnexpectedOpeningClaim { id }),
        (false, false) => Err(VerifierError::MissingOpeningClaim { id }),
        _ => Ok(()),
    }
}
...
mod tests {
    ...
    #[test]
    fn ram_ra_claim_shape_tracks_protocol_family() {
        let claims = super::super::inputs::RamRaClaimReductionOutputOpeningClaims {
            ram_ra: Fr::from_u64(1),
        };

        let error = validate_ram_ra_claim_shape(true, Some(&claims))
            .expect_err("lattice Stage 5 must not carry RAM RA reduction claims");
        assert!(matches!(
            error,
            VerifierError::UnexpectedOpeningClaim { id }
                if id == ram::ra_claim_reduction_output_openings()[0]
        ));

        let error = validate_ram_ra_claim_shape::<Fr>(false, None)
            .expect_err("curve Stage 5 must carry RAM RA reduction claims");
        assert!(matches!(
            error,
        ...
        validate_ram_ra_claim_shape(false, Some(&claims))
            .expect("curve Stage 5 accepts RAM RA reduction claims");
        validate_ram_ra_claim_shape::<Fr>(true, None)
            .expect("lattice Stage 5 omits RAM RA reduction claims");
    }
}
