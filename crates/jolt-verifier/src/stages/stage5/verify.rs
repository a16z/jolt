...
use super::{
    ...
    outputs::{
        Stage5Output, VerifiedInstructionReadRafSumcheck, VerifiedStage5Batch,
    ...
    T: Transcript<Challenge = PCS::Field>,
{
    ...
    let product_lookup_output = deps.stage2.output_claims.product_remainder.lookup_output;
    let reduced_lookup_output = deps
        .stage2
        .output_claims
    ...
    let input_claims = Stage5BatchInputClaims {
        instruction_read_raf: instruction_claims.input.expression.try_evaluate(
            |id| match *id {
                ...
                id if id == left_lookup_operand => Ok(deps
                    .stage2
                    .output_claims
                    .instruction_claim_reduction
                    .left_lookup_operand),
                id if id == right_lookup_operand => Ok(deps
                    .stage2
                    .output_claims
                    .instruction_claim_reduction
        ...
        ram_ra_claim_reduction: ram_claims.input.expression.try_evaluate(
            |id| match *id {
                id if id == ram_ra_raf => Ok(deps.stage2.output_claims.ram_raf_evaluation),
                id if id == ram_ra_read_write => Ok(deps.stage2.output_claims.ram_read_write.ra),
                id if id == ram_ra_val_check => Ok(deps.stage4.output_claims.ram_val_check.ram_ra),
                id => Err(VerifierError::MissingOpeningClaim { id }),
            },
            |id| match id {
        ...
        registers_val_evaluation: registers_claims.input.expression.try_evaluate(
            |id| match *id {
                id if id == registers_val => {
                    Ok(deps.stage4.output_claims.registers_read_write.registers_val)
                }
                id => Err(VerifierError::MissingOpeningClaim { id }),
            },
    ...
    let ram_raf_opening_point = &deps.stage2.batch.ram_raf_evaluation.opening_point;
    let ram_read_write_opening_point = &deps.stage2.batch.ram_read_write.opening_point;
    let ram_val_check_opening_point = &deps.stage4.batch.ram_val_check.opening_point;
    ...
    let registers_read_write_opening_point = &deps.stage4.batch.registers_read_write.opening_point;
