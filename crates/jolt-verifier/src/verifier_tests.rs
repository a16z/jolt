...
use super::*;
use crate::proof::{
    ClearProofClaims, CommitmentPayload, JoltProofClaims, JoltStageProofs, LatticeCommitmentPayload,
};
use common::jolt_device::{JoltDevice, MemoryConfig};
#[cfg(feature = "akita")]
use jolt_claims::protocols::jolt::{
    formulas::{
        claim_reductions::bytecode, dimensions::JoltFormulaDimensions, ra::JoltRaPolynomialLayout,
    },
...
fn proof_model_rejects_unknown_serialized_fields() {
    ...
    let zk =
        serde_json::to_value(proof_with_zk(true, zk_claims())).expect("ZK proof should serialize");
...
fn lattice_stage8_batch_statement_uses_precommitted_bytecode_source_components() {
    ...
    let unsigned_inc_chunk_count = jolt_claims::protocols::jolt::unsigned_inc_lower_chunk_count(
        proof.one_hot_config.committed_chunk_bits(),
    )
...
fn akita_snapshot_stage7_output(
    ...
    output_claims.hamming_weight_claim_reduction.bytecode_ra = field_zeros(ra_layout.bytecode());
    ...
    let bytecode_layout = precommitted
        ...
        .unwrap_or_else(|| panic!("committed bytecode schedule should exist"));
    let bytecode_point =
        field_zeros(bytecode::committed_lane_vars() + bytecode_layout.log_bytecode_chunk_size());
        ...
        batch: stage7::outputs::VerifiedStage7Batch {
            ...
            hamming_weight_claim_reduction:
                stage7::outputs::VerifiedHammingWeightClaimReductionSumcheck {
                    ...
                    instruction_ra_opening_points: vec![ra_point.clone(); ra_layout.instruction()],
...
#[cfg(feature = "akita")]
fn verified_stage6_sumcheck(opening_point: Vec<Fr>) -> stage6::outputs::VerifiedStage6Sumcheck<Fr> {
