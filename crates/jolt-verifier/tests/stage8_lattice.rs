...
use jolt_akita::{AkitaField, AKITA_FIELD_MODULUS};
...
use jolt_claims::protocols::jolt::{
    ...
    unsigned_inc_msb_opening, unsigned_inc_validity_requirements, JoltCommittedPolynomial,
    JoltOpeningId, JoltRelationId, LatticePackedFamilyId, LatticePackedValidityRequirement,
...
use jolt_openings::{
    ...
    PackingTerm, PackingWitnessLayout, PhysicalView,
};
...
use jolt_verifier::{
    ...
    IncrementCommitmentMode, JoltProtocolConfig, PackedWitnessConfig, PcsFamily, ProgramMode,
    VerifierError,
};
...
fn packed_alphabet_with_size(size: usize) -> Result<PackingAlphabet, VerifierError> {
    match size {
        0 => Err(VerifierError::InvalidProtocolConfig {
            reason: "packed validity requirement alphabet size must be nonzero".to_string(),
        }),
...
fn unsigned_increment_validity_layout(log_t: usize, log_k_chunk: usize) -> PackingWitnessLayout {
...
fn derive_validity_statements_adds_bytecode_store_rd_disjointness() {
    ...
    let requirement = LatticePackedValidityRequirement::bytecode_store_rd_disjoint(
        chunk,
...
fn validity_batch_builder_lowers_cell_booleanity_to_packed_terms() {
    ...
    let PhysicalView::Packing {
        ...
        panic!("validity opening should use a packing view");
    };
...
fn validity_batch_builder_lowers_bytecode_store_rd_disjointness_factors() {
    ...
    let statement = LatticePackedValidityStatement {
        requirement: LatticePackedValidityRequirement::bytecode_store_rd_disjoint(
            chunk,
            CircuitFlags::Store as usize,
        ),
    ...
    let PhysicalView::Packing { terms, .. } = &batch.statement.claims[0].view else {
        panic!("Store factor should use a packing view");
    };
    ...
    let PhysicalView::Packing { terms, .. } = &batch.statement.claims[1].view else {
        panic!("Rd-present factor should use a packing view");
    };
...
fn validity_batch_builder_lowers_field_element_canonical_byte_factors() {
    ...
    let PhysicalView::Packing { terms, .. } = &batch.statement.claims[0].view else {
        panic!("high-byte range factor should use a packing view");
    };
    ...
    let PhysicalView::Packing { terms, .. } = &batch.statement.claims[1].view else {
        panic!("high-byte equality factor should use a packing view");
    };
    assert_eq!(terms.len(), 1);
    assert_eq!(
        terms[0].family,
    ...
    let PhysicalView::Packing { terms, .. } = &batch.statement.claims[2].view else {
        panic!("low-byte range factor should use a packing view");
    };
...
fn validity_coverage_requires_bytecode_store_rd_disjointness() {
    ...
    let disjoint = LatticePackedValidityRequirement::bytecode_store_rd_disjoint(
        0,
        CircuitFlags::Store as usize,
    );
