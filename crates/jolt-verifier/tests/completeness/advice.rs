...
#[test]
#[cfg(all(feature = "core-fixtures", not(feature = "zk")))]
fn standard_advice_core_proof_reaches_frontier() {
    support::assert_reaches_current_frontier(
        crate::support::core_fixtures::standard_advice_case().verify(),
    );
}
...
fn standard_advice_core_proof_reaches_frontier() {
    assert_eq!(
        completeness_expectation(cases::STANDARD_ADVICE_COMMITMENTS),
