...
impl Stage1CpuProgram {
    ...
    fn verify_driver_bindings(&self) -> Result<(), EmitError> {
        ...
        for claim in &self.claims {
            if !kernels.contains(&claim.kernel) {
                return Err(EmitError::new(format!(
                    "sumcheck claim @{} references missing kernel @{}",
                    claim.symbol, claim.kernel
                )));
            }
        }
        for driver in &self.drivers {
            if !kernels.contains(&driver.kernel) {
                return Err(EmitError::new(format!(
                    "sumcheck driver @{} references missing kernel @{}",
                    driver.symbol, driver.kernel
                )));
            }
            ...
            for claim in &batch.ordered_claims {
                ...
                if claim.kernel != driver.kernel {
                    return Err(EmitError::new(format!(
                        "sumcheck driver @{} kernel @{} differs from claim @{} kernel @{}",
                        driver.symbol, driver.kernel, claim.symbol, claim.kernel
                    )));
                }
            }
        }
    ...
    fn verify_opening_flow(&self) -> Result<(), EmitError> {
        ...
        for claim in &self.opening_claims {
            if !drivers.contains(&claim.point_source) {
                return Err(EmitError::new(format!(
...
"
    ...
    fn emit_verifier_entrypoint() -> &'static str {
        r"pub fn verify_stage1_outer<E, T>(
    executor: &mut E,
    transcript: &mut T,
) -> Result<Stage1ExecutionArtifacts<Fr>, Stage1KernelError>
where
