...
fn append_challenge_extract<'c, 'a>(
    context: &'c MeliorContext,
    module: &'a BoltModule<'c, Protocol>,
    symbol: &str,
    source: &str,
    ...
    first_result(op, "field.challenge_extract")
}

fn append_field_expr<'c, 'a>(
    context: &'c MeliorContext,
    module: &'a BoltModule<'c, Protocol>,
    symbol: &str,
    kind: &str,
    ...
    let op = context.append_typed_op(
        ...
        &[
            ...
            ("operands", &symbol_array_attr(operand_symbols)),
        ],
        operands,
        &["!field.scalar"],
    )?;
    first_result(op, "field.expr")
}
...
fn append_product_uniskip<'c, 'a>(
    ...
    let tau_high = append_challenge_extract(
        context,
        module,
    ...
    let input_claim = append_field_expr(
        context,
        module,
...
fn append_stage2_batched_sumcheck<'c, 'a>(
    ...
    let ram_read_write_gamma = append_challenge_extract(
        context,
        module,
    ...
    let instruction_lookup_gamma = append_challenge_extract(
        context,
        module,
    ...
    let ram_read_write_claim = append_field_expr(
        context,
        module,
    ...
    let product_remainder_claim = append_field_expr(
        context,
        module,
        "stage2.product_virtual.remainder.claim_expr",
        "direct",
        "opening_eval",
        &["stage2.product_virtual.uniskip.eval.UnivariateSkip"],
        &[uniskip.eval],
    )?;
    let instruction_claim = append_field_expr(
        context,
        module,
    ...
    let ram_raf_claim = append_field_expr(
        context,
        module,
        "stage2.ram_raf.claim_expr",
        "direct",
        "opening_eval",
        &["stage2.input.stage1.RamAddress"],
        &[inputs.ram_address.eval],
    )?;
    let ram_output_claim = append_field_constant(context, module, "stage2.ram_output.zero", 0)?;
    let claims = [
