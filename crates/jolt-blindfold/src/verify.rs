...
fn folded_instance_from_proof<F, C, T>(
    protocol: &BlindFoldProtocol<F, C>,
    proof: &BlindFoldProof<F, C>,
    transcript: &mut T,
) -> Result<RelaxedInstance<F, C>, VerificationError<F>>
where
    F: Field + AppendToTranscript,
    C: Clone + HomomorphicCommitment<F> + AppendToTranscript,
    T: Transcript<Challenge = F>,
{
    let committed = protocol.committed_relaxed_instance(&proof.auxiliary_row_commitments)?;
    ...
    protocol.validate_cross_term_error_rows(&proof.cross_term_error_row_commitments)?;
    append_values(
        transcript,
        b"bf_cross_e",
    ...
    T: Transcript,
{
    ...
    instance.u.append_to_transcript(transcript);
    append_values(transcript, witness_label, &instance.witness_row_commitments);
    append_values(transcript, error_label, &instance.error_row_commitments);
    append_values(transcript, eval_label, &instance.eval_commitments);
}
...
fn verify_outer_folded_r1cs<F, VC, T>(
    protocol: &BlindFoldProtocol<F, VC::Output>,
    proof: &BlindFoldProof<F, VC::Output>,
    vc_setup: &VC::Setup,
    ...
    T: Transcript<Challenge = F>,
{
    ...
    append_values(
        transcript,
        b"bf_az_bz_cz",
        &[proof.az_rx, proof.bz_rx, proof.cz_rx],
    );
    ...
    VC::Output: Copy + AppendToTranscript,
{
    for (index, ((commitment, &output), &blinding)) in folded
        .eval_commitments
        .iter()
        .zip(&proof.folded_eval_outputs)
        .zip(&proof.folded_eval_blindings)
        .enumerate()
    {
    ...
    T: Transcript<Challenge = F>,
{
    ...
    let entry_vars =
        log2_power_of_two::<F>("witness row length", protocol.dimensions.witness.row_len)?;
    let num_vars =
        row_vars
            .checked_add(entry_vars)
            .ok_or(VerificationError::InvalidPowerOfTwo {
    ...
    let l_w_at_ry = compute_l_w_at_ry(&protocol.r1cs, &outer.point, &inner.point, ra, rb, rc)?;
...
fn log2_power_of_two<F>(name: &'static str, value: usize) -> Result<usize, VerificationError<F>>
where
    F: FieldCore,
{
    if value == 0 || !value.is_power_of_two() {
        return Err(VerificationError::InvalidPowerOfTwo { name, value });
    }
