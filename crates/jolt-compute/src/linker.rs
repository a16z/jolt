...
pub struct Executable<B: ComputeBackend, F: Field> {
    ...
    pub module: Module,
    /// Op sequence executed by the runtime.
    pub ops: Vec<Op>,
    /// Backend-compiled kernels, indexed by compute ops (e.g. `Op::Reduce` specs).
...
pub fn link<B: ComputeBackend, F: Field>(module: Module, _backend: &B) -> Executable<B, F> {
    let kernels: Vec<B::CompiledKernel<F>> = module
        .prover
        .kernels
        .iter()
        .map(|def| _backend.compile(&def.spec))
        .collect();
