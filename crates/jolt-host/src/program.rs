...
impl Program {
    ...
    #[expect(
        clippy::expect_used,
        reason = "the infallible host build API reports CLI failures by panicking"
    )]
    pub fn build_with_features(&mut self, target_dir: &str, extra_features: &[&str]) {
        if self.elf.is_none() {
            ...
            if !output.status.success() {
                ...
                let _ = io::stderr().write_all(output_msg.as_bytes());
    ...
    #[expect(
        clippy::expect_used,
        reason = "the infallible host decode API reports malformed programs by panicking"
    )]
    ...
    #[expect(
        clippy::expect_used,
        reason = "the infallible host trace API reports build and decode failures by panicking"
    )]
    pub fn trace(
        &mut self,
        inputs: &[u8],
        ...
        let (lazy_trace, trace, memory, jolt_device, _advice_tape) = tracer::trace(
            &elf_contents,
            Some(&elf),
            inputs,
            untrusted_advice,
    ...
    #[expect(
        clippy::expect_used,
        reason = "the infallible analysis API reports trace failures by panicking"
    )]
