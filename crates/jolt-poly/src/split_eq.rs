...
impl<F: Field> GruenSplitEqPolynomial<F> {
    ...
    pub fn gruen_poly_deg_3(
        ...
        let quadratic_eval_1 = field_div(cubic_eval_1, eq_eval_1);
    ...
    pub fn gruen_poly_deg_2(&self, q_0: F, previous_claim: F) -> UnivariatePoly<F> {
        ...
        let linear_eval_1 = field_div(quadratic_eval_1, eq_eval_1);
    ...
    pub fn gruen_poly_from_evals(&self, q_evals: &[F], s_0_plus_s_1: F) -> UnivariatePoly<F> {
        ...
        let q_at_0 = field_div(s_0_plus_s_1 - l_at_1 * q_evals[0], l_at_0);
...
#[inline]
fn field_div<F: Field>(numerator: F, denominator: F) -> F {
    let Some(inverse) = denominator.inverse() else {
        unreachable!("split equality denominator must be nonzero");
    };
    numerator * inverse
}
