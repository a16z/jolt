...
pub fn boolean_point_msb<F: Field>(num_vars: usize, index: usize) -> Vec<F> {
    ...
    for bit_index in 0..num_vars {
        let shift = num_vars - bit_index - 1;
        let bit = if shift < usize::BITS as usize {
            (index >> shift) & 1
        } else {
            0
...
mod tests {
    ...
    #[test]
    fn boolean_point_and_index_use_msb_order() {
        for n in 0..=8 {
            for index in 0..(1usize << n) {
                let point = boolean_point_msb::<Fr>(n, index);
                assert_eq!(point, index_to_bits(index, n));
                assert_eq!(boolean_index_msb(&point), Some(index));
            }
        }

        let non_boolean = [Fr::from_u64(2)];
        assert_eq!(boolean_index_msb(&non_boolean), None);
    }
