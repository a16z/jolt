...
fn akita_single_opening_roundtrips_across_dimensions() {
    single_opening_roundtrip(14, 10, (2..16).map(f).collect(), b"akita-e2e-arity-14");
    single_opening_roundtrip(15, 20, (3..18).map(f).collect(), b"akita-e2e-arity-15");
    single_opening_roundtrip(16, 30, (5..21).map(f).collect(), b"akita-e2e-arity-16");
}
