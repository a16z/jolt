...
use crate::adapters::{
    ...
    AkitaOneHotK16BackendScheme, AkitaOneHotK256BackendScheme, AkitaOneHotK256D128BackendScheme,
        ...
        let (backend_commitment, backend_hint) =
            with_backend_pool(
                || match (setup.one_hot_k(), setup.one_hot_ring_dimension()) {
                    (AKITA_ONE_HOT_K16, 64) => AkitaOneHotK16BackendScheme::commit(
                        backend_prover_setup,
                        &backend_polynomials,
                        &stack,
                    ),
                    (AKITA_ONE_HOT_K256, 64) => AkitaOneHotK256BackendScheme::commit(
                        backend_prover_setup,
                        &backend_polynomials,
                        &stack,
                    ),
...
mod tests {
    ...
    fn packed_trace_one_hot_roundtrip(
        one_hot_k: usize,
        column_capacity: usize,
        num_rows: usize,
    ...
    fn one_hot_k16_roundtrip() {
        ...
        packed_trace_one_hot_roundtrip(AKITA_ONE_HOT_K16, 64, 1 << 13, None);
    }

    #[test]
    fn one_hot_k256_roundtrip() {
        packed_trace_one_hot_roundtrip(AKITA_ONE_HOT_K256, 32, 1 << 8, None);
    }

    #[test]
    fn one_hot_k256_d128_roundtrip() {
        packed_trace_one_hot_roundtrip(AKITA_ONE_HOT_K256, 32, 1 << 8, Some(128));
    }
