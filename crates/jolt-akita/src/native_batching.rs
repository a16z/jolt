...
fn single_group_batch<'a, P>(
    ...
    // Post-cutover Akita: each group owns its complete opening point;
    // `OpeningClaims::from_groups` no longer takes a shared point.
    let group =
...
impl BatchOpeningScheme for AkitaNativeBatching {
        ...
        T: Transcript<Challenge = Self::Field>,
    {
        ...
        let openings: Vec<AkitaField> = statement
            ...
            .map(|claim| claim.evaluation.value)
            .collect();
        // Post-cutover Akita: the group owns its complete opening point.
