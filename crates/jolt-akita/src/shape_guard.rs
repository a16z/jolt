...
use akita_types::{
    sumcheck_rounds, AkitaScheduleLookupKey, CommittedGroupParams, DigitRangePlan,
    ExtensionOpeningReductionShape, FoldSchedule, LevelProofShape, NextWitnessBindingShape,
    OpeningClaimsLayout, OpeningScheduleSelection, RecursiveFoldParams, RelationWitnessGeometry,
...
/// Replays the backend verifier's root calculation. The commitment payload is the
...
fn expected_commitment_coeff_len(
    ...
    let relation = RelationWitnessGeometry::for_level(
        root_params,
        layout,
        <AkitaField as akita_field::ExtField<AkitaField>>::EXT_DEGREE,
    )
