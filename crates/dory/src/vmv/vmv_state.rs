...
/// Compute the commitments to the rows of the matrix M that is derived from coeffs `a`.
    ...
    E::G2: Group<Scalar = <E::G1 as Group>::Scalar>,
{
    ...
    let s1 = vmv_state.v_vec; //intermediate vector from L^T × M
    ...
    DoryProverState::new(v1, v2, s1, s2, nu)
}
