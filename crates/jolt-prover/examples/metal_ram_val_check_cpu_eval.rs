...
use jolt_kernels::metal::{
    RamValCheckCpuEvalSample, RamValCheckCpuMetalEvalFixture, RamValCheckEvalResult,
};
...
fn main() -> EvalResult<()> {
    ...
    let evaluator =
        RamValCheckCpuMetalEvalFixture::new(&fixture.witness, fixture.log_t, fixture.log_k, SEED)?;
    let mut oracle = None;
    ...
    let report = json!({
        "schema": "ram_val_check_cpu_metal_v1",
        "schema_version": 1,
        ...
        "fixture": {
            ...
            "increment_source_bytes": shape.increment_source_bytes,
        },
        "shape": {
            ...
            "active_increment_pairs_by_width": shape.active_increment_pairs_by_width,
            "increment_topology_entries_by_level": shape.increment_topology_entries_by_level,
            "increment_topology_bytes": shape.increment_topology_bytes,
            "increment_topology_wall_ns": duration_ns(shape.increment_topology_wall)?,
            "dense_inc_field_bytes": shape.dense_inc_field_bytes,
            "address_eq_bytes": shape.address_eq_bytes,
            "split_lt_bytes": shape.split_lt_bytes,
        },
        "parity": {
            "exact": true,
...
fn run_arm(
    arm: Arm,
    evaluator: &RamValCheckCpuMetalEvalFixture,
    witness: &TraceBackend<OwnedTrace>,
    oracle: &mut Option<RamValCheckEvalResult>,
) -> EvalResult<Value> {
...
fn require_exact(
    ...
    match oracle {
        Some(expected) if expected != result => Err(failure(
            "RAM value-check evaluator output changed between samples",
        )),
        Some(_) => Ok(()),
        None => {
...
fn sample_record(sample: &RamValCheckCpuEvalSample) -> EvalResult<Value> {
    Ok(json!({
