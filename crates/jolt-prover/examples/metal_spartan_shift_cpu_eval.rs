...
use jolt_kernels::metal::{
    SpartanShiftCpuMetalEvalFixture, SpartanShiftEvalResult, SpartanShiftEvalSample,
};
...
fn main() -> EvalResult<()> {
    ...
    let evaluator = SpartanShiftCpuMetalEvalFixture::new(
        &fixture.witness,
        fixture.log_t,
        SEED,
        cli.build_threads,
        cli.high_tile_elements,
        cli.fold_threads,
    )?;
    let mut oracle = None;
    ...
    let report = json!({
        "schema": "spartan_shift_cpu_metal_v1",
        "schema_version": 1,
        ...
        "shape": {
            ...
            "total_resident_bytes": shape.total_resident_bytes,
            "build_unique_bytes": shape.build_unique_bytes,
            "build_coalesced_bytes_with_halo": shape.build_coalesced_bytes_with_halo,
            "fold_unique_bytes": shape.fold_unique_bytes,
            "readback_bytes": shape.readback_bytes,
            "mixed_full_products": shape.mixed_full_products,
            "mixed_half_products": shape.mixed_half_products,
            "fold_half_products": shape.fold_half_products,
            "command_buffers": shape.command_buffers,
            "dispatches": shape.dispatches,
            "unexpanded_pc_max": shape.unexpanded_pc_max,
            "pc_max": shape.pc_max,
            "unexpanded_pc_above_u32": shape.unexpanded_pc_above_u32,
            "pc_above_u32": shape.pc_above_u32,
        },
        "dispatch": {
            "build_threads_per_threadgroup": shape.build_threads_per_threadgroup,
            "high_tile_elements": shape.high_tile_elements,
            "fold_threads_per_threadgroup": shape.fold_threads_per_threadgroup,
            "production_trace_cutoff_elements": shape.production_trace_cutoff_elements,
            "forced_device_below_production_cutoff": shape.forced_device_below_production_cutoff,
        },
        "boundary": {
            "cpu": "row collection, prefix preparation, all messages and binds, midpoint regeneration, finish, output, derived-table and terminal-relation validation",
            "metal": "resident-row alias, prefix and fold allocation/submission/synchronization/readback, all host messages and binds, finish, output, derived-table and terminal-relation validation",
            "excluded_shared_source": "production Stage-1 construction of the already-resident native row planes",
            "prefetch_overlap_credit": false,
            "silent_fallback_allowed": false,
        },
        "parity": {
            "exact": true,
            "scope": "exact round polynomials, final claim, output claims, derived tables, and terminal relation across every arm",
...
fn run_arm(
    arm: Arm,
    evaluator: &SpartanShiftCpuMetalEvalFixture,
    witness: &TraceBackend<'static, OwnedTrace>,
    oracle: &mut Option<SpartanShiftEvalResult>,
) -> EvalResult<Value> {
...
fn require_exact(
    ...
    match oracle {
        Some(expected) if expected != result => Err(failure(
            "Spartan shift evaluator output changed between samples",
        )),
        Some(_) => Ok(()),
        None => {
...
fn sample_record(sample: &SpartanShiftEvalSample) -> EvalResult<Value> {
    Ok(json!({
