...
mod tests {
    ...
    #[test]
    fn counts_nonzero_and_active_ram_zero_entries() {
        let selected = [0, 0, 7, 0, 3];
        assert_eq!(committed_entry_count(&selected, false, 0b0_1011), 2);
        assert_eq!(committed_entry_count(&selected, true, 0b0_1011), 5);
    }
}
