...
fn measure_instruction_read_raf(
    ...
    for round in 0..rounds {
        let challenge = Fr::from_u64(17 + round as u64);
        let start = Instant::now();
        let outcome = kernel.prove_round(bind, round, claim);
        let elapsed = start.elapsed();
        match round.cmp(&address_bits) {
            std::cmp::Ordering::Less => address += elapsed,
            std::cmp::Ordering::Equal => handoff += elapsed,
            std::cmp::Ordering::Greater => cycle += elapsed,
        }
        claim = match outcome {
            Ok(poly) => poly.evaluate(challenge),
            Err(SumcheckError::RoundCheckFailed { actual, .. }) => actual,
            Err(error) => panic!("stage-5 vertical round {round} failed: {error:?}"),
        };
        bind = Some(challenge);
    }
    kernel
        .finish_rounds(Fr::from_u64(17 + rounds as u64))
        .expect("finish the stage-5 read-RAF rounds");

    let start = Instant::now();
    let _ = kernel
        .output_claims(&claims)
        .expect("stage-5 read-RAF output claims");
