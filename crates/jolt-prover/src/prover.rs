...
use jolt_witness::{
    protocols::jolt_vm::{
        ...
        JoltVmStage5InstructionReadRafRows, JoltVmStage6Rows, RV64_LOOKUP_ADDRESS_BITS,
    },
    CommittedWitnessProvider, WitnessProvider,
};

use crate::api::{
    BlindFoldProverBackend, ClearProverBackend, FieldInlineProverWitness, ProofResult, ProverPcs,
};
#[cfg(feature = "zk")]
...
#[cfg(feature = "zk")]
use crate::zk;
use crate::{JoltProverPreprocessing, ProofParameters, ProverConfig, ProverError};
...
pub(crate) fn prove_with_components_inner<PCS, VC, B, W, FI>(
    ...
    FI: FieldInlineProverWitness<PCS::Field>,
{
    ...
    if config.features.zk {
        let (stage_proofs, joint_opening_proof, blindfold_proof) = prove_zk_stages(
            preprocessing,
            public_io,
            witness,
            field_inline_witness,
    ...
    let (stage_proofs, clear_claims, joint_opening_proof) = prove_clear_stages(
        preprocessing,
        public_io,
        witness,
        field_inline_witness,
    ...
    VC: VectorCommitment<Field = PCS::Field>,
{
    ...
    let proof_parameters = required_proof_parameters(config)?;
    ...
    FI: FieldInlineProverWitness<PCS::Field>,
{
    ...
    let formula_dimensions = proof_parameters_formula_dimensions(preprocessing, proof_parameters)?;
    let stage0_config =
        stage0_prepare::prover_config(public_io, proof_parameters, formula_dimensions);
...
fn required_proof_parameters(config: &ProverConfig) -> Result<ProofParameters, ProverError> {
    config
        .proof_parameters
        .ok_or_else(|| ProverError::InvalidProverConfig {
            reason: "proof_parameters is required before proof construction".to_owned(),
        })
}

fn proof_parameters_log_t(proof_parameters: ProofParameters) -> usize {
    proof_parameters.trace_length.trailing_zeros() as usize
}

fn proof_parameters_log_k(proof_parameters: ProofParameters) -> usize {
