...
//! sumcheck, proven with the naive tier at harness scale.
//!
...
//! (see [`OuterRemainderMember`]): the stream round is hand-computed — its
...
//! to match the legacy prover's convention. Every driver (challenge draw,
//! batch head, point derivation, final-claim fold, absorb order) is
//! `jolt-verifier`'s generated code.
//!
//! Brute-force costs are `O(T · |constraints|)` and table memory
//! `O(|inputs|² · T)` — a bring-up path; the streaming-remainder kernel
//! replaces it for real trace lengths.
...
use jolt_sumcheck::{
    prove_batch, prove_uniskip_clear, ClearSumcheckRecorder, ProveRounds, SumcheckError,
    SumcheckProof, SumcheckRecorder,
};
...
use crate::{NaiveSumcheckProver, ProveSumcheck, ProverError};
    ...
    W: WitnessProvider<F, JoltVmNamespace>,
{
    ...
    let (tau_low, tau_high) = tau.split_at(log_t + 1);
    let tau_high = tau_high[0];

    // Cycle-indexed R1CS input tables (big-endian cycle index), in the
    // relation's variable order, plus the per-constraint Az/Bz row values.
    let input_tables = materialize_input_tables::<F, W>(witness, &dimensions)?;
    ...
    let uniskip_poly =
        uniskip_first_round_poly::<F>(tau_high, &eq_table, &az_rows, &bz_rows, log_t)?;
    ...
    let sumchecks = Stage1BatchSumchecks {
        outer_remainder: OuterRemainder::new(dimensions.clone(), tau.clone(), uniskip_challenge),
    };
    ...
    // The remainder member: the stream round is hand-computed (its
    // coefficient structure is QUADRATIC in the stream variable — a product
    // of the two per-group linear forms — so it has no faithful multilinear
    // table); the remaining cycle rounds are a plain naive member.
    let mut member =
        OuterRemainderMember::prepare(log_t, &tau, uniskip_challenge, &matrices, input_tables)?;
        ...
        .map(|&variable| {
            ...
            let values = view
                ...
                .ok_or(ProverError::Unsupported {
                    reason: "stage-1 input oracle view was not materialized",
                })?
                .to_vec();
            Ok(values)
        })
...
/// Brute-force the uni-skip first-round polynomial. The summand's row-node
/// polynomial `t1(Y) = Σ_(s,t) eq(τ_low, (t,s)) · Az(Y,s,t) · Bz(Y,s,t)`
/// vanishes on the 10 in-domain nodes (each row is a satisfied
/// `guard · (left − right) = 0`), so `t1` is interpolated over the 19-point
/// centered domain from the 9 extended-node evaluations; the transmitted
/// polynomial is `LK(τ_high, ·) × t1`.
fn uniskip_first_round_poly<F: Field>(
    tau_high: F,
    eq_table: &[F],
    az_rows: &[Vec<F>],
    bz_rows: &[Vec<F>],
    log_t: usize,
) -> Result<UnivariatePoly<F>, ProverError<F>> {
        ...
        for t in 0..cycles {
            for (s, stream_weights) in weights.iter().enumerate() {
                let az: F = stream_weights
                    .iter()
                    .enumerate()
                    .map(|(row, &w)| w * az_rows[row][t])
                    .sum();
                let bz: F = stream_weights
                    .iter()
                    .enumerate()
                    .map(|(row, &w)| w * bz_rows[row][t])
                    .sum();
                sum += eq_table[(t << 1) | s] * az * bz;
            }
        }
        *value = sum;
    }
    ...
    let t1_coefficients = jolt_poly::lagrange::interpolate_to_coeffs(extended_start, &t1_values);
...
struct OuterRemainderMember<F: Field> {
    log_t: usize,
    kernel: F,
    tau: Vec<F>,
...
impl<F: Field> OuterRemainderMember<F> {
    fn prepare(
        log_t: usize,
        tau: &[F],
        uniskip_challenge: F,
        ...
        let variable_count = input_tables.len();
        ...
        for (index, stream) in [F::zero(), F::one()].into_iter().enumerate() {
            ...
            az_values[index] = row_form(&az_rows);
            bz_values[index] = row_form(&bz_rows);
        ...
        Ok(Self {
            ...
            kernel,
            tau: tau.to_vec(),
            eq_cycle: EqPolynomial::new(tau_low[..log_t].to_vec()).evaluations(),
            tau_stream: tau_low[log_t],
            az_values,
            bz_values,
            az_columns,
            bz_columns,
    ...
    fn build_inner(&mut self, stream_challenge: F) -> Result<(), ProverError<F>> {
        ...
        .with_binding_order(BindingOrder::LowToHigh);
    ...
    > {
        self.inner
            .as_mut()
            .ok_or(ProverError::NotFullyBound {
...
impl<F: Field> ProveRounds<F> for OuterRemainderMember<F> {
        ...
        if round > 0 {
            return self
                ...
                .ok_or(SumcheckError::MissingEvaluationSource {
                    kind: "stage-1 inner naive member",
                })?
