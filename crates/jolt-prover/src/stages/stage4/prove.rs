...
#[cfg(feature = "zk")]
use jolt_verifier::stages::stage4::stage4_output_claim_values;
use jolt_verifier::stages::stage4::{
    append_ram_val_check_gamma_domain_separator,
    append_stage4_opening_claims as append_verifier_stage4_opening_claims,
    stage4_expected_final_claim, stage4_expected_output_claims, stage4_input_claims,
    stage4_opening_points, Stage4ExpectedOutputClaims, Stage4ExpectedOutputRequest,
    Stage4InputClaimRequest, Stage4InputClaims, Stage4OpeningPointRequest, Stage4OpeningPoints,
};
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct Stage4RegularBatchProofOutput<F: Field, Proof> {
...
fn prove_stage4_transparent_sumchecks<F, W, B, T, C>(
    ...
    prefix: &Stage4RegularBatchPrefixOutput<F>,
    transcript: &mut T,
) -> Result<Stage4RegularBatchProofOutput<F, SumcheckProof<F, C>>, ProverError>
where
    F: Field,
    W: WitnessProvider<F, JoltVmNamespace> + JoltVmStage2Rows + JoltVmRegisterReadWriteRows,
    B: SumcheckBackend<F, JoltVmNamespace> + Stage4ReadWriteSumcheckBackend<F>,
...
fn prove_stage4_transparent_sumchecks<F, W, FI, B, T, C>(
    config: Stage4ProverConfig,
    witness: &W,
...
trait Stage4RegularBatchProofRecorder<F: Field> {
    type Proof;
...
struct ClearStage4RegularBatchProofRecorder<F: Field, C> {
    round_polynomials: Vec<jolt_poly::CompressedPoly<F>>,
    _marker: std::marker::PhantomData<C>,
}

impl<F, C> ClearStage4RegularBatchProofRecorder<F, C>
where
    F: Field,
{
    fn new() -> Self {
...
impl<F, C> Stage4RegularBatchProofRecorder<F> for ClearStage4RegularBatchProofRecorder<F, C>
where
    F: Field,
{
...
#[cfg(feature = "zk")]
struct PendingCommittedStage4RegularBatch<'a, F, VC>
where
    F: Field,
    VC: VectorCommitment<Field = F>,
{
...
#[cfg(feature = "zk")]
impl<F, VC> PendingCommittedStage4RegularBatch<'_, F, VC>
where
    F: Field,
    VC: VectorCommitment<Field = F>,
{
...
#[cfg(feature = "zk")]
struct CommittedStage4RegularBatchProofRecorder<'a, F, VC>
where
    F: Field,
    VC: VectorCommitment<Field = F>,
{
...
#[cfg(feature = "zk")]
impl<'a, F, VC> CommittedStage4RegularBatchProofRecorder<'a, F, VC>
where
    F: Field,
    VC: VectorCommitment<Field = F>,
{
...
#[cfg(feature = "zk")]
impl<'a, F, VC> Stage4RegularBatchProofRecorder<F>
    for CommittedStage4RegularBatchProofRecorder<'a, F, VC>
where
    F: Field,
    VC: VectorCommitment<Field = F>,
{
    type Proof = PendingCommittedStage4RegularBatch<'a, F, VC>;

    fn absorb_input_claims<T>(
        &mut self,
        _prefix: &Stage4RegularBatchPrefixOutput<F>,
        _transcript: &mut T,
    ) where
        T: Transcript<Challenge = F>,
    {
    ...
    fn finish(self) -> Result<Self::Proof, ProverError> {
        Ok(PendingCommittedStage4RegularBatch {
    ...
    T: Transcript<Challenge = F>,
{
    prove_stage4_specialized_regular_batch_sumcheck_with_recorder(
        ...
        backend,
        transcript,
        ClearStage4RegularBatchProofRecorder::<F, C>::new(),
    )
}

#[cfg(not(feature = "field-inline"))]
fn prove_stage4_specialized_regular_batch_sumcheck_with_recorder<F, W, B, T, R>(
    input: Stage4RegularBatchRunInput<'_, F>,
    witness: &W,
    backend: &mut B,
    transcript: &mut T,
    mut proof_recorder: R,
) -> Result<Stage4RegularBatchProofOutput<F, R::Proof>, ProverError>
where
    F: Field,
    W: WitnessProvider<F, JoltVmNamespace> + JoltVmStage2Rows + JoltVmRegisterReadWriteRows,
    B: Stage4ReadWriteSumcheckBackend<F>,
    T: Transcript<Challenge = F>,
    R: Stage4RegularBatchProofRecorder<F>,
{
    ...
    proof_recorder.absorb_input_claims(prefix, transcript);
    ...
    VC: VectorCommitment<Field = F>,
{
    let batch = prove_stage4_specialized_regular_batch_sumcheck_with_recorder(
        ...
        backend,
        transcript,
        CommittedStage4RegularBatchProofRecorder::<F, VC>::new(vc_setup)?,
    )?;

    let output_claim_values = stage4_output_claim_values(&batch.output_openings);
    let claims = Stage4Claims {
    ...
    let built = batch.proof.finish(&output_claim_values, transcript)?;
...
#[cfg(feature = "field-inline")]
fn prove_stage4_field_inline_specialized_regular_batch_sumcheck_with_recorder<F, W, FI, B, T, R>(
    input: Stage4RegularBatchRunInput<'_, F>,
    witness: &W,
    field_inline_witness: &FI,
    backend: &mut B,
    ...
    R: Stage4RegularBatchProofRecorder<F>,
{
    ...
    let field_register_request = SumcheckFieldRegistersReadWriteStateRequest::new(
        ...
        field_register_read_write_rows(field_inline_witness)?,
...
where
    ...
    T: Transcript<Challenge = F>,
{
    append_verifier_stage4_opening_claims(
        transcript,
        claims.advice.untrusted.is_some(),
        claims.advice.trusted.is_some(),
        claims,
    )
    .map_err(|error| invalid_sumcheck_output(error.to_string()))
}
