...
fn stage2_field_inline_committed_proof_component_produces_native_verifier_output(
    ...
    let stage1 = prove_stage1_committed_proof_component::<Fr, _, _, _, _, Pedersen<Bn254G1>>(
        Stage1ProverInput::new(Stage1ProverConfig::new(4), &witness, &field_inline_witness),
        &mut backend,
        &mut prover_transcript,
        &vc_setup,
    )?;
    let stage2 = prove_stage2_committed_proof_component::<Fr, _, _, _, _, Pedersen<Bn254G1>>(
