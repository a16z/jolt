...
//! every limb column empty, the limb object absent by the claim-gated
//! presence rule), each over both kernel backends with wire equality.
//! Tamper (all must reject): a shifted digit column inside the committed
//! limb object, a limb leaf-claim perturbation, a limb-commitment
//! layout-digest mutation, the limb object stripped from an FR-active proof,
//! and the reconstruction round polynomial at the FR member's position.
...
mod support {
    ...
    use jolt_prover_legacy::zkvm::packed::{
        akita_verifier_preprocessing, AkitaField, AkitaJoltProof, AkitaPackedScheme, AkitaScheme,
    ...
    /// Everything a tamper needs to forge a limb commitment through the real
    /// commit path: the prover-side setup, the canonical plan, and the honest
    /// per-cycle `FieldRdInc` values.
    pub struct ProveOutput {
        pub verifier_preprocessing: jolt_verifier::JoltVerifierPreprocessing<AkitaScheme, AkitaVc>,
        pub public_io: JoltDevice,
        pub proof: Proof,
        pub object_setup: <AkitaScheme as VerifierCommitmentScheme>::ProverSetup,
        pub config: ProverConfig,
        pub rd_inc: Vec<AkitaField>,
    }

    /// Prove `guest` FR-on with the modular packed prover.
    pub fn prove_fr(
        guest: FrGuest,
        ...
        // (the FR limb object commits under the same setup — same packed
        // arity by the norm-budget geometry).
        ...
        ProveOutput {
            ...
            public_io,
            proof,
            object_setup: prover_preprocessing.pcs_setup,
            config,
            rd_inc,
        }
    }
    ...
    /// The honest per-cycle limb-column selected rows (the shared encoder,
    /// row-major over `(cycle, column)`) — the tamper tests' base material
    /// for forging limb commitments through the real commit path.
    pub fn limb_selected_rows(
        output: &ProveOutput,
    ) -> (
        jolt_claims::protocols::field_inline::lattice::FieldIncLimbPackingPlan,
        Vec<u8>,
        usize,
    ) {
        use jolt_claims::lattice::BalancedIncChunking;
        use jolt_claims::protocols::field_inline::lattice::{canonical_limbs, column_selected_row};

        let log_t = output.config.trace_length.ilog2() as usize;
        let plan = limb_plan::<AkitaField>(log_t, output.config.one_hot_config)
            .expect("canonical limb plan");
        let chunking = BalancedIncChunking::new(plan.chunk_width()).expect("chunking");
        let columns = plan.packing().ids().to_vec();
        let mut selected = vec![0u8; output.rd_inc.len() * columns.len()];
        for (value, row) in output
            .rd_inc
            .iter()
            .zip(selected.chunks_exact_mut(columns.len()))
        {
            let limbs = canonical_limbs(value);
            for (slot, column) in row.iter_mut().zip(&columns) {
                let hot = column_selected_row(chunking, &limbs, *column).expect("limb column");
                *slot = u8::try_from(hot).expect("hot row fits the one-hot K");
            }
        }
        let num_columns = columns.len();
        (plan, selected, num_columns)
    }

    /// Commit `selected` limb rows under `digest` through the real packed
    /// commit path, returning the commitment a tamper splices into a proof.
    pub fn commit_limb_rows(
        output: &ProveOutput,
        plan: &jolt_claims::protocols::field_inline::lattice::FieldIncLimbPackingPlan,
        digest: [u8; 32],
        selected: Vec<u8>,
        num_columns: usize,
        ...
        impl TraceOneHotRows for ForgedRows {
        ...
        let num_rows = selected.len() / num_columns;
        let (commitment, _hint) = <AkitaScheme as TraceOneHotCommitment>::commit_trace_one_hot(
            &output.object_setup,
            digest,
...
mod clear {
    ...
    fn akita_field_inline_eqpoly_proof_is_accepted() {
        ...
        for (label, backend) in support::backends() {
            ...
            assert!(
                ...
                "packed FR proofs must carry the limb commitment ({label})",
            );
            assert!(
                output.proof.joint_opening_proof.field_inc_limbs.is_some(),
                "packed FR proofs must carry the limb opening ({label})",
            );
            assert!(matches!(output.proof.claims, JoltProofClaims::Clear(_)));
            support::verify_full(
    ...
    /// FR instructions — `FieldRdInc` identically zero, every limb column
    /// empty — still proves and verifies under the packed FR protocol, with
    /// the limb object absent (the claim-gated presence rule: an all-empty
    /// one-hot object is unprovable under the catalogued fold schedules).
    #[test]
    fn akita_field_inline_inactive_muldiv_proof_is_accepted() {
        ...
        for (label, backend) in support::backends() {
            ...
            assert!(
                output.proof.field_inc_limbs_commitment.is_none(),
                "a zero FieldRdInc admits no limb object ({label})",
            );
            assert!(output.proof.joint_opening_proof.field_inc_limbs.is_none());
            assert!(output
                .rd_inc
                .iter()
    ...
    /// The packed FR tamper matrix: one honest proof, four mutations on
    /// fresh clones, every one rejected.
    #[test]
    fn akita_field_inline_tampered_proofs_are_rejected() {
        ...
        let (plan, honest_rows, num_columns) = support::limb_selected_rows(&output);
        let one_hot_k = 1usize << plan.chunk_width();

        // Shift one digit column's hot rows: same digest and arity, a
        // different committed polynomial — rejection comes from the packed
        // opening itself, not the metadata gate.
        let shifted_commitment = {
            let mut rows = honest_rows.clone();
            let column = (0..num_columns)
                .find(|column| rows.chunks_exact(num_columns).any(|row| row[*column] != 0))
                .expect("the eq-MLE trace commits at least one hot limb cell");
            for row in rows.chunks_exact_mut(num_columns) {
                let hot = usize::from(row[column]);
                if hot != 0 {
                    let shifted = if hot + 1 < one_hot_k {
                        hot + 1
                    } else {
                        hot - 1
                    };
                    row[column] = u8::try_from(shifted).expect("shifted row fits");
                }
            }
            assert_ne!(rows, honest_rows);
            support::commit_limb_rows(&output, &plan, plan.layout_digest(), rows, num_columns)
        };

        // The honest rows under a corrupted layout digest: the metadata gate
        // must reject before any opening verification.
        let wrong_digest_commitment = {
            let mut digest = plan.layout_digest();
            digest[0] ^= 0x01;
            support::commit_limb_rows(&output, &plan, digest, honest_rows, num_columns)
        };

        type Tamper = (&'static str, Box<dyn Fn(&mut support::Proof)>);
        let tampers: Vec<Tamper> = vec![
            (
                "shifted digit column in the committed limb object",
                Box::new(move |proof| {
                    proof.field_inc_limbs_commitment = Some(shifted_commitment.clone());
                }),
            ),
            (
                "limb leaf-claim perturbation",
                Box::new(move |proof| {
                    ...
                    let limbs = claims
                        ...
                        .field_inc_limbs
                        .as_mut()
                        .expect("packed FR proof carries limb reconstruction claims");
                    let column = limbs
                        .columns
                        .first_mut()
                        .expect("limb reconstruction has columns");
                    *column += one;
                }),
            ),
            (
                "limb commitment layout-digest mutation",
                Box::new(move |proof| {
                    proof.field_inc_limbs_commitment = Some(wrong_digest_commitment.clone());
                }),
            ),
            (
                "limb object stripped from an FR-active proof",
                Box::new(|proof| {
                    proof.field_inc_limbs_commitment = None;
                    proof.joint_opening_proof.field_inc_limbs = None;
                }),
            ),
            (
                "reconstruction round poly at the FR member's position",
                Box::new(|proof| {
                    let Some(SumcheckProof::Clear(ClearProof::Compressed(batch))) =
                        &mut proof.stages.reconstruction_sumcheck_proof
                    else {
                        panic!("clear compressed reconstruction batch expected");
                    };
                    // The eq-MLE guest has no advice and a full program, so
                    // the FR limb member is the batch's only instance — every
                    // round is its position.
                    let round = batch
                        .round_polynomials
                        .first_mut()
                        .expect("reconstruction batch has rounds");
                    *round = CompressedPoly::new(vec![AkitaField::from_u64(7)]);
                }),
            ),
        ];
