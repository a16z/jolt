...
use jolt_kernels::metal::solinas::{InstructionInputStorageInitialization, SolinasMetal};
use serde_json::json;
...
fn initialization_mode() -> EvalResult<InstructionInputStorageInitialization> {
    match env::var("JOLT_METAL_INSTRUCTION_INPUT_STORAGE_INITIALIZATION")
        .as_deref()
        .unwrap_or("minimal")
    {
        "minimal" => Ok(InstructionInputStorageInitialization::Minimal),
        "full" => Ok(InstructionInputStorageInitialization::Full),
        _ => Err("storage initialization must be `minimal` or `full`".into()),
    }
}
...
fn main() -> EvalResult<()> {
    ...
    if !(3..=28).contains(&log_n) || cutoff_log2 < 1 || cutoff_log2 + 2 > log_n {
    ...
    let mut sequence =
        workload.prepare_sequence_with_storage_initialization(&context, dispatch, mode)?;
    ...
    let expected_initialization_bytes = match mode {
        InstructionInputStorageInitialization::Minimal => 6 * 16,
        InstructionInputStorageInitialization::Full => storage_bytes,
        InstructionInputStorageInitialization::Lazy => {
            return Err("the cold evaluator excludes lazy allocation-only storage".into());
        }
    };
    ...
    let guards = json!({
        ...
        "storage_initialization_mode_exact": initialization.mode == mode,
        "storage_initialization_bytes_exact": initialization.bytes == expected_initialization_bytes,
        "storage_initialization_one_command_six_buffers": initialization.device_buffers == 6,
        "storage_initialization_completed_before_member": sequence_preparation >= initialization.wall,
        "storage_initialization_timestamps_valid": initialization.gpu_active > Duration::ZERO && initialization.gpu_active <= initialization.wall,
        ...
        "resident_rows_stable": hybrid.resident_rows_stable,
    ...
    if !all_exact {
        return Err("InstructionInput residency evaluator guard failed".into());
    }
    ...
    let output = json!({
        ...
        "metrics": {
            ...
            "resident_member_wall_ns": duration_ns(hybrid.wall.saturating_sub(hybrid.reset))?,
        ...
        "fingerprint": {
            ...
            "storage_initialization": mode.as_str(),
