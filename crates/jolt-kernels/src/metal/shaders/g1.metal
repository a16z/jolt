...
// Stage-0 commit staging derived from the session-shared trace record.
// `rows` is the repr(C) InstructionCycleRow view (12 uint words per cycle):
// lookup-index limbs at [0..4), mapped-PC+1 at [4..6), remapped-RAM+1 at
// [6..8). One-hot metadata is `(family, shift)`, where family 0/1/2 selects
// instruction/bytecode/RAM. Increment metadata 0/1 selects rd/RAM values.
// Outputs are column-major over this superchunk; UINT_MAX is a cold address.
struct CommitExtractParams {
    uint count;
    uint cycle_base;
    uint one_hot_columns;
    uint increment_columns;
    uint one_hot_mask;
};

inline ulong jk_load_u64(device const uint* words) {
    return (ulong)words[0] | ((ulong)words[1] << 32);
}

inline uint jk_chunk_u128(device const uint* words, uint shift, uint mask) {
    uint word = shift >> 5;
    uint bit = shift & 31u;
    uint value = words[word] >> bit;
    if (bit != 0u && word < 3u) {
        value |= words[word + 1u] << (32u - bit);
    }
    return value & mask;
}

kernel void jk_commit_extract(
    device const uint* rows [[buffer(0)]],
    device const ulong* rd_pre [[buffer(1)]],
    device const ulong* rd_post [[buffer(2)]],
    device const ulong* ram_pre [[buffer(3)]],
    device const ulong* ram_post [[buffer(4)]],
    device const uint* one_hot_meta [[buffer(5)]],
    device const uint* increment_meta [[buffer(6)]],
    device uint* hot [[buffer(7)]],
    device ulong2* increments [[buffer(8)]],
    constant CommitExtractParams& p [[buffer(9)]],
    uint gid [[thread_position_in_grid]])
{
    uint hot_count = p.one_hot_columns * p.count;
    if (gid < hot_count) {
        uint column = gid / p.count;
        uint local_cycle = gid - column * p.count;
        uint cycle = p.cycle_base + local_cycle;
        device const uint* row = rows + cycle * 12u;
        uint family = one_hot_meta[2u * column];
        uint shift = one_hot_meta[2u * column + 1u];
        uint value = 0xffffffffu;
        if (family == 0u) {
            value = jk_chunk_u128(row, shift, p.one_hot_mask);
        } else {
            uint plus_one_offset = family == 1u ? 4u : 6u;
            ulong plus_one = jk_load_u64(row + plus_one_offset);
            if (plus_one != 0u) {
                ulong address = plus_one - 1u;
                value = (uint)(address >> shift) & p.one_hot_mask;
            }
        }
        hot[gid] = value;
    }

    uint increment_count = p.increment_columns * p.count;
    if (gid < increment_count) {
        uint column = gid / p.count;
        uint local_cycle = gid - column * p.count;
        uint cycle = p.cycle_base + local_cycle;
        bool rd = increment_meta[column] == 0u;
        ulong before = rd ? rd_pre[cycle] : ram_pre[cycle];
        ulong after = rd ? rd_post[cycle] : ram_post[cycle];
        ulong2 value;
        if (after >= before) {
            value = ulong2(after - before, 0u);
        } else {
            ulong magnitude = before - after;
            value = ulong2(~magnitude + 1u, 0xfffffffffffffffful);
        }
        increments[gid] = value;
    }
}
