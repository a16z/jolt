...
struct JkShiftParams {
    uint outer;
    uint inner;
};

kernel void jk_shift_q(
    device const ulong* upc [[buffer(0)]],
    device const ulong* pc [[buffer(1)]],
    device const uint* flags [[buffer(2)]],
    device const uint* outer_0 [[buffer(3)]],
    device const uint* outer_1 [[buffer(4)]],
    device const uint* product_0 [[buffer(5)]],
    device const uint* product_1 [[buffer(6)]],
    device const uint* gammas [[buffer(7)]],
    device uint* output [[buffer(8)]],
    constant JkShiftParams& p [[buffer(9)]],
    uint gid [[thread_position_in_grid]])
{
    if (gid >= p.outer) return;
    Fr256 q0 = fr_zero();
    Fr256 q1 = fr_zero();
    Fr256 q2 = fr_zero();
    Fr256 q3 = fr_zero();
    Fr256 gamma1 = fr_load(gammas, 1u);
    Fr256 gamma2 = fr_load(gammas, 2u);
    Fr256 gamma3 = fr_load(gammas, 3u);
    for (uint hi = 0u; hi < p.inner; hi++) {
        uint j = hi * p.outer + gid;
        uint f = flags[j];
        Fr256 value = jk_fr_from_i192(jk_i192_u64(upc[j]));
        value = fr_add(value, fr_mont_mul(gamma1, jk_fr_from_i192(jk_i192_u64(pc[j]))));
        if (jk_outer_flag(f, 7u)) value = fr_add(value, gamma2);
        if (jk_outer_flag(f, 12u)) value = fr_add(value, gamma3);
        q0 = fr_add(q0, fr_mont_mul(fr_load(outer_0, hi), value));
        q1 = fr_add(q1, fr_mont_mul(fr_load(outer_1, hi), value));
        if (!jk_outer_flag(f, 21u)) {
            q2 = fr_add(q2, fr_load(product_0, hi));
            q3 = fr_add(q3, fr_load(product_1, hi));
        }
    }
    Fr256 gamma4 = fr_load(gammas, 4u);
    fr_store(output, gid, q0);
    fr_store(output, p.outer + gid, q1);
    fr_store(output, 2u * p.outer + gid, fr_mont_mul(gamma4, q2));
    fr_store(output, 3u * p.outer + gid, fr_mont_mul(gamma4, q3));
}

kernel void jk_shift_refold(
    device const ulong* upc [[buffer(0)]],
    device const ulong* pc [[buffer(1)]],
    device const uint* flags [[buffer(2)]],
    device const uint* eq_prefix [[buffer(3)]],
    device uint* output [[buffer(4)]],
    constant JkShiftParams& p [[buffer(5)]],
    uint gid [[thread_position_in_grid]])
{
    if (gid >= p.outer) return;
    Fr256 folds[5] = {fr_zero(), fr_zero(), fr_zero(), fr_zero(), fr_zero()};
    uint base = gid * p.inner;
    for (uint offset = 0u; offset < p.inner; offset++) {
        uint j = base + offset;
        uint f = flags[j];
        Fr256 eq = fr_load(eq_prefix, offset);
        folds[0] = fr_add(folds[0], fr_mont_mul(eq, jk_fr_from_i192(jk_i192_u64(upc[j]))));
        folds[1] = fr_add(folds[1], fr_mont_mul(eq, jk_fr_from_i192(jk_i192_u64(pc[j]))));
        if (jk_outer_flag(f, 7u)) folds[2] = fr_add(folds[2], eq);
        if (jk_outer_flag(f, 12u)) folds[3] = fr_add(folds[3], eq);
        if (jk_outer_flag(f, 21u)) folds[4] = fr_add(folds[4], eq);
    }
    for (uint lane = 0u; lane < 5u; lane++) {
        fr_store(output, 5u * gid + lane, folds[lane]);
    }
}
