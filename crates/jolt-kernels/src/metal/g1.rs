...
/// `bases[indices[seg_starts[t] .. seg_starts[t+1]]]` and writes Jacobian
/// result `t`. Synchronous; results are valid in `out` after return.
///
/// `seg_starts` must be a nondecreasing prefix array of `n_segs + 1`
/// entries; `out` must hold `n_segs * JAC_U32S` u32s.
pub fn g1_seg_sum_dispatch(
    ctx: &MetalContext,
    bases: &DeviceBuffer<'_>,
    indices: &DeviceBuffer<'_>,
    seg_starts: &DeviceBuffer<'_>,
    out: &DeviceBuffer<'_>,
    n_segs: usize,
) -> Result<(), MetalError> {
    debug_assert!(seg_starts.len_bytes() >= (n_segs + 1) * 4);
    debug_assert!(out.len_bytes() >= n_segs * JAC_U32S * 4);
    assert!(n_segs <= u32::MAX as usize, "segment count overflows u32");
    ctx.run_once(
        KernelId::G1SegSum,
        &[n_segs as u32],
        &[bases, indices, seg_starts, out],
        n_segs,
    )
}
...
pub fn g1_seg_sums(
    ...
    let starts_buf = ctx.wrap_slice(seg_starts)?;
