...
impl MetalHammingBooleanityKernel {
    ...
    /// The fused device round: fold `H` (optional) and accumulate the two
    /// Gruen sums, one dispatch, one command buffer, one wait. The eq levels
    /// are the CURRENT (post-`eq.bind`) ones — the caller binds eq first.
    fn device_round(
        &self,
        ...
        pass.dispatch(
            ...
            &params,
            &[&h_cur, &h_nxt, &e_in_buffer, &e_out_buffer, &partials],
            groups,
        );
        pass.run()?;
