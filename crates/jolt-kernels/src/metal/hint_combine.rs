...
fn combine_rows_device(
    ...
    // Canonical integer limbs of each live hint's scalar, plus the highest
    // set bit across them (the kernel's shared bit sweep starts there).
    let mut scalar_limbs: Vec<u32> = Vec::with_capacity(num_hints * FR_U32_LIMBS);
    ...
    for &i in &order {
        ...
        for (word_index, word) in big.0.iter().enumerate() {
            ...
            scalar_limbs.push(lo);
            scalar_limbs.push(hi);
    ...
    // Flatten hint-major and batch-normalize to affine; identity rows lower
    // to the (0, 0) sentinel the kernel skips. Only nonzero points reach
    // `normalize_batch`, so its inversion never sees a zero Z.
    let flat: Vec<G1Projective> = order
        .iter()
        .flat_map(|&i| hints[i].row_commitments.iter().map(|p| p.into_inner()))
        .collect();
    let nonzero: Vec<G1Projective> = flat.iter().copied().filter(|p| !p.is_zero()).collect();
    let normalized = G1Projective::normalize_batch(&nonzero);
    let mut normalized_iter = normalized.into_iter();
    let points: Vec<G1Affine> = flat
        .iter()
        .map(|p| {
            if p.is_zero() {
