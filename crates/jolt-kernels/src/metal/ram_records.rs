//! The Metal RAM families' record model: one typed trace walk producing the
//! sparse `(K × T)` RAM view (remapped address or no-access sentinel, pre- and
//! post-access values, increment activity, compact RA records), parked in the
//! [`ProofSession`] so one walk serves stages 2, 4, 5 and 6b.
//!
//! The CPU tier moved to session-shared columns (`optimized::ram_trace`) that
//! no longer materialize these records; the Metal kernels keep their own
//! derivation here, sourced from the same witness plane.
