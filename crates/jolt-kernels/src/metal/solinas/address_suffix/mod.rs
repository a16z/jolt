...
const TILE_PIPELINE: &str = "solinas_address_suffix_one_tile";
const FINALIZE_PIPELINE: &str = "solinas_address_suffix_one_finalize";
...
pub struct AddressSuffixOneSums {
    values: Vec<Fp128>,
}

impl AddressSuffixOneSums {
    pub fn as_flat_slice(&self) -> &[Fp128] {
        &self.values
    }

    pub fn table(&self, table: usize) -> Option<&[Fp128]> {
        (table < ADDRESS_SUFFIX_TABLES)
            .then(|| &self.values[table * ADDRESS_SUFFIX_BINS..(table + 1) * ADDRESS_SUFFIX_BINS])
    }
}

impl SolinasMetal {
    pub fn prepare_address_suffix_one(
        &self,
        rows: &[AddressRafScanRow],
        weights: &[Fp128],
        config: AddressRafScanConfig,
    ) -> Result<AddressSuffixOneInvocation<'_>, MetalError> {
        if rows.is_empty() {
        ...
        let mut keys = Vec::with_capacity(selected_rows);
        let mut table_major_weights = Vec::with_capacity(selected_rows);
        let mut jobs = Vec::new();
        let mut table_jobs = Vec::with_capacity(ADDRESS_SUFFIX_TABLES);
        for (table, bucket) in buckets.iter().enumerate() {
            let job_start =
                u32::try_from(jobs.len()).map_err(|_| MetalError::InputTooLong(jobs.len()))?;
            let bucket_start = keys.len();
            for &row in bucket {
                let row = row as usize;
                keys.push(((rows[row].lookup_index() >> config.suffix_len) & 0xff) as u16);
            ...
            table_jobs.push(AddressSuffixTableJobs {
                start: job_start,
                end: u32::try_from(jobs.len()).map_err(|_| MetalError::InputTooLong(jobs.len()))?,
            });
        }
        if jobs.is_empty() {
        ...
        if finalize_limits.max_total_threads_per_threadgroup < ADDRESS_SUFFIX_BINS {
            return Err(MetalError::InvalidThreadgroupWidth {
                requested: ADDRESS_SUFFIX_BINS,
        ...
        let output_elements = ADDRESS_SUFFIX_TABLES * ADDRESS_SUFFIX_BINS;
        let partial_bytes = byte_length::<Fp128>(partial_elements)?;
        let output_bytes = byte_length::<Fp128>(output_elements)?;
        for requested in [
            byte_length::<u16>(keys.len())?,
            byte_length::<Fp128>(table_major_weights.len())?,
            byte_length::<AddressSuffixJob>(jobs.len())?,
            byte_length::<AddressSuffixTableJobs>(table_jobs.len())?,
            partial_bytes,
            output_bytes,
        ] {
        ...
        Ok(AddressSuffixOneInvocation {
            ...
            buffers: AddressSuffixBuffers {
                ...
                table_jobs: buffer_from_slice(&self.device, &table_jobs),
                partials: self
                    .device
...
impl AddressSuffixOneInvocation<'_> {
    ...
    pub const fn intermediate_partial_bytes(&self) -> u64 {
        self.jobs as u64 * ADDRESS_SUFFIX_BINS as u64 * size_of::<Fp128>() as u64
    }
    ...
    pub fn execute_timed(&self) -> Result<Duration, MetalError> {
        autoreleasepool(|| {
            ...
            tile.set_buffer(3, Some(&self.buffers.partials), 0);
            tile.set_threadgroup_memory_length(
                0,
                (ADDRESS_SUFFIX_BINS * ADDRESS_SUFFIX_ACCUMULATOR_WORDS * size_of::<u32>()) as u64,
            );
            ...
            finalize.set_buffer(1, Some(&self.buffers.table_jobs), 0);
            ...
            finalize.dispatch_thread_groups(
                ...
                MTLSize {
                    width: ADDRESS_SUFFIX_BINS as u64,
                    height: 1,
                    depth: 1,
                },
            );
    ...
    pub fn read_output(&self) -> Result<AddressSuffixOneSums, MetalError> {
        if !self.completed.get() {
            return Err(MetalError::NotExecuted);
        }
        let elements = ADDRESS_SUFFIX_TABLES * ADDRESS_SUFFIX_BINS;
        ...
        self.context
            .validate_inputs("address suffix output", values)?;
        Ok(AddressSuffixOneSums {
...
const _: () = assert!(size_of::<AddressSuffixJob>() == 16);
const _: () = assert!(size_of::<AddressSuffixTableJobs>() == 8);
...
mod tests {
    ...
    #[test]
    fn suffix_one_matches_jolt_field_for_every_table() {
        let mut state = 0x5aff_1ce0_1234_5678;
        ...
        for suffix_len in [0, 56, 64, 120] {
            let invocation = context
                .prepare_address_suffix_one(
                    &rows,
                    &weights,
    ...
    fn oracle(rows: &[AddressRafScanRow], weights: &[Fp128], suffix_len: u32) -> Vec<Fp128> {
        let mut output = vec![AkitaField::zero(); ADDRESS_SUFFIX_TABLES * ADDRESS_SUFFIX_BINS];
        for (&row, &weight) in rows.iter().zip(weights) {
            let table = row.table_index().unwrap();
            let chunk = ((row.lookup_index() >> suffix_len) as usize) & (ADDRESS_SUFFIX_BINS - 1);
            output[table * ADDRESS_SUFFIX_BINS + chunk] += weight.into_jolt_field::<AkitaField>();
        }
