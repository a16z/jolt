...
kernel void solinas_bytecode_address_sparse_worker_5_4(
    ...
    uint lane [[thread_index_in_simdgroup]],
    uint simdgroup [[simdgroup_index_in_threadgroup]],
    uint item_index [[threadgroup_position_in_grid]])
{
    ...
    for (uint offset = tid; offset < item.count; offset += 256u) {
    ...
    for (uint stage = 0u; stage < BYTECODE_ADDRESS_WORKER_BASE_STAGES; stage++) {
        ...
        if (lane == 0u) {
            bytecode_address_major_store_scratch(scratch, simdgroup, stage, sum);
        }
    }
    ...
    if (simdgroup == 0u && lane < BYTECODE_ADDRESS_WORKER_BASE_STAGES) {
        ...
        partials[lane * params.work_items + item_index] = solinas_mul_wide(
            total,
            e_hi[lane * params.outer_length + outer]);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (uint local_stage = 0u;
        ...
        if (lane == 0u) {
            bytecode_address_major_store_scratch(scratch, simdgroup, local_stage, sum);
        }
    }
