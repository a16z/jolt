...
pub const RAM_RAF_TARGET_LOG_T: usize = 26;
pub const RAM_RAF_TARGET_ROWS: usize = 1 << RAM_RAF_TARGET_LOG_T;
pub const RAM_RAF_TARGET_CPU_NS: u64 = 74_870_252;
pub const RAM_RAF_TARGET_CPU_PREPARE_NS: u64 = 71_039_375;
pub const RAM_RAF_TARGET_CPU_TAIL_NS: u64 = 3_860_836;
pub const RAM_RAF_TARGET_FIVE_X_NS: u64 = 14_974_050;
pub const RAM_RAF_PURSUIT_NS: u64 = 11_500_000;
pub const RAM_RAF_FOLD_REDESIGN_NS: u64 = 8_500_000;
pub const RAM_RAF_FIXED_PROJECTION_NS: u64 = 500_000;

pub const M4_MAX_FIELD_PRODUCTS_PER_SECOND: u64 = 18_100_000_000;
pub const M4_MAX_COPY_BYTES_PER_SECOND: u64 = 451_701_710_520;

const FIELD_BYTES: usize = 16;
const ATOMIC_RMW_BYTES_PER_SUBTOTAL: usize = RAM_RAF_ACCUMULATOR_WORDS * 8;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct RamRafMeasuredResult {
    pub complete_member_ns: u64,
    pub fold_and_finalize_ns: u64,
    pub one_hot_fold_ns: u64,
    pub random_fold_ns: u64,
...
impl RamRafMeasuredResult {
    pub fn assess(self) -> RamRafDecision {
        if !self.parity_passed {
            return RamRafDecision::RejectParity;
        }
        if self.invalid_rows != 0 || self.unsupported_dispatches != 0 {
            return RamRafDecision::RejectDispatch;
        }
        if self.fold_and_finalize_ns > RAM_RAF_FOLD_REDESIGN_NS {
            return RamRafDecision::RedesignFold;
        }
        if self.random_fold_ns != 0 && self.one_hot_fold_ns > self.random_fold_ns.saturating_mul(2)
        {
            return RamRafDecision::RedesignAtomicContention;
        }
        if self.register_spill_bytes.saturating_mul(10) > self.modeled_dram_bytes {
            return RamRafDecision::RedesignRegisterPressure;
        }
        if self.complete_member_ns.saturating_mul(5) > RAM_RAF_TARGET_CPU_NS {
            return RamRafDecision::RejectFiveX;
        }
        if self.complete_member_ns > RAM_RAF_PURSUIT_NS {
            return RamRafDecision::ContinueTowardPursuit;
        }
        RamRafDecision::Promote
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RamRafDecision {
    Promote,
    ContinueTowardPursuit,
    RedesignFold,
    RedesignAtomicContention,
    RedesignRegisterPressure,
    RejectFiveX,
    RejectParity,
    RejectDispatch,
}
...
pub struct RamRafTopology {
    ...
    pub live_subtotals: usize,
    pub maximum_subtotal_rows: usize,
}

impl RamRafTopology {
    /// Profiling census only; running it in the timed member must be charged.
    pub fn from_addresses(raw: &[u32], shape: RamRafShape) -> Result<Self, RamRafError> {
        if raw.len() != shape.rows {
        ...
        let mut counts = vec![0u32; shape.addresses];
        let mut accessed_rows = 0usize;
        let mut live_subtotals = 0usize;
        let mut maximum_subtotal_rows = 0usize;
        for block in raw.chunks_exact(shape.inner_length) {
            counts.fill(0);
            for &address in block {
                ...
                if address_index >= shape.addresses {
                    return Err(RamRafError::AddressOutsideDomain { address });
                }
                counts[address_index] += 1;
                accessed_rows += 1;
            }
            for count in counts.iter().copied().filter(|&count| count != 0) {
                live_subtotals += 1;
                maximum_subtotal_rows = maximum_subtotal_rows.max(count as usize);
            }
        }
        Ok(Self {
            accessed_rows,
            live_subtotals,
            maximum_subtotal_rows,
        })
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RamRafEvidence {
    FrozenCpuMeasurement,
    AnalyticalProjection,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct RamRafCostModel {
    shape: RamRafShape,
    live_subtotals: usize,
}

impl RamRafCostModel {
    pub fn new(shape: RamRafShape, live_subtotals: usize) -> Result<Self, RamRafError> {
        let maximum = shape.maximum_live_subtotals()?;
        if live_subtotals > maximum {
            return Err(RamRafError::TooManyLiveSubtotals {
                maximum,
                got: live_subtotals,
            });
        }
    ...
    pub fn address_scan_bytes(self) -> Result<u64, RamRafError> {
        checked_bytes_u64(
            "address scan bytes",
            self.shape.rows,
            self.shape.tiles * size_of::<u32>(),
        )
    }

    pub fn global_atomic_bytes(self) -> Result<u64, RamRafError> {
        checked_bytes_u64(
            "global atomic bytes",
            self.live_subtotals,
            ATOMIC_RMW_BYTES_PER_SUBTOTAL,
        )
    }

    pub fn e_lo_shader_logical_bytes(self) -> Result<u64, RamRafError> {
        checked_bytes_u64("E_lo shader-logical bytes", self.shape.rows, FIELD_BYTES)
    }

    pub fn upload_bytes(self) -> Result<u64, RamRafError> {
        checked_bytes_u64("upload bytes", self.shape.rows, size_of::<u32>())
    }

    pub fn products(self) -> u64 {
        self.live_subtotals as u64
    }

    pub fn projection(
        self,
        include_private_upload: bool,
        e_lo_cache_resident: bool,
    ) -> Result<RamRafProjection, RamRafError> {
        let dominant_bytes = self
            .address_scan_bytes()?
            .checked_add(self.global_atomic_bytes()?)
            .ok_or(RamRafError::SizeOverflow {
                label: "dominant traffic bytes",
            })?;
        let modeled_bytes = if e_lo_cache_resident {
            dominant_bytes
        } else {
            dominant_bytes
                .checked_add(self.e_lo_shader_logical_bytes()?)
                .ok_or(RamRafError::SizeOverflow {
                    label: "modeled traffic bytes",
                })?
        };
        let traffic_floor_ns = ceil_rate_ns(modeled_bytes, M4_MAX_COPY_BYTES_PER_SECOND)?;
        let product_floor_ns = ceil_rate_ns(self.products(), M4_MAX_FIELD_PRODUCTS_PER_SECOND)?;
        let fold_floor_ns = traffic_floor_ns.max(product_floor_ns);
        let fold_eighty_percent_ns = ceil_ratio(fold_floor_ns, 5, 4)?;
        let upload_eighty_percent_ns = if include_private_upload {
            ceil_ratio(
                ceil_rate_ns(self.upload_bytes()?, M4_MAX_COPY_BYTES_PER_SECOND)?,
                5,
                4,
            )?
        } else {
        ...
        let projected_member_ns = [
            fold_eighty_percent_ns,
            upload_eighty_percent_ns,
            RAM_RAF_TARGET_CPU_TAIL_NS,
            RAM_RAF_FIXED_PROJECTION_NS,
        ]
        .into_iter()
        .try_fold(0u64, |total, time| {
            total.checked_add(time).ok_or(RamRafError::SizeOverflow {
                label: "projected member time",
            })
        })?;
        Ok(RamRafProjection {
            evidence: RamRafEvidence::AnalyticalProjection,
            modeled_bytes,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct RamRafProjection {
    pub evidence: RamRafEvidence,
    pub modeled_bytes: u64,
    pub products: u64,
    pub traffic_floor_ns: u64,
    pub product_floor_ns: u64,
    pub fold_eighty_percent_ns: u64,
    pub upload_eighty_percent_ns: u64,
    pub projected_member_ns: u64,
}

impl RamRafProjection {
    pub fn projected_speedup(self) -> f64 {
        RAM_RAF_TARGET_CPU_NS as f64 / self.projected_member_ns as f64
    }

    pub fn clears_five_x(self) -> bool {
        self.projected_member_ns.saturating_mul(5) <= RAM_RAF_TARGET_CPU_NS
    }
}
...
pub enum RamRafError {
    ...
    #[error("live subtotal count exceeds {maximum}: {got}")]
    TooManyLiveSubtotals { maximum: usize, got: usize },
...
fn checked_bytes_u64(label: &'static str, count: usize, width: usize) -> Result<u64, RamRafError> {
    let bytes = checked_product(label, count, width)?;
    u64::try_from(bytes).map_err(|_| RamRafError::SizeOverflow { label })
}
...
fn ceil_rate_ns(count: u64, rate_per_second: u64) -> Result<u64, RamRafError> {
    let numerator =
        u128::from(count)
            .checked_mul(1_000_000_000)
            .ok_or(RamRafError::SizeOverflow {
                label: "roofline numerator",
            })?;
    let result = numerator.div_ceil(u128::from(rate_per_second));
    u64::try_from(result).map_err(|_| RamRafError::SizeOverflow {
        label: "roofline time",
    })
}

fn ceil_ratio(value: u64, numerator: u64, denominator: u64) -> Result<u64, RamRafError> {
    let scaled =
        u128::from(value)
            .checked_mul(u128::from(numerator))
            .ok_or(RamRafError::SizeOverflow {
                label: "roofline ratio",
            })?;
    let result = scaled.div_ceil(u128::from(denominator));
    u64::try_from(result).map_err(|_| RamRafError::SizeOverflow {
        label: "roofline ratio",
    })
}
