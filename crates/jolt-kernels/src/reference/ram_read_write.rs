...
//! address-major layout (`index = k·2^log_T + j`), reordered to bind
//! `LowToHigh` in the configured phase order. The cycle-indexed `inc` and
//! eq tables are tiled across the address dimension before reordering.
...
impl<F: JoltField> PrepareKernel<F, RamReadWriteChecking<F>> for ReferenceBackend {
    fn prepare(
        &self,
    ...
    ) -> Result<Box<dyn SumcheckKernel<F, Relation = RamReadWriteChecking<F>>>, KernelError<F>>
    {
        ...
        let opening_tables = BTreeMap::from([
            (ram_val(), layout.table(dense_view(witness, ram_val())?)?),
            (ram_ra(), layout.table(dense_view(witness, ram_ra())?)?),
            (
                ram_inc(),
                layout.table(tile(&dense_view(witness, ram_inc())?, copies))?,
            ),
        ]);
        let derived_tables = BTreeMap::from([(
            JoltDerivedId::from(RamReadWritePublic::EqCycle),
            layout.table(tile(&eq_table(tau_low), copies))?,
        )]);
