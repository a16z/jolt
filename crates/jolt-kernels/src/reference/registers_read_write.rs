...
//! layout (`index = k·2^log_T + j`), reordered to bind `LowToHigh` in the
//! configured phase order. The cycle-indexed `rd_inc` and eq tables are
//! tiled across the register dimension before reordering.
...
impl<F: JoltField> PrepareKernel<F, RegistersReadWriteChecking<F>> for ReferenceBackend {
    ...
    ) -> Result<Box<dyn SumcheckKernel<F, Relation = RegistersReadWriteChecking<F>>>, KernelError<F>>
    {
        ...
        let layout = ReadWriteTableLayout::joint::<F>(dimensions)?;

        let copies = 1usize << dimensions.log_k();
        let opening_tables = BTreeMap::from([
            (
                registers_val_read_write(),
                layout.table(dense_view(witness, registers_val_read_write())?)?,
            ),
            (
                rs1_ra_read_write(),
                layout.table(dense_view(witness, rs1_ra_read_write())?)?,
            ),
            (
                rs2_ra_read_write(),
                layout.table(dense_view(witness, rs2_ra_read_write())?)?,
            ),
            (
                rd_wa_read_write(),
                layout.table(dense_view(witness, rd_wa_read_write())?)?,
            ),
            (
                rd_inc_read_write(),
                layout.table(tile(&dense_view(witness, rd_inc_read_write())?, copies))?,
            ),
        ]);
        let derived_tables = BTreeMap::from([(
            JoltDerivedId::from(RegistersReadWritePublic::EqCycle),
            layout.table(tile(&eq_table(r_cycle), copies))?,
        )]);
