    ...
    PCS: CommitmentScheme<Field = F>,
{
    ...
    /// `ram_output_check`, `ram_ra_virtualization`, and the commit /
    /// joint-opening slots). Same construction bounds as the reference
    /// backend (the commit slot is the reference streaming implementation),
    /// plus the accumulator bound the registers kernels' compact-scalar
    /// walks need.
