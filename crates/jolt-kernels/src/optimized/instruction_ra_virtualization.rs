...
pub(crate) fn prepare_optimized_instruction_ra_virtualization<F: Field>(
    ...
    prepare_instruction_ra_from_initialization(session, witness, initialization)
}

pub(crate) fn prepare_instruction_ra_initialization<F: Field>(
    inputs: ProverInputs<'_, F, InstructionRaVirtualization<F>>,
...
pub(crate) fn prepare_instruction_ra_from_initialization<F: Field>(
    session: &mut ProofSession,
    witness: &dyn JoltWitnessPlane<F>,
...
pub(crate) struct InstructionRaInitialization<F: Field> {
    log_t: usize,
    pub(crate) num_committed_per_virtual: usize,
    pub(crate) committed_chunk_bits: usize,
    gamma_powers_inv: Vec<F>,
    pub(crate) chunk_tables: Vec<Vec<F>>,
...
impl<F: Field> InstructionRaInitialization<F> {
    fn new(
        ...
        let chunk_tables = map_indices(chunks.len(), |i| {
    ...
    pub(crate) fn into_cpu(
        ...
        let chunk_tables = std::mem::take(&mut self.chunk_tables);
        let num_committed = chunk_tables.len();
        let tables = InstructionRaTableState::Cpu(LazyFoldedRa::new(
            chunk_tables,
            LookupIndexChunks {
                rows,
                num_committed,
    ...
    pub(crate) fn into_kernel(
        self,
        num_committed: usize,
        tables: InstructionRaTableState<F>,
...
pub(crate) enum InstructionRaTableState<F: Field> {
...
pub struct OptimizedInstructionRaVirtualizationKernel<F: Field> {
    ...
    pub(crate) num_committed_per_virtual: usize,
    /// Only the first factor in each virtual product carries `γ^v`.
    gamma_powers_inv: Vec<F>,
    pub(crate) tables: InstructionRaTableState<F>,
...
impl<F: Field> OptimizedInstructionRaVirtualizationKernel<F> {
    ...
    fn bind_cpu(&mut self, challenge: F) -> Result<(), SumcheckError<F>> {
    ...
    fn message(
        ...
        let num_committed = self.num_committed;
        let folded_ra = self.cpu_tables()?;
...
pub(crate) fn instruction_ra_state_error<F: Field>(message: impl Into<String>) -> SumcheckError<F> {
