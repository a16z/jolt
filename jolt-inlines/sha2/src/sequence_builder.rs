...
mod tests {
    #[test]
    fn test_sha2_inline_sequence_builder_instruction_count() {
        use super::sha2_inline_sequence_builder;
        ...
        let instructions = sha2_inline_sequence_builder(asm, operands);
        
        // Get the length of the instruction sequence
        let instruction_count = instructions.len();
        
        // Print the instruction count for visibility
        println!("SHA2 inline sequence generated {} instructions", instruction_count);
        ...
        // Assert that we generated a reasonable number of instructions for SHA2 compression
        // SHA2 has 64 rounds with multiple operations per round
        assert!(instruction_count > 100, "Expected more than 100 instructions for SHA2 compression");
        
        println!("Test passed: Generated {} instructions for SHA2 compression", instruction_count);
    }

    #[test]
    fn test_sha2_init_inline_sequence_builder_instruction_count() {
        use super::sha2_init_inline_sequence_builder;
        use tracer::emulator::cpu::Xlen;
        use tracer::instruction::format::format_inline::FormatInline;
        use tracer::utils::inline_helpers::InstrAssembler;
        use tracer::utils::virtual_registers::VirtualRegisterAllocator;
        ...
        // Call the function and get the instruction sequence
        let instructions = sha2_init_inline_sequence_builder(asm, operands);
        
        // Get the length of the instruction sequence
        let instruction_count = instructions.len();
        
        // Print the instruction count for visibility
        println!("SHA2 init inline sequence generated {} instructions", instruction_count);
        
        // Assert that we generated some instructions (non-empty sequence)
        assert!(instruction_count > 0, "Expected non-empty instruction sequence");
        
        // Assert that we generated a reasonable number of instructions for SHA2 init compression
        // SHA2 init also has 64 rounds with multiple operations per round
        assert!(instruction_count > 100, "Expected more than 100 instructions for SHA2 init compression");
        
        println!("Test passed: Generated {} instructions for SHA2 init compression", instruction_count);
    }
}
