...
impl TestVectors {
    ...
    pub fn get_edge_cases() -> Vec<([u64; INPUT_LIMBS], [u64; INPUT_LIMBS], [u64; OUTPUT_LIMBS], &'static str)> {
        use ark_ff::{BigInteger, BigInteger256};
        ...
        cases.push((pow2_64, pow2_128, compute(pow2_64, pow2_128), "2^64 * 2^128"));
        ...
        cases.push((single_limb_max, single_limb_max, compute(single_limb_max, single_limb_max), "Single limb MAX * Single limb MAX"));
        
        // Edge case 13: Alternating bits pattern
        let alternating = [0xAAAAAAAAAAAAAAAA, 0x5555555555555555, 0xAAAAAAAAAAAAAAAA, 0x5555555555555555];
        cases.push((alternating, alternating, compute(alternating, alternating), "Alternating bits * Alternating bits"));
        
        // Edge case 14: Only MSB set in each limb
        let msb_only = [0x8000000000000000; INPUT_LIMBS];
        cases.push((msb_only, msb_only, compute(msb_only, msb_only), "MSB only * MSB only"));
        
        // Edge case 15: Only LSB set in each limb  
        let lsb_only = [1; INPUT_LIMBS];
        cases.push((lsb_only, lsb_only, compute(lsb_only, lsb_only), "LSB only * LSB only"));
        
        // Edge case 16: Sequential values
        let sequential = [1, 2, 3, 4];
        let sequential2 = [5, 6, 7, 8];
        cases.push((sequential, sequential2, compute(sequential, sequential2), "Sequential values"));
        
        // Edge case 17: Only highest limb non-zero
        let high_limb_only = [0, 0, 0, u64::MAX];
        cases.push((high_limb_only, high_limb_only, compute(high_limb_only, high_limb_only), "High limb only * High limb only"));
        
        // Edge case 18: Only lowest limb non-zero
        let low_limb_only = [u64::MAX, 0, 0, 0];
        cases.push((low_limb_only, low_limb_only, compute(low_limb_only, low_limb_only), "Low limb only * Low limb only"));
        
        // Edge case 19: Middle limbs only
        let middle_limbs = [0, u64::MAX, u64::MAX, 0];
        cases.push((middle_limbs, middle_limbs, compute(middle_limbs, middle_limbs), "Middle limbs * Middle limbs"));
        
        // Edge case 20: Prime-like pattern (Mersenne prime approximation)
        let mersenne_like = [u64::MAX, u64::MAX, u64::MAX, 0x7FFFFFFFFFFFFFFF];
        cases.push((mersenne_like, two, compute(mersenne_like, two), "Mersenne-like * 2"));
