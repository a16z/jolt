...
#
# Why `cargo run -p stdlib` instead of `jolt run` on a single ELF?
...
#
# This runs the stdlib host driver which internally uses `jolt build`
...
# The stdlib host driver internally uses `jolt build -p stdlib-guest --mode std ...`
