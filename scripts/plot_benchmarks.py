        ...
        with open(csv_path, 'r') as f:
            ...
            for row in reader:
                if len(row) >= 7 and row[1].isdigit():  # Skip header
                    ...
                    data[name].append((int(scale), float(time), int(size), int(size_comp)))
...
def create_speed_plot(data, output_path):
    ...
    for i, (name, points) in enumerate(data.items()):
        scales, times = zip(*[(s, 2**s / (t * 1000)) for s, t, _, _ in sorted(points)])
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Trace length (RISCV64IMAC Cycles)", tickmode='array', tickvals=ticks, ticktext=labels),
        yaxis=dict(title="Prover Speed (Cycles proved per millisecond, aka KHz)"),
...
def create_size_plot(data, output_path):
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Trace length (RISCV64IMAC Cycles)", tickmode='array', tickvals=tick_vals, ticktext=tick_labels),
...
def main():
    ...
    parser.add_argument('--plot-type', choices=['all', 'speed', 'size'], default='all',
    ...
    if not data:
        print("No data found in CSV file. Run ./scripts/jolt_benchmarks.sh to generate data.")
        return
    
    print(f"Loaded {len(data)} benchmark types from CSV")
    
    if args.plot_type in ['all', 'speed']:
        create_speed_plot(data, os.path.join(args.output_dir, 'benchmark_plot.html'))
    
    if args.plot_type in ['all', 'size']:
        create_size_plot(data, os.path.join(args.output_dir, 'proof_size_plot.html'))
