...
echo
echo 'RUSTFLAGS="-C target-cpu=native -C opt-level=3 -C codegen-units=1 -C embed-bitcode=yes" RUST_LOG=info chrt -f 80 cargo run --release -p jolt-prover-legacy profile --name sha2-chain --format chrome'
