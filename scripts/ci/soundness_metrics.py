...
def find_error_enums(source: str) -> dict[str, tuple[int, int, list[str]]]:
    ...
    while i < len(lines):
        ...
        if not opened or depth != 0:
            # Unterminated body (macro trickery or malformed source): skip only
            # this header line so later enums in the file are still inventoried,
...
TARGET_CTOR_RE = re.compile(
    r"(?:checked_standard|checked_both|later_standard|final_opening_standard|"
    r"final_opening_zk|zk_target)\("
)


def tamper_ratio(manifest_source: str) -> tuple[int, int]:
    """(active, total) targets, counted per constructor call.

    Anchored to the target-constructor invocations rather than raw token
    counting, so comments, helper bodies, and comparison expressions that
    mention `TamperCoverage::...` cannot inflate the ratio. Each constructor
    call's coverage is the first `TamperCoverage::` token before the next
    constructor call (or end of file).
    """
    starts = [m.start() for m in TARGET_CTOR_RE.finditer(manifest_source)]
    active = total = 0
    for index, start in enumerate(starts):
        end = starts[index + 1] if index + 1 < len(starts) else len(manifest_source)
        coverage = re.search(
            r"TamperCoverage::(Active|IgnoredUntilFixture|Deferred)\b",
            manifest_source[start:end],
        )
        if coverage is None:
            continue  # a constructor definition, not a target entry
        total += 1
        if coverage.group(1) == "Active":
            active += 1
    return active, total
...
"""
    ...
    def test_tamper_ratio_counts_constructor_calls_only(self):
        source = """\
// TamperCoverage::Active mentioned in a comment must not count
const fn checked_standard(coverage: TamperCoverage) -> TamperTarget {
    TamperTarget { coverage }
}
pub const TARGETS: &[TamperTarget] = &[
    checked_standard("a", "a", Phase::S1, Strategy::Offset,
        TamperCoverage::Active, "reason"),
    zk_target("b", "b", Strategy::Offset,
        TamperCoverage::Deferred, "reason"),
    checked_both("c", "c", Phase::S2, Strategy::Flip,
        TamperCoverage::Active, "reason"),
];
fn audit() { assert!(matches!(t.coverage, TamperCoverage::Active)); }
        """
        # the assert! sits after the last constructor call in this source, so
        # anchoring must attribute exactly one coverage token per call
        self.assertEqual(tamper_ratio(source), (2, 3))

    def test_unterminated_enum_does_not_swallow_later_enums(self):
        source = """\
...
"""
        # The malformed first body never balances; the parser must still
        # inventory the enum that follows instead of consuming the file.
        enums = find_error_enums(source)
        self.assertIn("LaterError", enums)
        self.assertEqual(enums["LaterError"][2], ["Real"])
        self.assertNotIn("BrokenError", enums)
...
def main() -> int:
    ...
    repo_root = coverage_gate.repo_root_from(args.config.resolve().parent)
