...
def repo_root_from(start: Path) -> Path:
    """Nearest ancestor of `start` containing Cargo.toml (the workspace root)."""
    for candidate in [start, *start.parents]:
        if (candidate / "Cargo.toml").is_file():
            return candidate
    raise SystemExit(f"no Cargo.toml found above {start}")
...
def undeclared_features(metadata: dict, crates: dict[str, dict]) -> dict[str, list[str]]:
    """Per-crate features that no declared [paths] entry compiles.

    Code gated behind these features never enters a coverage run, so it is
    invisible to the floors (absent from the denominator, not counted as
    uncovered). Reported informationally so the blind spot stays visible.
    """
    packages = workspace_packages(metadata)
    result: dict[str, list[str]] = {}
    for name, info in crates.items():
        features = packages[name]["features"]
        declared: set[str] = {"default"}
        for path in info["paths"]:
            if path["features"]:
                declared.update(path["features"].split(","))
        # everything a declared path compiles, expanded transitively
        compiled: set[str] = set()
        stack = [f for f in declared if f in features]
        while stack:
            feature = stack.pop()
            if feature in compiled:
                continue
            compiled.add(feature)
            for enabled in features[feature]:
                if enabled in features and enabled not in compiled:
                    stack.append(enabled)
        missing = sorted(f for f in features if f not in compiled and f != "default")
        if missing:
            result[name] = missing
    return result


def report_undeclared_features(metadata: dict, crates: dict[str, dict]) -> None:
    missing = undeclared_features(metadata, crates)
    if not missing:
        return
    print(
        "note: features not compiled by any declared [paths] entry "
        "(their gated code is invisible to the floors):",
        file=sys.stderr,
    )
    for name in sorted(missing):
        print(f"  {name}: {', '.join(missing[name])}", file=sys.stderr)
...
def line_execution_counts(file_entry: dict) -> dict[int, int]:
    """Per-line execution count reconstructed from llvm-cov segments.
    ...
    regions terminate ranges but never mark lines themselves. Mirroring
    llvm-cov's line table: on a line where any region *entry* starts, the max
    entry count wins; other lines take the count carried over them by the
    enclosing segment. Without the entry/carried split, a continuation
    segment's count would bleed onto lines whose own entries are uncovered.
    """
    segments = file_entry.get("segments", [])
    entry_counts: dict[int, int] = {}
    carried_counts: dict[int, int] = {}
    for idx, seg in enumerate(segments):
        line, _col, count, has_count, is_entry, is_gap = seg[:6]
        if not has_count or is_gap:
            continue
        # end-exclusive: the next segment's start line carries its own count
        end_line = segments[idx + 1][0] if idx + 1 < len(segments) else line + 1
        if is_entry:
            entry_counts[line] = max(entry_counts.get(line, 0), count)
            carry_from = line + 1
        else:
            carry_from = line
        for covered_line in range(carry_from, max(end_line, line + 1)):
            carried_counts[covered_line] = max(carried_counts.get(covered_line, 0), count)
...
def cmd_check_config(metadata: dict, config: dict) -> int:
    ...
    report_undeclared_features(metadata, crates)
...
class GateTests(unittest.TestCase):
    ...
    def test_entry_segments_take_precedence_over_carried_counts(self):
        entry = {
            "filename": "/ws/field/src/lib.rs",
            "segments": [
                # continuation segment carrying count 4 over lines 8-9
                [8, 1, 4, True, False, False],
                # an uncovered region entry on line 8: llvm's line table
                # reports the line by its entries, so line 8 must be 0
                [8, 5, 0, True, True, False],
                [10, 1, 0, False, False, False],
            ],
        }
        counts = line_execution_counts(entry)
        self.assertEqual(counts[8], 0)
        self.assertEqual(counts[9], 0)

    def test_undeclared_features_reports_paths_blind_spot(self):
        cfg = self.config(paths={"claims": ["default", "extra"]})
        crates = in_scope_crates(self.metadata(), cfg)
        missing = undeclared_features(self.metadata(), crates)
        # field declares a `solinas` feature but no path compiles it
        self.assertEqual(missing, {"field": ["solinas"]})
...
def main() -> int:
    ...
    metadata = load_cargo_metadata(repo_root_from(args.config.resolve().parent))
