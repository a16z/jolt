...
impl<F: JoltField, ProofTranscript: Transcript> RAMTwistProof<F, ProofTranscript> {
    ...
    pub fn prove<PCS: CommitmentScheme<ProofTranscript, Field = F>>(
        ...
        // TODO: Reuse from ReadWriteCheckingProof
        ...
        let deltas: Vec<Vec<i64>> = trace[..T - chunk_size]
            ...
            .map(|trace_chunk| {
                ...
                for cycle in trace_chunk.iter() {
                    let ram_op = cycle.to_memory_ops()[2];
                    if let MemoryOp::Write(address, pre_value, post_value) = ram_op {
                        let increment = post_value as i64 - pre_value as i64;
                        let k = address as usize; // TODO: Will probably need to add 1 to preserve no-op address
                        delta[k] += increment;
                    }
                }
                delta
