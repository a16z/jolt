use jolt::{end_cycle_tracking, start_cycle_tracking, AdviceTapeIO};
...
#[jolt::advice]
fn factor_u16(n: u16) -> jolt::UntrustedAdvice<(u16, u16)> {
    let mut a = 1u16;
    let mut b = n;
    for i in 2..=n {
        if n % i == 0 {
...
#[jolt::advice]
fn factor_u64(n: u64) -> jolt::UntrustedAdvice<(u64, u64)> {
    let mut a = 1u64;
    let mut b = n;
    for i in 2..=n {
        if n % i == 0 {
...
fn advice_demo(n: u8, a: Vec<usize>, b: Vec<usize>) {
    // Exercise all the advice functions with a simple composite number test
