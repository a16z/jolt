...
pub fn main() {
    ...
    let mut program = compile_fr_poseidon2_sdk(target_dir);
    ...
    let (output, bundle) = prove_fr_poseidon2_sdk(&mut program, s0, s1, s2)
        .expect("modular prove succeeds on fr_poseidon2_sdk");
    let prove_secs = prove_start.elapsed().as_secs_f64();

    let verify_start = Instant::now();
    let verify_result = verify_fr_poseidon2_sdk(&bundle, &mut program);
    let verify_secs = verify_start.elapsed().as_secs_f64();
    let valid = verify_result.is_ok();

    info!("=== bn254-fr-poseidon2-sdk (modular Bolt backend, native FR coprocessor) ===");
