...
use jolt::{
    Bn254Curve, JoltDevice, JoltVerifierPreprocessing, RV64IMACProof, RV64IMACVerifier, F, PCS,
};
...
fn verify(bytes: &[u8]) -> u32 {
    ...
    let verifier_preprocessing: JoltVerifierPreprocessing<F, Bn254Curve, PCS> =
        JoltVerifierPreprocessing::<F, Bn254Curve, PCS>::deserialize_with_mode(&mut cursor, Compress::Yes, Validate::No).unwrap();
