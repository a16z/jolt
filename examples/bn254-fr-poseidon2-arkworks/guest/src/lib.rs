...
fn poseidon2_permute(state: &mut [Fr; 3]) {
    ...
    while r < 60 {
        state[0] = state[0] + rc_element(r, 0);
...
fn add_rc_full(state: &mut [Fr; 3], round: usize) {
    state[0] = state[0] + rc_element(round, 0);
    state[1] = state[1] + rc_element(round, 1);
    state[2] = state[2] + rc_element(round, 2);
}

#[inline]
fn sbox_full(state: &mut [Fr; 3]) {
    state[0] = sbox_p(&state[0]);
    state[1] = sbox_p(&state[1]);
...
fn rc_element(round: usize, idx: usize) -> Fr {
    fr_from_limbs(RC3[round][idx])
}

// Round constants — transcribed from HorizenLabs Poseidon2 BN256 reference
// (`plain_implementations/src/poseidon2/poseidon2_instance_bn256.rs`, `RC3`).
// Each Fr is stored as little-endian `[u64; 4]` limbs. Rows 4..=59 have
// zero entries in indices 1 and 2 (internal rounds add to state[0] only).
pub const RC3: [[[u64; 4]; 3]; 64] = [
    [
        [
            0x59a09a1a97052816,
            0x7f8fcde48bb4c37a,
