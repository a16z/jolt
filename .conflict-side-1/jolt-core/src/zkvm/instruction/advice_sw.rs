...
use tracer::instruction::{advice_sw::AdviceSW, RISCVCycle};

use crate::zkvm::lookup_table::{range_check::RangeCheckTable, LookupTables};

use super::{CircuitFlags, Flags, InstructionLookup, LookupQuery, NUM_CIRCUIT_FLAGS};

impl<const XLEN: usize> InstructionLookup<XLEN> for AdviceSW {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(RangeCheckTable.into())
    }
}

impl Flags for AdviceSW {
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<AdviceSW> {
    fn to_instruction_inputs(&self) -> (u64, i128) {
        (0, 0)
    }

    fn to_lookup_output(&self) -> u64 {
        // The advice store instructions don't have a traditional output;
        // they store advice data to memory
        0
    }
}
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, AdviceSW>();
    }
}
