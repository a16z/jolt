...
**Macro parameters** — start with `#[jolt::provable(stack_size = 8388608)]` as a baseline (8 MB stack matches the Linux default and avoids most overflow surprises). Only add other parameters when you have a reason:

| Parameter | Recommended start | When to change | How to pick a value |
|-----------|------------------|----------------|---------------------|
| `stack_size` | 8388608 (8 MB) | `stack overflow` | 8 MB covers almost all cases; reduce only in the optimization pass. |
| `max_trace_length` | bare default (2^22) | `max_trace_length exceeded` | Run `analyze_<fn>` to get actual cycle count, round up to next power of 2. **Proving time and memory scale with this** — tighten in Step 9. |
| `heap_size` | bare default (128 MB) | `heap allocation failed` | Estimate peak live allocations; halve until it fails, then double back. |
...
```rust
#[jolt::advice]
fn expensive_witness(input: &[u8]) -> [u8; 32] { ... }

#[jolt::provable]
fn my_fn(input: &[u8]) -> bool {
    let result = expensive_witness(input);
    jolt::check_advice_eq!(result, compute_expected_cheaply(input));
    true
}
```
