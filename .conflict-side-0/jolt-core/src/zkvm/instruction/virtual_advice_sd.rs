...
use tracer::instruction::{virtual_advice_sd::VirtualAdviceSD, RISCVCycle};

use crate::zkvm::lookup_table::{range_check::RangeCheckTable, LookupTables};

use super::{CircuitFlags, Flags, InstructionLookup, LookupQuery, NUM_CIRCUIT_FLAGS};

impl<const XLEN: usize> InstructionLookup<XLEN> for VirtualAdviceSD {
    fn lookup_table(&self) -> Option<LookupTables<XLEN>> {
        Some(RangeCheckTable.into())
    }
}

impl Flags for VirtualAdviceSD {
...
impl<const XLEN: usize> LookupQuery<XLEN> for RISCVCycle<VirtualAdviceSD> {
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test::<Fr, VirtualAdviceSD>();
    }
}
