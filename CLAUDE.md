...
- **Machine-checked on added lines:** import types, traits, enums, and constants; reference them by short name. Keep enum variants qualified by the imported enum type (`Kind::ADD`, not bare `ADD`). Lowercase namespace free functions (`std::mem::take`) may stay qualified. Fully qualified paths remain valid for ambiguity, attribute arguments (`allocative(visit = jolt_poly::visit_scalars)`), and macro bodies. Once a path is imported, never spell it qualified in the same file.
