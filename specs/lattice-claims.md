...
> joint opening carries a further always-present dense precommitted group —
> `FieldRdInc`'s u64 limb-word columns, role-ordered after `TrustedAdvice`
> (the verifier rejects committed-program mode with field-inline enabled, so
> the direct-program roles never share a batch with it) and bound to the
> stage-6b reduced claim by one linear recomposition check (`specs/field-inline-portability.md`, the dense-group
> status section).
