...
| OPT-ZK-003 | Parallel non-coefficient row commitments | `zkvm/prover.rs::prove_blindfold` | Commits remaining BlindFold witness rows in parallel. | cpu-backend + `jolt-blindfold` | BlindFold timing benchmark. |
...
| OPT-ZK-006 | Random satisfying instance folding | `subprotocols/blindfold/folding.rs`, `protocol.rs` | Uses Nova-style folding with random satisfying instance to hide witness. | `jolt-blindfold` | BlindFold standalone and e2e tests. |
