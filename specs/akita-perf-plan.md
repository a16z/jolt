...
> Status 2026-07-13: lead 1 landed (batched one-hot W_jolt, commit
> 52decbb76); the backend is bumped to upstream tip and builds from the
> local `../akita` clone for co-design. The current bottleneck analysis and
> the committed-symbols plan live in
> `specs/akita-optimal-committed-data.md` and the akita_perf_goal
> memory ledger — read those before this section, which is kept as
> original context.
