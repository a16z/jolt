...
Later PRs should broaden the hierarchy and adoption surface: extension fields, Hachi consuming `jolt-field`, granular bound tightening across `jolt-prover`, binary-field-specific requirements, and any eventual rename of the top `Field` bundle.
...
7. Touching `jolt-poly`, `jolt-sumcheck`, `jolt-openings`, `jolt-r1cs`, `jolt-crypto`, `jolt-prover` semantics. Their bounds get relaxed where possible to enable Hachi reuse, but no behavioral changes.
...
- `cargo nextest run -p jolt-prover muldiv --features host` and `--features host,zk` pass.
...
- All tests in `crates/jolt-field/`, `crates/jolt-transcript/`, `crates/jolt-poly/`, `crates/jolt-sumcheck/`, `crates/jolt-openings/`, `crates/jolt-crypto/`, `crates/jolt-r1cs/`, `jolt-prover/`.
- `jolt-eval` invariant suites (`field_mul_scalar`, `split_eq_bind`, `soundness`).
- `jolt-prover muldiv` end-to-end in both `host` and `host,zk` modes.
...
Jolt currently computes halving at the few relevant callsites with `F::from_u64(2).inverse().unwrap()`, and this PR leaves those `jolt-prover` callsites under the existing `F: Field` umbrella.
...
Default features stay on for in-workspace consumers (jolt-prover, etc.), so no jolt-internal callsites change.
...
1. **Delete BN254-shaped capabilities entirely instead of moving them to named traits.** Rejected — loses callable functionality from the existing API and forces churn at every jolt-prover callsite that uses fixed-width bytes, sampling, accumulators, `mul_pow_2`, etc. Better to keep the capabilities available through explicit bounds.
...
5. **Put `Div` on `FieldCore`, `Field`, or anywhere reachable by shared verifier code.** Rejected — hides the zero-denominator case behind one infix operator. Arkworks BN254 division by zero panics through `inverse().unwrap()`, and the only current generic field infix-division callsite in `jolt-prover` is migrated to explicit inversion instead.
...
16. **Add a `TWO_INV` / halving capability in this PR.** Rejected — Jolt does not currently expose a `TWO_INV` trait or constant. The few existing halving callsites are in `jolt-prover`, stay under the existing `F: Field` umbrella in this PR, and can be revisited when Jolt is ready to make those paths binary-field-aware.
...
`jolt-prover`, `jolt-r1cs`, `jolt-crypto`, `jolt-eval` keep their `F: Field` bounds unchanged.
...
3. **Granular bound tightening across `jolt-prover`.** Audit every `F: Field` bound in `jolt-prover` and relax to the minimum required (e.g. `F: RingCore + CanonicalBytes + FixedByteSize`). Mechanical, but blast radius is workspace-wide. Optional follow-up; no behavioral change.
