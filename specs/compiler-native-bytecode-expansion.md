...
26. Port arithmetic, shifts, memory, division, and control-flow families.
27. Replace `InstrAssembler<'a>` in production expansion code.
28. Preserve tracer inline adapter support as finalized rows outside
    provider-free core.
29. Delete the old recursive assembler once all parity tests pass.
30. Run Hax/Aeneas again on:
   - metadata stamping,
   - allocator transitions,
   - ADDIW shallow lowering,
   - provider-free `expand_one_core`.
31. Record the separate semantics follow-up: a hand-modeled Lean transition
...
32. Run formatting, clippy, host tests, ZK tests, and dependency checks.
