...
12. Add the measured final-row capacity guard to provider-free sequence
    materialization.
13. Add an explicit `expand::buffer` boundary for per-source row storage.
14. Split sequence metadata stamping into a dedicated `expand::metadata`
    boundary.
...
36. Port arithmetic, shifts, memory, division, and control-flow families.
37. Replace `InstrAssembler<'a>` in production expansion code.
38. Preserve tracer inline adapter support as finalized rows outside
    provider-free core.
39. Delete the old recursive assembler once all parity tests pass.
40. Run Hax/Aeneas again on:
   - metadata stamping,
   - allocator transitions,
   - ADDIW shallow lowering,
   - provider-free `expand_one_core`.
41. Record the separate semantics follow-up: a hand-modeled Lean transition
...
42. Run formatting, clippy, host tests, ZK tests, and dependency checks.
