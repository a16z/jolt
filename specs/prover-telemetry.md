...
- [ ] **Allocative lane**: with `--features profiling,allocative`, the same profile command additionally emits heap flamegraph SVGs to `benchmark-runs/flamegraphs/{trace_name}_{StageLabel}_prepared.svg` — one per driver batch, taken right after `prepare_members` with every member kernel live and its tables materialized, covering the member kernels and the `ProofSession` contents. Each SVG has an exact-bytes folded-stacks twin (`.folded`), and the per-snapshot totals are folded into `summary.json`'s `heap` section (a schema field, empty when the lane is off) — so heap attribution follows the same one-stream/two-renderings rule as the time metrics. Verified by the existence check in the Execution phase-6 procedure. (Implementation note, deviation: the spec originally asked for end-of-stage `stage{N}.svg` snapshots of the clear-output carriers; those proved near-empty by construction — stage working sets free on exit, and cross-boundary carries surface inside the consuming kernel in the next batch's `_prepared` snapshot — so the boundary snapshots were replaced by the mid-stage ones, which is where the reference backend's multi-GiB tables actually show.)
