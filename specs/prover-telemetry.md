...
- `crates/jolt-kernels` (landed in #1712): spans at the trait seams — `commit_witness`, `commit_advice`, `SpartanOuterUniskip::prepare`/`::first_round_poly`, `SpartanProductUniskip::prepare`/`::first_round_poly`, `RamInitialOpeningEvaluation::evaluate`, `JointOpeningPolynomials::prepare`, `build_committed_bytecode_chunk_coeffs` — so any future optimized backend inherits attribution for free by implementing the same traits.
