...
- **Versioned span taxonomy**, documented in `crates/jolt-profiling/README.md` (the sole normative source once written; the v1 label set is the convention shipped in PR #1712, pinned in Architecture): identity-in-the-name naming (`Type::method` for leaf and kernel-seam spans, `prove_stage{N}` for stage spans, `<StageLabel>::prove` / `<Relation>::prepare` / `<Relation>::prove_round` for driver spans), tracing-level policy, and the hot-loop rule (round-granularity spans are fine; none inside per-index inner loops).
- **Artifacts.** A chrome-format profile run emits `{trace_name}.json` (chrome-format trace: Perfetto UI for humans, `trace_processor` SQL for arbitrary machine queries) and `{trace_name}.summary.json` (thin, schema-versioned aggregates) side by side in `benchmark-runs/perfetto_traces/` under the current working directory, overwriting in place. `{trace_name}` follows #1712's harness: `modular_{workload}_{scale}` with hyphens mapped to underscores and `{scale}` the decimal log2 trace length (e.g. `modular_sha2_chain_22.summary.json`); the `modular_` prefix disambiguates side-by-side legacy traces and is dropped when the legacy prover is deleted. Any consumer that knows the workload and scale can compute the artifact paths deterministically (`jolt-eval` chooses both — see Architecture).
...
- [ ] **Allocative lane**: with `--features profiling,allocative`, the same profile command additionally emits per-stage heap flamegraph SVGs to `benchmark-runs/flamegraphs/{trace_name}_stage{N}.svg`, covering at minimum each stage's clear-output carrier and the `ProofSession` contents. Verified by the existence check in the Execution phase-6 procedure.
...
crates/jolt-profiling subscriber stack          the only crate hosting subscribers
        ...
        ├── tracing-chrome layer ─────────────────► {trace_name}.json ──┬─► Perfetto UI (human)
        │     └── integrated flush-time counter rewrite                  └─► trace_processor SQL (agent, ad-hoc)
        ├── summary aggregation layer ────────────► {trace_name}.summary.json ──► jolt-eval TelemetryObjective
...
- Supporting crates (landed in #1712): `jolt-sumcheck` `prove_batch` (plus per-round `sumcheck_round`, ~log T per batch) and `prove_uniskip_clear`; `jolt-witness` `stream_witnesses`, `collect_bundles`, `TraceBackend::oracle_table`; `jolt-openings` `HomomorphicBatch::prove_batch`. Round-granularity spans are fine; none inside per-index inner loops (the overhead-budget discipline).
...
- Once `crates/jolt-profiling/README.md` is written (Execution phase 1), it is the sole normative source for the taxonomy; the label set above is its v1 content and the acceptance criteria are evaluated against it.

**Pipeline (`crates/jolt-profiling`, extended):**

- Remains the only crate hosting subscribers. Gains: (a) an in-process **summary aggregation layer** that accumulates per-label/per-stage aggregates and writes `{trace_name}.summary.json` at flush; (b) **integrated counter conversion** — `tracing-chrome` cannot emit chrome counter events (`"ph": "C"`), so the pipeline rewrites `counters.*`-prefixed events into counter events at flush time, inside the same command (this is what kills the offline `postprocess_trace.py` step); (c) counter-name unification (legacy emits `counters.memory_gb`, `jolt-profiling` emits `counters.memory_gib` — the new pipeline standardizes on `memory_gib`).
...
- **Span taxonomy schema** lives in `crates/jolt-profiling/README.md`, versioned: naming convention, the v1 pipeline label set, required fields, level policy, hot-loop rule, and the process for evolving it.
...
6. **Validate + document** — run the documented budget procedure (dark time ≤ 5%, overhead ≤ 105%, sha2-chain at scale 2^22, median of 3); check the allocative SVGs exist per the acceptance criterion; book/README/CLAUDE.md updates.
