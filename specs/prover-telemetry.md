...
- **Artifacts.** Each profile run writes into its own directory, `benchmark-runs/{timestamp}_{trace_name}/`, holding every artifact the run produces — `{trace_name}.json` (chrome trace: Perfetto UI for humans, `trace_processor` SQL for machines), `{trace_name}.summary.json` (thin, schema-versioned aggregates), `{trace_name}.memory.html`, the `.folded` heap snapshots, and the per-run CSV — and `benchmark-runs/latest_{trace_name}` is symlinked to the newest successful run. `{trace_name}` = `modular_{workload}_{scale}` (hyphens mapped to underscores, `{scale}` the decimal log2 trace length); the `modular_` prefix disambiguates side-by-side legacy artifacts and is dropped when the legacy prover is deleted. Consumers that know the workload and scale read through the `latest_` link — deterministic paths with run history preserved (`jolt-eval` chooses both components — see Architecture). (Implementation note, evolution: originally a flat `benchmark-runs/perfetto_traces/` layout overwriting in place; regrouped by run once multiple artifacts per run existed.)
