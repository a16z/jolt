...
| 5x (pre-6a) | — | `IncVirtualization` — same four consumed claims as the base inc reduction (stage-2/4/5 outputs), produces `FusedInc` + `OpFlags(Store)`. Runs as its own single-instance phase **strictly between stage 5 and 6a** (adversarial verification of the prover mechanics killed both weaker placements: a 6b sibling's output can't enter any 6b input, and a 6a *sibling* still isn't available when 6a's F-polynomials are built — `F[stage][k] = Σ_c eq(r_cycle_s, c)·ra(k,c)` needs the consumer's cycle point at 6a construction). The lattice proof carries one extra sumcheck for this phase (mode-typed slot, zero bytes in base mode). |
| 6 | `Booleanity` | lattice `Booleanity` — same relation id, output fold extended over `UnsignedIncChunk(0..N)` + `UnsignedIncMsb` |
| 6a/6b | `BytecodeReadRaf` (full mode) | + one val stage (`LATTICE_BYTECODE_VAL_STAGES`): the store claim joins the **6a input fold as a genuine sixth stage** (`γ⁵`-term; raf/pc/entry shift to `γ⁶..γ⁸`), its `F[store]` built from `eq(r_inc, ·)` exactly like the five base stages, and it rides 6b like the others (verifier evaluates the sixth StageValue from public bytecode; output fold extends mechanically). Prover cost is structural, not mechanical: `read_raf_checking.rs` is hand-shaped around `N_STAGES = 5` with the RAF injection unrolled per-stage — the lattice instance needs the 6-stage rework (index-driven RAF injection). |
